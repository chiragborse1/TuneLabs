...
from tunelabs_zoo.dataset_utils import (
    train_on_responses_only,
    standardize_data_formats,
)
...
# =========================================== TuneLabs
# TuneLabs efficient template leverages from Zephyr
tunelabs_template = \
    "{{ bos_token }}"\
...
tunelabs_ollama = _ollama_template("tunelabs")

tunelabs_eos_token = "eos_token"
CHAT_TEMPLATES["tunelabs"] = (tunelabs_template, tunelabs_eos_token, False, tunelabs_ollama,)
DEFAULT_SYSTEM_MESSAGE["tunelabs"] = "You are a helpful assistant to the user"
...
def _change_system_message(template: str, type_chat_template: str, system_message: str = None):
    ...
    if default_system_message is None:
        if system_message is not None:
            logger.warning_once(
                f"TuneLabs: You tried to change the system message for {type_chat_template}, "
    ...
    if type_chat_template is None:
        ...
        if has_placeholder:
            if system_message is None:
                raise ValueError("TuneLabs: You need to provide a system message for custom templates.")
...
def get_chat_template(
    ...
    if type(chat_template) in (list, tuple,):
        ...
        if not is_fast_tokenizer:
            pass
            # print(
            #     "TuneLabs: Not a fast tokenizer, so can't process it as of yet :(\n"\
            ...
            if map_eos_token and (not stop_word in token_mapping.values()):
                ...
                logger.warning_once(f"TuneLabs: Will map {stop_word} to EOS = {tokenizer.eos_token}.")
        ...
        raise TypeError(
            f"TuneLabs: `chat_template` must be a tuple of (your_template, eos_token,) or one of\n"\
    ...
    if use_zoo_tokenizer_patch:
        ...
        from tunelabs_zoo.tokenizer_utils import patch_tokenizer
    else:
...
def _parse_combined_prompt(combined_prompt, dataset):
    ...
    for column in possible_columns:
        if column not in dataset_columns:
            raise KeyError(
                f"TuneLabs: Your prompt includes '{column}' but this does not exist in the dataset. "\
...
def _create_formatter(possible_columns, final_optional_prompts, user_column_name):
    ...
    for j, optional_prompt in enumerate(final_optional_prompts):
        ...
        if len(needed_columns) == 0:
            raise IndexError("TuneLabs: Optional [[...]] blocks must contain at least 1 {column}.")
...
def to_sharegpt(
    ...
    if "conversations" in dataset.column_names:
        ...
        if type(convo) is list:
            raise TypeError("TuneLabs: Your dataset is probably already in ShareGPT format!")
    ...
    def __convert_to_sharegpt__(examples):
        ...
        if len(users) != len(assistants):
            raise ValueError(
                "TuneLabs: Input and output columns must have matching batch lengths. "
...
extra_eos_tokens = None,
    ...
    for extra_eos in extra_eos_tokens:
        ...
        if extra_eos not in vocab:
            raise ValueError(f"TuneLabs: `{extra_eos}` is not a singular token in the tokenizer.")

    error_msg = \
        "TuneLabs: Your prompt template must have 2 examples showing the user input {INPUT} "\
    ...
    if len(extra_eos_tokens) == 0:
        raise RuntimeError(
            "TuneLabs: Your tokenizer does not have an EOS token? Please provide one via extra_eos_tokens!"
        )

    # Check tokenizer types
    tokenizer_name = tokenizer.name_or_path.lower()
    if tokenizer_name.startswith(("tunelabs/llama-3-8b-instruct", "tunelabs/llama-3-70b-instruct")):
        # Add <|eot_id|>
        extra_eos_tokens.append("<|eot_id|>")
    elif ("<|eot_id|>" in extra_eos_tokens or "<|eot_id|>" in chat_template) and \
        tokenizer_name.startswith(("tunelabs/llama-3-8b", "tunelabs/llama-3-70b")):
        # Warn
        logger.warning(
            "TuneLabs: Base llama-3 models did not train <|eot_id|>.\n"\
    ...
    try:
        ...
        if combined != final_combined_check:
            ...
            raise RuntimeError(
                "TuneLabs: The prompt template you provided isn't correct. You gave:\n"\
                f"{combined_changed}\n\n"\
        ...
        if output_pos == -1 or input_pos == -1:
            ...
            raise RuntimeError(
                f"TuneLabs: chat_template must contain {' and '.join(missing)} "
        ...
        if len(response_part) == 0:
            raise RuntimeError(
                "TuneLabs: Could not recover a two-example structure from chat_template. "
        ...
        if found is None:
            raise RuntimeError(
                "TuneLabs: Could not locate a separator between examples in chat_template. "
                "Provide exactly two {INPUT}/{OUTPUT} pairs (and optionally {SYSTEM}). "
                f"Got: {chat_template[:200]!r}"
            )
    ...
    if count_eos == 0:
        logger.warning("TuneLabs: We automatically added an EOS token to stop endless generations.")
    ...
    if len(system_part) != 0:
        ...
        if "{SYSTEM}" in partial_system:
            if default_system_message is None:
                raise RuntimeError("TuneLabs: Please specify a default system message!")
    ...
    if input_idx == -1:
        raise RuntimeError(
            f"TuneLabs: The instruction section of the template must contain the "
            f"'{{INPUT}}' placeholder. Section: {input_part[:200]!r}"
        )
    if output_idx == -1:
        raise RuntimeError(
            f"TuneLabs: The response section of the template must contain the "
...
extra_eos_tokens = None,
    ...
    tokenizer._tunelabs_input_part  = input_part
    tokenizer._tunelabs_output_part = output_part
    if hasattr(tokenizer, "tokenizer"):
        tokenizer.tokenizer.chat_template = jinja_template
        tokenizer.tokenizer._ollama_modelfile = modelfile
        tokenizer.tokenizer._tunelabs_input_part  = input_part
        tokenizer.tokenizer._tunelabs_output_part = output_part
...
def test_chat_templates():
    ...
    correct_tokenizer = AutoTokenizer.from_pretrained("tunelabs/llama-2-7b-chat")
    correct_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    correct_tokenizer.chat_template = template
    our_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    assert(correct_prompt == our_prompt)
    ...
    # Gemma
    correct_tokenizer = AutoTokenizer.from_pretrained("tunelabs/gemma-7b-it")
    correct_prompt = correct_tokenizer.apply_chat_template(messages[1:], tokenize = False, add_generation_prompt = True)
    ...
    correct_tokenizer = AutoTokenizer.from_pretrained("tunelabs/llama-3-8b-Instruct")
    correct_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    correct_tokenizer.chat_template = template
    our_prompt = correct_tokenizer.apply_chat_template(messages, tokenize = False, add_generation_prompt = True)
    assert(correct_prompt == our_prompt)
...
def test_hf_gguf_equivalence(tokenizer, gguf_model = "./model-tunelabs.F16.gguf"):
    """
