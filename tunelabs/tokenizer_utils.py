...
os.environ["TUNELABS_IGNORED_TOKENIZER_NAMES"] = "\n".join(IGNORED_TOKENIZER_NAMES)
...
def convert_to_fast_tokenizer(slow_tokenizer, temporary_location = "_tunelabs_sentencepiece_temp"):
...
def assert_same_tokenization(slow_tokenizer, fast_tokenizer):
    ...
    try:
        ...
        # For eg see https://github.com/tunelabsai/tunelabs/issues/292
...
def fix_sentencepiece_tokenizer(
    ...
    new_tokenizer,
    token_mapping,
    temporary_location = "_tunelabs_sentencepiece_temp",
):
    ...
    try:
        ...
        try:
            ...
            if protobuf_version > Version("3.20.3"):
                raise RuntimeError(
                    f"TuneLabs: Your protobuf version = {protobuf_version} is too new.\n"
...
def fix_sentencepiece_gguf(saved_location):
    ...
    if patched > 0:
        logger.warning(
            f"TuneLabs: Patched {patched} special token(s) in {saved_location}/tokenizer.model "
    ...
    logger.warning(
        f"TuneLabs: Extending {saved_location}/tokenizer.model with added_tokens.json.\n"
...
def _load_correct_tokenizer(
    ...
    try:
        ...
        slow_tokenizer = None
        # print(
        #     f"TuneLabs: {tokenizer_name} has no tokenizer.model file.\n"\
    ...
    # Ignore Phi-4 ones as well
        ...
        if assert_same_tokenization(slow_tokenizer, fast_tokenizer):
            ...
            logger.warning(f"TuneLabs: Will load {tokenizer_name} as a legacy tokenizer.")
...
def load_correct_tokenizer(
    ...
    # Also check Llama-2 old style models
        ...
        if old_chat_template is not None and chat_template is None:
            raise RuntimeError(
                "TuneLabs: Fixing chat template failed - please file a report immediately!"
            )
        pass
...
_RENDER_DIFF_SENTINEL_A = "AAAA_0123456789_TUNELABS_RENDER_DIFF_SENTINEL"
_RENDER_DIFF_SENTINEL_B = "BBBB_0123456789_TUNELABS_RENDER_DIFF_SENTINEL"
_RENDER_DIFF_SENTINEL_C = "CCCC_0123456789_TUNELABS_RENDER_DIFF_SENTINEL"
...
def _is_strict_chat_template_mode():
    ...
    val = os.environ.get("TUNELABS_STRICT_CHAT_TEMPLATE", "0")
...
def _format_chat_template_message(
    ...
    strict_suffix = (
        "" if strict else (" Set TUNELABS_STRICT_CHAT_TEMPLATE=1 to raise instead of warn.")
    )
    if repaired:
        return (
            "TuneLabs: Patched the chat_template on `{name}` to add a "
            "{{% if add_generation_prompt %}} block. {hint}"
        ).format(name = name_or_path, hint = source_hint)
    if has_generation_block:
        return (
            "TuneLabs: The tokenizer `{name}` has a "
    ...
    return (
        "TuneLabs: The tokenizer `{name}` does not have a "
...
def _fix_chat_template_for_tokenizer(tokenizer, chat_template):
    ...
    TUNELABS_STRICT_CHAT_TEMPLATE: warn + return original (default) or raise
    RuntimeError (strict)."""
...
def check_tokenizer(
    model,
    tokenizer,
    model_name = "tunelabs/llama-2-7b-bnb-4bit",
    ...
    for j, index in enumerate(added_tokens_fast.keys()):
        if index >= max_embedding_size:
            ...
            if not _reload:
                ...
                if can_be_removed:
                    ...
                    if max(tokenizer.added_tokens_decoder.keys()) < max_embedding_size:
                        logger.warning_once(
                            f"TuneLabs loaded a broken tokenizer `{model_name}`, but managed to repair it!\n"
                ...
                # :( Failure
                raise RuntimeError(
                    f"TuneLabs tried to load `{model_name}`, but cannot succeed.\n"
                    f"Tokens {bad_tokens} with ids {bad_indices} exceeds the max vocab size of {max_embedding_size}.\n"
            ...
            try:
                ...
                logger.warning_once(
                    "TuneLabs: Tokenizer is most likely buggy, and TuneLabs failed to repair it.\n"
...
def get_tokenizer_info(tokenizer) -> dict:
    ...
    Example output::
        ...
        {
            "name_or_path": "tunelabs/Llama-3.2-1B-Instruct",
...
def patch_sft_trainer_tokenizer():
    ...
    # tunelabs/models/_utils.py:patch_linear_scaling.
    ...
    for path_to_trainer in (
        ...
        check_text = (
            ...
            "        raise RuntimeError('TuneLabs: No GPU detected. AMD ROCm users: install ROCm-enabled PyTorch -- see https://docs.tunelabs.ai/get-started/install-and-update/amd')\n"
            ...
            "        raise RuntimeError('TuneLabs currently does not support multi GPU setups - but we are working on it!')\n"
