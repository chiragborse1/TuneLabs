...
from ..save import tunelabs_save_pretrained_torchao, tunelabs_save_pretrained_gguf
import contextlib
import shutil
...
def _save_pretrained_torchao(
    ...
    def patch_tunelabs_save():
        original_causal = transformers.AutoModelForCausalLM
        original_rmtree = shutil.rmtree
        # tunelabs_save_pretrained_torchao expects AutoModelForCausalLM
        transformers.AutoModelForCausalLM = transformers.AutoModel
        # prevent tunelabs from deleting the unquantized model directory
    ...
    with patch_tunelabs_save():
        tunelabs_save_pretrained_torchao(
            inner_model,
            transformer_dir,
            tokenizer = tokenizer,
    ...
    try:
        FastSentenceTransformer._add_tunelabs_branding(save_directory)
    except:
        pass
...
def _save_pretrained_gguf(
    ...
    **kwargs,
    ...
    # TuneLabs saves + converts here; absolute for later commonpath comparison
    transformer_dir = os.path.join(save_directory, transformer_path)
    transformer_dir = os.path.abspath(transformer_dir)
    ...
    # 4. Patch environment so TuneLabs treats this embedding model correctly
    @contextlib.contextmanager
    def patch_tunelabs_gguf_save():
    ...
    # 5. Call TuneLabs's GGUF saver on the inner model targeting the transformer subdirectory
    with patch_tunelabs_gguf_save():
    ...
    if push_to_hub:
        ...
        print(f"TuneLabs: Uploading to {repo_id}...")
        try:
            ...
            print(f"TuneLabs: Uploaded to https://huggingface.co/{repo_id}")
        except Exception as e:
            print(f"TuneLabs: Upload failed: {e}")
...
def _push_to_hub_gguf(
    ...
    token = None,
    private = None,
    commit_message = "Upload GGUF SentenceTransformer model trained with TuneLabs",
    commit_description = "Upload GGUF model trained with TuneLabs 2x faster",
    max_shard_size = "5GB",
    temporary_location = "_tunelabs_temporary_saved_buffers",
    maximum_memory_usage = 0.85,
    create_pr = False,
    ...
    **kwargs,
    ...
    try:
        ...
        print(f"TuneLabs Warning: Could not create repo: {e}")

    # Convert locally in a temp dir, then upload
    with tempfile.TemporaryDirectory(prefix = "tunelabs_st_gguf_") as temp_dir:
        print(f"TuneLabs: Converting SentenceTransformer to GGUF format...")
        ...
        print(f"TuneLabs: Uploading GGUF to https://huggingface.co/{full_repo_id}...")
...
This sentence-transformers model was finetuned and converted to GGUF format using [TuneLabs](https://github.com/tunelabsai/tunelabs).
...
"""
    ...
    # Add tags
    all_tags = ["gguf", "llama-cpp", "tunelabs", "sentence-transformers"]
    if is_vlm:
...
class FastSentenceTransformer(FastModel):
    ...
    def _patch_transformer_module_save_config(transformer_module, base_config = None):
        transformer_module._tunelabs_st_managed = True
        if base_config is not None and getattr(base_config, "model_type", None):
            transformer_module._tunelabs_base_config = base_config

        if getattr(transformer_module, "_tunelabs_save_config_patched", False):
        ...
        def _save_with_base_config(self, output_path, *args, **kwargs):
            ...
            FastSentenceTransformer._save_base_config_for_processor_resume(
                getattr(self, "_tunelabs_base_config", None), output_path
            )

        transformer_module.save = types.MethodType(_save_with_base_config, transformer_module)
        transformer_module._tunelabs_save_config_patched = True
        return transformer_module
    ...
    def _patch_distilbert_v5():
        ...
        def forward(
            ...
            **kwargs,
            ...
            # patch here: tunelabs gradient checkpointing hook needs positional arguments
            return self.transformer(
                embeddings,
                attention_mask,
                **kwargs,
    ...
    @staticmethod
    def _add_tunelabs_tags(
        repo_id,
        token,
        tags = None,
    ):
        """Add TuneLabs + sentence-transformers tags to the HF Hub repo."""
        from huggingface_hub import HfApi

        api = HfApi(token = token)
        if tags is None:
            tags = []
        tags.extend(["tunelabs", "sentence-transformers"])
        try:
            api.add_tags(
                repo_id = repo_id,
                tags = tags,
    ...
    @staticmethod
    def _add_tunelabs_branding(save_directory):
        """Add TuneLabs branding to the sentence-transformers-generated README.md."""
        readme_path = os.path.join(save_directory, "README.md")
        ...
        # add tunelabs tag to frontmatter
        if "---\ntags:\n" in content:
            ...
            if re.search(pattern, content, re.MULTILINE):
                content = re.sub(pattern, r"\1- tunelabs\n", content, count = 1, flags = re.MULTILINE)

        branding = (
            "\n\nThis model was finetuned with [TuneLabs](https://github.com/tunelabsai/tunelabs).\n\n"
            '[<img src="https://raw.githubusercontent.com/tunelabsai/tunelabs/main/images/tunelabs%20made%20with%20love.png" width="200"/>](https://github.com/tunelabsai/tunelabs)\n'
        )
    ...
    def _create_transformer_module(model_name, model, tokenizer, max_seq_length, trust_remote_code):
        ...
        def is_requested_model_name(args, kwargs):
            ...
            try:
                ...
                logging.debug(
                    "TuneLabs: Could not normalize SentenceTransformer model path: %s",
                    exception,
                )
                return False
            ...
            try:
                ...
                logging.debug(
                    "TuneLabs: Could not compare SentenceTransformer model paths: %s",
                    exception,
                )
            return False
    ...
    def _is_transformer_module_ref(class_ref):
        ...
        try:
            ...
            logging.debug(
                "TuneLabs: Could not resolve SentenceTransformer module ref %r: %s",
                class_ref,
                exception,
            )
            return False
                ...
                if FastSentenceTransformer._is_transformer_module_ref(class_ref):
                    ...
                    if os.path.isdir(model_name):
                        ...
                        try:
                            ...
                            print(f"TuneLabs Warning: Could not download module {module_path}: {e}")
                            continue
                    ...
                    try:
                        ...
                        print(f"TuneLabs Warning: Failed to load module {name} ({class_ref}): {e}")

            return modules, False

        # fallback if no modules.json (non sentence-transformers models)
        print(
            "TuneLabs: No modules.json found, falling back to [Transformer, Pooling, Normalize]. This may or may not work."
        )
    ...
    # Encoder model types that benefit from native torch.compile instead of TuneLabs patching
    ENCODER_MODEL_TYPES = {
        "mpnet",
        "bert",
        "distilbert",
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            raise ImportError(
                "TuneLabs: To use `FastSentenceTransformer`, you must install `sentence-transformers`.\n"
                "Run `pip install sentence-transformers` to install it."
            )

        # if for_inference == True, skip TuneLabs optimizations to avoid torch compile issues
        if for_inference:
        ...
        if full_finetuning and (load_in_4bit or load_in_8bit):
            print(
                "TuneLabs: You selected full finetuning support, but 4bit / 8bit is enabled - disabling LoRA / QLoRA."
            )
            load_in_4bit = False
            load_in_8bit = False
            load_in_fp8 = False
        ...
        if int(load_in_4bit) + int(load_in_8bit) + int(load_in_16bit) >= 2:
            raise RuntimeError(
                "TuneLabs: Can only load in 4bit or 8bit or 16bit, not a combination!\n"
        ...
        # This bypasses TuneLabs's auto-compiler which adds @torch.compiler.disable decorators
        # that interfere with torch.compile and cause runtime errors for encoder models.
        # NOTE: The old TuneLabs path is BROKEN for encoder models with torch 2.9+ due to
        # conflicting @torch.compile and @torch.compiler.disable decorators.
        # Set TUNELABS_COMPILE_DISABLE=1 to disable torch.compile and use the old path.
        is_encoder_model = model_type.lower() in FastSentenceTransformer.ENCODER_MODEL_TYPES
        use_fast_encoder = os.environ.get("TUNELABS_COMPILE_DISABLE", "0") != "1"
        if use_fast_encoder and is_encoder_model:
            ...
            if dtype is None:
                ...
                print("TuneLabs: Device does not support bfloat16. Using float16 instead.")
                dtype = torch.float16
            ...
            if load_in_4bit:
                print(
                    f"TuneLabs: Using fast encoder path for {model_type} with 4-bit quantization{sdpa_str}"
                )
            else:
                print(
                    f"TuneLabs: Using fast encoder path for {model_type} (torch.compile{sdpa_str})"
                )
            ...
            if _use_gc and _use_gc != False:
                print("TuneLabs Warning: Gradient checkpointing is incompatible with torch.compile.")
            ...
            # Load via native SentenceTransformer (bypasses TuneLabs patching)
            ...
            st_model._tunelabs_fast_encoder = True
            ...
            def _push_to_hub_merged(self, repo_id, **push_kwargs):
                ...
                try:
                    ...
                    pass
                FastSentenceTransformer._add_tunelabs_tags(repo_id, hub_token)
                ...
                print(f"TuneLabs: Pushed to https://huggingface.co/{repo_id}")
        ...
        if is_encoder_model and load_in_4bit:
            print("TuneLabs Warning: 4-bit quantization adds ~2.3x overhead for encoder models.")
        ...
        if fp8:
            logging.info("TuneLabs: Disabling fp8 for model")
        load_in_fp8 = False

        # this is a fix for Snowflake/snowflake-arctic-embed-l-v2.0
        # it has pooler weights which we don't care about for training,
        # however tunelabs throws an exception if "TUNELABS_WARN_UNINITIALIZED" == 1 and it sees unused weights
        old_environ = os.environ.get("TUNELABS_WARN_UNINITIALIZED", "1")
        os.environ["TUNELABS_WARN_UNINITIALIZED"] = "0"
        ...
        if not has_modules_json and load_in_4bit:
            print(
                "TuneLabs: No modules.json found. This is not a sentence-transformers model.\n"
        ...
        try:
            model, tokenizer = FastModel.from_pretrained(
                ...
                tunelabs_tiled_mlp = tunelabs_tiled_mlp,
                **kwargs,
            )
        finally:
            os.environ["TUNELABS_WARN_UNINITIALIZED"] = old_environ
        ...
        def _save_pretrained_merged(self, save_directory, **kwargs):
            ...
            if self.no_modules:
                ...
                print("TuneLabs: No modules detected. Using standard merge_and_unload for saving...")
                safe_kwargs = kwargs.copy()
                # filter out TuneLabs-specific args that are not in huggingface's save_pretrained
                tunelabs_args = [
                    "save_method",
                    "temporary_location",
                    "maximum_memory_usage",
                ]
                for k in tunelabs_args:
            ...
            # add TuneLabs branding to the generated README
            try:
                FastSentenceTransformer._add_tunelabs_branding(save_directory)
            except Exception as e:
                print(f"TuneLabs Warning: Failed to add branding to README: {e}")
        ...
        def _push_to_hub_merged(self, repo_id, **kwargs):
            ...
            FastSentenceTransformer._add_tunelabs_tags(repo_id, token)
            ...
            print(f"TuneLabs: Successfully pushed merged model to https://huggingface.co/{repo_id}")
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        if isinstance(model, SentenceTransformer):
            # Check if this is a fast encoder model (uses torch.compile instead of TuneLabs patching)
            is_fast_encoder = getattr(model, "_tunelabs_fast_encoder", False)
            ...
            if is_fast_encoder:
                ...
                if is_quantized:
                    ...
                    try:
                        ...
                        print("TuneLabs: Prepared quantized model for k-bit training")
                        ...
                        if "does not support gradient checkpointing" in str(e):
                            ...
                            print(
                                f"TuneLabs Warning: {inner_model.__class__.__name__} does not support gradient checkpointing. Skipping."
                            )
                            inner_model = prepare_model_for_kbit_training(
                                inner_model,
                            ...
                            print(
                                "TuneLabs: Prepared quantized model for k-bit training (without gradient checkpointing)"
                            )
                        else:
                            raise
                ...
                # Enable gradient checkpointing if requested (only for non-quantized, since prepare_model handles it)
                    ...
                    if hasattr(inner_model, "gradient_checkpointing_enable"):
                        try:
                            ...
                            print("TuneLabs: Enabled gradient checkpointing")
                            gc_enabled = True
                        except ValueError as e:
                            if "does not support gradient checkpointing" in str(e):
                                print(
                ...
                if compile_mode is None and not gc_enabled:
                    compile_mode = "default"
                    print(
                        "TuneLabs: Re-enabling torch.compile since gradient checkpointing is not supported"
                    )
                ...
                if compile_mode is not None:
                    ...
                    print(
                        f"TuneLabs: torch.compile will be applied automatically if max_steps > {model._compile_threshold}"
                    )
                else:
                    model._compile_mode = None
                    model._compile_pending = False
                    print("TuneLabs: torch.compile disabled (gradient checkpointing enabled)")
...
def _patch_sentence_transformer_trainer():
    ...
    if getattr(SentenceTransformerTrainer, "_tunelabs_auto_compile_patched", False):
    ...
    def _patched_init(self, *args, **kwargs):
        ...
        if (
            ...
            if max_steps > 0 and max_steps >= threshold:
                print(f"TuneLabs: Auto-compiling model ({max_steps} steps >= {threshold} threshold)")
                FastSentenceTransformer._apply_torch_compile(model, mode = compile_mode)
                model._compile_pending = False
            elif max_steps > 0:
                print(
                    f"TuneLabs: Skipping torch.compile ({max_steps} steps < {threshold} threshold)"
                )
                model._compile_pending = False
        ...
        if os.environ.get("TUNELABS_FORCE_FLOAT32", "0") == "1":
            if hasattr(self, "args") and self.args is not None:
                if self.args.fp16 or self.args.bf16:
                    print(
                        "TuneLabs: Switching to float32 training since model cannot work with float16"
                    )
    ...
    SentenceTransformerTrainer._tunelabs_auto_compile_patched = True
...
def _patch_st_trainer_load_from_checkpoint():
    try:
        ...
        return
    if getattr(SentenceTransformerTrainer, "_tunelabs_load_from_checkpoint_patched", False):
        return
    ...
    def _tunelabs_load_from_checkpoint(self, checkpoint_path):
        try:
        ...
        if not isinstance(inner, PeftModel):
            return _original(self, checkpoint_path)
        if not getattr(mod0, "_tunelabs_st_managed", False):
            return _original(self, checkpoint_path)
        ...
        if isinstance(adapter_name, (list, tuple, set)):
            if len(adapter_name) != 1:
                raise RuntimeError("TuneLabs: Cannot resume multiple active PEFT adapters.")
            adapter_name = next(iter(adapter_name))
        adapter_name = adapter_name or "default"
        if adapter_name not in getattr(inner, "peft_config", {}):
            raise RuntimeError(f"TuneLabs: PEFT adapter {adapter_name!r} is not loaded.")
        ...
        if unexpected or missing:
            raise RuntimeError(
                "TuneLabs: PEFT checkpoint does not match the active adapter "
                f"(missing={missing[:8]}, unexpected={unexpected[:8]})."
            )

        modules_json = os.path.join(checkpoint_path, "modules.json")
        if not os.path.isfile(modules_json):
            raise RuntimeError("TuneLabs: PEFT checkpoint is missing modules.json.")
        try:
            with open(modules_json, "r") as f:
                module_configs = json.load(f)
        except Exception as e:
            raise RuntimeError("TuneLabs: Cannot parse checkpoint modules.json.") from e
        ...
        for entry in module_configs:
            ...
            if idx < 0 or idx >= len(self.model):
                raise RuntimeError(f"TuneLabs: Bad module index in modules.json: {idx}.")
            module = self.model[idx]
            module_cls = type(module)
            saved_type = entry.get("type", "")
            if saved_type and not saved_type.endswith(f".{module_cls.__name__}"):
                raise RuntimeError(f"TuneLabs: Checkpoint module {idx} type mismatch.")
            ...
            if not module_path or not inside_root or not os.path.isdir(module_dir):
                raise RuntimeError(f"TuneLabs: Bad checkpoint module path for index {idx}.")
            if not hasattr(module_cls, "load"):
                raise RuntimeError(f"TuneLabs: Module {idx} cannot be reloaded.")
            fresh = module_cls.load(module_dir)
            if not isinstance(fresh, module_cls):
                raise RuntimeError(f"TuneLabs: Module {idx} reload returned wrong type.")
        ...
        if missing_idx:
            raise RuntimeError(
                f"TuneLabs: Checkpoint modules.json is incomplete (missing idx={missing_idx[:8]})."
            )

    SentenceTransformerTrainer._load_from_checkpoint = _tunelabs_load_from_checkpoint
    SentenceTransformerTrainer._tunelabs_load_from_checkpoint_patched = True
