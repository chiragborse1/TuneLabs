...
TuneLabs's autoregressive kernel/compile patching and load the unmodified HF model (outputs stay
...
def _resolve_diffusion_model_class(config):
    ...
    for name in (
        ...
        if cls is not None:
            return cls
    raise RuntimeError(
        f"TuneLabs: could not resolve a diffusion model class from architectures={archs}. "
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if not is_diffusion_model_type(model_type):
            raise RuntimeError(
                f"TuneLabs: FastDiffusionModel only supports diffusion model_types {DIFFUSION_MODEL_TYPES}, "
        ...
        print(f"==((  TuneLabs: FastDiffusionModel (slow / transformers-only path)  ))==")
