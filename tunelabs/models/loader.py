...
from tunelabs_zoo.hf_utils import dtype_from_config
from tunelabs_zoo.tiled_mlp import patch_tiled_mlp
...
# Single source of truth is tunelabs_zoo.model_lists. Re-exported so callers
# doing `from tunelabs.models.loader import FORCE_FLOAT32` keep working.
# Fallback list mirrors zoo for users who upgrade tunelabs without upgrading
# tunelabs_zoo (so this module never fails at import).
try:
    from tunelabs_zoo import FORCE_FLOAT32  # noqa: F401
except ImportError:
...
def _strip_tunelabs_bnb_4bit_suffix(model_name: str) -> str:
    """Remove TuneLabs 4bit suffixes without lowercasing (HF cache dirs are case-sensitive)."""
    s = model_name
    for suffix in ("-tunelabs-bnb-4bit", "-bnb-4bit"):
...
def _fix_rope_inv_freq(model):
    ...
    but TuneLabs rotary classes lack that attr, so inv_freq stays corrupted -> wrong
    ...
    for name, module in model.named_modules():
        # TuneLabs's LlamaRotaryEmbedding and subclasses (Extended, LinearScaling,
        ...
        **kwargs,
        ...
        if fast_inference:
            if importlib.util.find_spec("vllm") is None:
                raise ImportError(
                    "TuneLabs: Please install vLLM before enabling `fast_inference`!\n"
            ...
            if DEVICE_TYPE_TORCH == "cuda":
                for i in range(DEVICE_COUNT):
                    ...
                    if "NVIDIA GB10" in str(torch.cuda.get_device_name(i)).upper():
                        print(
                            "TuneLabs: DGX Spark detected - `fast_inference=True` is currently broken as of January 2026.\n"
                            "Defaulting to native TuneLabs inference."
                        )
        ...
        if not ALLOW_BITSANDBYTES and not use_exact_model_name:
            if load_in_4bit or load_in_8bit or model_name.lower().endswith("-bnb-4bit"):
                print(
                    "TuneLabs: AMD currently is not stable with 4bit bitsandbytes. Disabling for now."
                )
            load_in_4bit = False
        ...
        if not ALLOW_PREQUANTIZED_MODELS and model_name.lower().endswith(
            ("-tunelabs-bnb-4bit", "-bnb-4bit")
        ):
            model_name = _strip_tunelabs_bnb_4bit_suffix(model_name)
        ...
        try:
            ...
            if "architecture" in autoconfig_error:
                if "qwen3_5" in autoconfig_error:
                    raise ImportError(
                        f"TuneLabs: Your transformers version of {transformers_version} does not support Qwen3.5.\n"
        ...
        if both_exist:
            raise RuntimeError(
                "TuneLabs: Your repo has a LoRA adapter and a base model.\n"
        ...
        if not is_model and not is_peft:
            ...
            if "rope_scaling" in error.lower() and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"TuneLabs: Your transformers version of {transformers_version} does not support new RoPE scaling methods.\n"
            ...
            combined_error = (
                "TuneLabs: Failed to load model. Both AutoConfig and PeftConfig loading failed.\n\n"
        ...
        if model_type == "llama":
            ...
            if scaling_type == "llama3" and not SUPPORTS_LLAMA31:
                raise ImportError(
                    f"TuneLabs: Your transformers version of {transformers_version} does not support Llama 3.1.\n"
            ...
            if not SUPPORTS_GEMMA:
                raise ImportError(
                    f"TuneLabs: Your transformers version of {transformers_version} does not support Gemma.\n"
            ...
            if not SUPPORTS_GEMMA2:
                raise ImportError(
                    f"TuneLabs: Your transformers version of {transformers_version} does not support Gemma2.\n"
            ...
            if is_bfloat16_supported() and not HAS_FLASH_ATTENTION:
                print(
                    "TuneLabs: If you want to finetune Gemma 2, install flash-attn to make it faster!\n"
            ...
            if not SUPPORTS_QWEN3 or not SUPPORTS_QWEN3_MOE:
                raise ImportError(
                    f"TuneLabs: Your transformers version of {transformers_version} does not support Qwen3.\n"
        ...
        #         raise ImportError(
        #             f"TuneLabs: Your transformers version of {transformers_version} does not support FalconH1.\n"\
        #             f"The minimum required version is 4.50.3.\n"\
        #             f'Try `pip install --upgrade "transformers>=4.50.3"`\n'\
        #             f"to obtain the latest transformers build, then restart this session."\
        #         )
        ...
        # Best effort: never crash the load (old tunelabs_zoo without the
        # zoo #710 fix raises NameError here on Python 3.13).
        try:
            patch_compiling_bitsandbytes()
        except Exception as e:
            print(f"TuneLabs: Could not patch bitsandbytes for torch.compile - {e}")
        ...
        # In case the model supports tagging, add the tunelabs tag.
        ...
        # Patch Tiled MLP
        # to turn on set TUNELABS_TILED_MLP to "arctic", "target", or "target:{GB}""
        patch_tiled_mlp_choice = os.environ.get(
            "TUNELABS_TILED_MLP", "arctic" if tunelabs_tiled_mlp else "0"
        )
        if patch_tiled_mlp_choice != "0" or tunelabs_tiled_mlp:
...
class FastModel(FastBaseModel):
    ...
    def get_peft_model(model, *args, **kwargs):
        ...
        if getattr(model, "_tunelabs_slow_diffusion", False):
    ...
    @staticmethod
    def from_pretrained(
        model_name = "tunelabs/Llama-3.2-11B-Vision-Instruct-bnb-4bit",
        max_seq_length = 2048,
        dtype = None,
        load_in_4bit = True,  # 4bit QLoRA
        load_in_8bit = False,  # 8bit  LoRA
        ...
        **kwargs,
        ...
        # These use a custom block-diffusion `generate` and a novel backbone, so we skip TuneLabs's
        ...
        # Build TUNELABS_MODEL_NAME fresh from THIS load's model types + flags; do not prepend the
        ...
        # authoritative correction is sync_tunelabs_model_name_bnb_flags(...) after check_and_disable.
        try:
            from tunelabs_zoo.utils import get_quant_type
        ...
        if load_in_fp8:
            string += "load_in_fp8"
        os.environ["TUNELABS_MODEL_NAME"] = string
        ...
        if "pixtral" in model_types_all and transformers_version < Version("4.49.0"):
            raise RuntimeError("TuneLabs: Pixtral only works on transformers >= 4.49.0." + LATEST)
        # Qwen 2.5
        elif "qwen2_5" in model_types_all and transformers_version < Version("4.49.0"):
            raise RuntimeError("TuneLabs: Qwen 2.5 only works on transformers >= 4.49.0." + LATEST)
        # Gemma 4 must be before Gemma 3N and Gemma 3
        elif "gemma4" in model_types_all:
            if not SUPPORTS_GEMMA4:
                raise RuntimeError("TuneLabs: Gemma 4 requires transformers >= 5.5.0" + LATEST)
            os.environ["TUNELABS_DISABLE_STATIC_GENERATION"] = "1"
            os.environ["TUNELABS_HIGH_PRECISION_LAYERNORM"] = "1"
        # Gemma 3N must be before Gemma 3
        elif "gemma3n" in model_types_all:
            if transformers_version < Version("4.53.0"):
                raise RuntimeError(
                    "TuneLabs: Gemma 3N only works on transformers >= 4.53.0" + LATEST
                )
            os.environ["TUNELABS_DISABLE_STATIC_GENERATION"] = "1"
            os.environ["TUNELABS_FORCE_CUSTOM_DTYPE"] = (
                "float16;torch.float16;torch.float16;"
                "if name.endswith('norm'): "
                "module._pre_set_compute_dtype = torch.float32\n"
                ";"
                "from tunelabs_zoo.temporary_patches.gemma3n import patch_Gemma3nConv_Embed_forwards; patch_Gemma3nConv_Embed_forwards()"
            )
        ...
        # Gemma 3
            ...
            if transformers_version < Version("4.50.0.dev0"):
                raise RuntimeError(
                    "TuneLabs: Gemma 3 only works on transformers >= 4.50.0." + NIGHTLY
                )
            # Set norms to float32 since anyways they get upcasted to float32
            # common in both gemma-3 and gemma-3n
            os.environ["TUNELABS_HIGH_PRECISION_LAYERNORM"] = "1"
            ...
            # See https://github.com/tunelabsai/tunelabs/issues/3385
            from tunelabs.kernels.utils import is_rdna

            if is_rdna():
                os.environ["TUNELABS_COMPILE_DISABLE"] = "partial"
        # Cohere
        elif "cohere2" in model_types_all and transformers_version < Version("4.50.0.dev0"):
            raise RuntimeError(
                "TuneLabs: Cohere's Command model only works on transformers >= 4.50.0." + NIGHTLY
            )
        # Sesame
        elif "csm" in model_types_all:
            os.environ["TUNELABS_COMPILE_DISABLE"] = "partial"  # Inference is too slow
            os.environ["TUNELABS_DISABLE_STATIC_GENERATION"] = "1"  # Sesame fails
            os.environ["TUNELABS_FORCE_CUSTOM_DTYPE"] = (
        ...
        # OLMo 2
            ...
            raise RuntimeError("TuneLabs: OLMo-2 only works on transformers >= 4.50.0." + NIGHTLY)
        # OLMo 3
        elif "olmo3" in model_types_all and transformers_version < Version("4.57.0.dev0"):
            raise RuntimeError("TuneLabs: OLMo-3 only works on transformers >= 4.57.0." + LATEST)
            ...
            for check_model_name in DISABLE_COMPILE_MODEL_NAMES:
                if check_model_name in lowered_model_name:
                    ...
                    if transformers_version < Version("4.50.0.dev0"):
                        raise RuntimeError(
                            f"TuneLabs: {check_model_name} only works on transformers >= 4.50.0."
                            + NIGHTLY
                        )
                    break
        ...
        do_logging = os.environ.get("TUNELABS_ENABLE_LOGGING", "0") == "1"
        if do_logging:
        ...
        os.environ["TUNELABS_FORCE_FLOAT32"] = "0"
        ...
        for disable_name in FORCE_FLOAT32:
            ...
            if (
                ...
                os.environ["TUNELABS_FORCE_FLOAT32"] = "1"
        ...
        with redirector:
            ...
            model_types, supports_sdpa = tunelabs_compile_transformers(
                dtype = dtype,
                model_name = model_name,
                ...
                trust_remote_code = trust_remote_code,
                tunelabs_force_compile = tunelabs_force_compile,
            )
        # Fix SDPA issues
