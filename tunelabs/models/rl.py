...
from tunelabs_zoo.compiler import create_new_function
from tunelabs_zoo.log import logger
from tunelabs_zoo.logging_utils import PatchRLStatistics
from tunelabs_zoo.rl_replacements import RL_REPLACEMENTS
...
try:
    import vllm.sampling_params as _tunelabs_vllm_sp
    if not hasattr(_tunelabs_vllm_sp, "GuidedDecodingParams"):

        class GuidedDecodingParams:
            def __init__(self, **kwargs):
                self.kwargs = kwargs

        _tunelabs_vllm_sp.GuidedDecodingParams = GuidedDecodingParams
except Exception:
    pass
...
def _patch_resume_from_checkpoint_memory(trainer_class):
    ...
    if original_train is None:
        return
    if getattr(original_train, "_tunelabs_resume_guard", False):
        return

    def _tunelabs_train_with_resume_guard(self, *args, **kwargs):
    ...
    _tunelabs_train_with_resume_guard._tunelabs_resume_guard = True
    trainer_class.train = _tunelabs_train_with_resume_guard
...
def PatchRL(FastLanguageModel):
    ...
    @contextmanager
    def tunelabs_unwrap_model_for_generation(model, *args, **kwargs):
        ...
        # mode value (e.g. "tunelabs") rather than collapsing it to a bool, so
        ...
        with unwrap_model_for_generation(model, *args, **kwargs) as unwrapped_model:
            ...
            # We must use .clone for TuneLabs since we force inference_mode
    ...
    @torch.no_grad()
    def tunelabs_prediction_step(self, model, inputs, prediction_loss_only, ignore_keys):
        """
        ...
        os.environ["TUNELABS_RETURN_LOGITS"] = "1"
        with torch.no_grad():
            if has_labels or loss_without_labels:
        ...
        os.environ["TUNELABS_RETURN_LOGITS"] = "0"
    ...
    for trainer in trainers:
        ...
        if hasattr(current_trainer, unwrap):
            try:
                setattr(current_trainer, unwrap, tunelabs_unwrap_model_for_generation)
            except:
                continue
    Trainer.prediction_step = tunelabs_prediction_step
...
if align_completion_tool_mask is None:
    ...
    def align_completion_tool_mask(
        ...
        raise RuntimeError(
            "env_mask/tool_mask GRPO requires an tunelabs_zoo build whose "
...
from tunelabs_zoo.device_type import DEVICE_TYPE, device_synchronize

# Wrap trainer with padding to right and enable training mode
import functools
from types import MethodType
try:
    from tunelabs_zoo.gradient_checkpointing import reset_tunelabs_gradient_checkpointing_buffers
except:
    def reset_tunelabs_gradient_checkpointing_buffers(): pass
def prepare_for_training_mode(f):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        # See: https://github.com/tunelabsai/tunelabs/issues/3954
        if getattr(self, '_tunelabs_training_completed', False):
            try:
                import wandb
...
@dataclass
class TuneLabs{RLConfig_name}({RLConfig_name}):
    """
    {__RLConfig_doc__}
    """
    ...
    tunelabs_num_chunks : Optional[int] = field(
        default = -1,
        metadata = {{'help': 'Chunk size to reduce memory usage. -1 is most efficient.'}},
    )
    tunelabs_logit_chunk_multiplier : Optional[int] = field(
            default = None,
            metadata = {{'help': 'Multiplier for chunked logit computations.'}},
        )
    tunelabs_grpo_mini_batch : Optional[int] = field(
        default = None,
    ...
    def __init__({RLConfig_arguments},
        ...
        tunelabs_num_chunks = -1,
        tunelabs_logit_chunk_multiplier = None,
        tunelabs_grpo_mini_batch = None,
        {max_seq_length_call}
        **kwargs,
    ):
{RLConfig_extra_args}
        ...
        self.tunelabs_num_chunks = tunelabs_num_chunks
        if tunelabs_grpo_mini_batch is not None:
            if self.generation_batch_size >= tunelabs_grpo_mini_batch:
                self.tunelabs_grpo_mini_batch = tunelabs_grpo_mini_batch
            else:
                raise ValueError(
                    f"TuneLabs GRPO mini batch size needs to be less than or equal to the effective generation batch size, "
                    f"which is self.per_device_train_batch_size * gradient_accumulation_steps."
                )
        self.tunelabs_logit_chunk_multiplier = tunelabs_logit_chunk_multiplier
        {max_seq_length_post}
{RLConfig_post}
pass
...
class TuneLabs{RLTrainer_name}(_TuneLabs{RLTrainer_name}):
    """
    {__RLTrainer_doc__}
    """
    def __init__({RLTrainer_arguments},
        **kwargs
        ...
        if args is None: args = TuneLabs{RLConfig_name}()
...
def _wrap_grpo_generate_and_score(trainer_cls):
    ...
    if getattr(original, "_tunelabs_restore_training_wrapped", False):
        return
    ...
    wrapped._tunelabs_restore_training_wrapped = True
    trainer_cls._generate_and_score_completions = wrapped


_TUNELABS_RETURN_HIDDEN_STATES_SUPPORT_MARKER = "__TUNELABS_SUPPORTS_RETURN_HIDDEN_STATES__"
_TUNELABS_GRPO_HIDDEN_STATES_WRAPPED_ATTR = "_tunelabs_grpo_hidden_states_forward_wrapped"
_TUNELABS_GRPO_HIDDEN_STATES_WARNING_ATTR = "_tunelabs_grpo_hidden_states_warning_issued"
...
def _model_supports_tunelabs_return_hidden_states(model):
    target_model = _grpo_hidden_states_wrap_target(model)
    for candidate in (model, target_model):
        if candidate is None:
            continue
        if getattr(candidate, _TUNELABS_RETURN_HIDDEN_STATES_SUPPORT_MARKER, False):
            return True
        if getattr(type(candidate), _TUNELABS_RETURN_HIDDEN_STATES_SUPPORT_MARKER, False):
            return True
    return False
...
def _get_num_logits_to_keep(forward_signature, args, kwargs):
    try:
        ...
        logger.debug(
            "TuneLabs: Could not bind forward arguments for GRPO hidden-state fallback.",
            exc_info = True,
        )
...
def _warn_grpo_hidden_states_fallback_once(model, message):
    if getattr(model, _TUNELABS_GRPO_HIDDEN_STATES_WARNING_ATTR, False):
        return
    setattr(model, _TUNELABS_GRPO_HIDDEN_STATES_WARNING_ATTR, True)
...
def _install_grpo_hidden_states_forward_wrapper(model):
    if model is None or getattr(model, _TUNELABS_GRPO_HIDDEN_STATES_WRAPPED_ATTR, False):
        return False
    if _model_supports_tunelabs_return_hidden_states(model):
        return False

    target_model = _grpo_hidden_states_wrap_target(model)
    if getattr(target_model, _TUNELABS_GRPO_HIDDEN_STATES_WRAPPED_ATTR, False):
        setattr(model, _TUNELABS_GRPO_HIDDEN_STATES_WRAPPED_ATTR, True)
        return False
    ...
    def wrapped_forward(*args, **kwargs):
        if os.environ.get("TUNELABS_RETURN_HIDDEN_STATES", "0") != "1":
        ...
        try:
            ...
            _warn_grpo_hidden_states_fallback_once(
                target_model,
                f"TuneLabs: GRPO fallback could not request hidden states for unsupported model {model_name}; using logits directly.",
            )
            return original_forward(*args, **kwargs)
        ...
        if hidden_states is None or len(hidden_states) == 0:
            _warn_grpo_hidden_states_fallback_once(
                target_model,
                f"TuneLabs: GRPO fallback did not receive hidden states for unsupported model {model_name}; using logits directly.",
            )
            return outputs
    ...
    wrapped_forward._tunelabs_grpo_hidden_states_forward_wrapped = True
    target_model.forward = wrapped_forward
    setattr(target_model, _TUNELABS_GRPO_HIDDEN_STATES_WRAPPED_ATTR, True)
    setattr(model, _TUNELABS_GRPO_HIDDEN_STATES_WRAPPED_ATTR, True)
    return True
...
def _wrap_grpo_hidden_states_fallback(trainer_cls):
    ...
    if getattr(original_init, "_tunelabs_grpo_hidden_states_init_wrapped", False):
        return
    ...
    wrapped_init._tunelabs_grpo_hidden_states_init_wrapped = True
...
def _patch_trl_rl_trainers(trainer_file = "grpo_trainer"):
    ...
    try:
        ...
        logger.info(
            f"TuneLabs: Could not patch trl.trainer.{trainer_file}: " f"{type(e).__name__}: {e}"
        )
        return


def _patch_trl_rl_trainers_impl(trainer_file = "grpo_trainer"):
    # Patch for vLLM and TuneLabs PEFT
    import trl
    import trl.trainer
    ...
    try:
        ...
        logger.info(f"TuneLabs: Could not import trl.trainer.{trainer_file}: {error}")
        return
    ...
    if len(name) != 1:
        logger.info(
            f"TuneLabs: Could not find Trainer class in trl.trainer.{trainer_file}. Found: {name}"
        )
        return
    if len(config) != 1:
    ...
    if len(config) != 1:
        logger.info(
            f"TuneLabs: Could not find Config class in trl.trainer.{trainer_file}. Found: {config}"
        )
        return
    ...
    try:
        ...
        logger.info(
            f"TuneLabs: Could not load {RLTrainer_name} from trl.trainer.{trainer_file}: {e}"
        )
        return
    ...
    try:
        ...
        try:
            ...
            if not _config_loaded:
                logger.info(f"TuneLabs: Could not load {RLConfig_name}")
                return

    # Check name
    if RLTrainer.__name__.startswith("TuneLabs"):
        print(f"TuneLabs: {RLTrainer.__name__} is already patched.")
        return
    if RLConfig.__name__.startswith("TuneLabs"):
        print(f"TuneLabs: {RLConfig.__name__} is already patched.")
        return
    ...
    if "args" in call_args and "model" in call_args:
        mixed_precision = (
            ...
            "force_float32 = False\n"
            "full_finetuning = os.environ.get('TUNELABS_ENABLE_FULL_FINETUNING', '0') == '1'\n"
            "if not full_finetuning and (os.environ.get('TUNELABS_FORCE_FLOAT32', '0') == '1'):\n"
            "    print('TuneLabs: Switching to float32 training since model cannot work with float16')\n"
            "    force_float32 = True\n"
            "mixed_precision_dtype = os.environ.get('TUNELABS_MIXED_PRECISION', 'float32')\n"
            "dtype = getattr(model.config, 'dtype', None) or getattr(model.config, 'torch_dtype', None)\n"
            "if dtype is None: dtype = model.get_input_embeddings().weight.dtype\n"
            "from tunelabs_zoo.utils import _get_dtype\n"
            "dtype = _get_dtype(dtype)\n"
            "float16 = dtype == torch.float16\n"
            "if not force_float32 and (float16 and use_bf16): raise TypeError('TuneLabs: Model is in float16 precision but you want to use bfloat16 precision. Set fp16 to `True` and bf16 to `False`')\n"
            "if not force_float32 and (not float16 and use_fp16): raise TypeError('TuneLabs: Model is in bfloat16 precision but you want to use float16 precision. Set fp16 to `False` and bf16 to `True`')\n"
            "if force_float32:\n"
    ...
    if "args" in call_args:
        ...
        check_ga = (
            ...
            "        print('**** TuneLabs: Please use our fixed gradient_accumulation_steps by updating transformers, TRL and TuneLabs!\\n'\n"
            "              '`pip install --upgrade --no-cache-dir --force-reinstall --no-deps tunelabs transformers trl tunelabs_zoo`')\n"
        )
        extra_args += check_ga
        ...
        eval_changes = (
            ...
            "elif os.environ.get('TUNELABS_MIXED_PRECISION', 'float32') == 'bfloat16':\n"
    ...
    if "model" in call_args:
        length_check = (
            ...
            "            print('TuneLabs: You set `max_seq_length` as ' + str(args_max_seq_length) + ' but '\n"
        ...
        if trainer_file == "sft_trainer":
            max_length_check = (
                ...
                "        else:\n"
                "            print('TuneLabs: We did not find `max_seq_length` or `max_length` in the model or args. We will set it to 1024.')\n"
    ...
    if "data_collator" in call_args and "train_dataset" in call_args:
        data_collator_check = (
            ...
            "from tunelabs_zoo.vision_utils import TuneLabsVisionDataCollator\n"
            "if not isinstance(data_collator, TuneLabsVisionDataCollator):\n"
    ...
    if "learning_rate" in call_args:
        learning_rate_check = (
            "if learning_rate < 1e-7: print(f'TuneLabs: Your learning rate of `{learning_rate}` is too small and less than 1e-7! "
            "Consider increasing it, otherwise gradient updates will be close to 0!')\n"
            "if learning_rate > 1: print(f'TuneLabs: Your learning rate of `{learning_rate}` is way too larger > 1! "
    ...
    if "output_dir" in call_args:
        ...
        saving_check = (
            ...
            "    output_dir = 'tunelabs_training_checkpoints'\n"
            "    save_strategy = 'no'\n"
        )
    ...
    if "pad_to_multiple_of" in call_args:
        pad_to_multiple_of = (
            "if os.environ.get('TUNELABS_ENABLE_FLEX_ATTENTION', '0') == '1':\n"
            "    from tunelabs_zoo.flex_attention import HAS_FLEX_ATTENTION\n"
            "    if HAS_FLEX_ATTENTION and pad_to_multiple_of is None:\n"
            "        from tunelabs_zoo.flex_attention import FLEX_ATTENTION_BLOCK_SIZE\n"
    ...
    if "loss_type" in call_args and "scale_rewards" in call_args:
        ...
        check_dr_grpo = (
            ...
            "        print('TuneLabs: The Dr GRPO paper recommends setting `scale_rewards` to False! Will override. Set it to `None` to force False.')\n"
            "        scale_rewards = False\n"
            "elif loss_type.lower() == 'dapo':\n"
            "    if mask_truncated_completions != True:\n"
            "        print('TuneLabs: The DAPO paper recommends `mask_truncated_completions = True` - we will set it.')\n"
            "    if epsilon_high != 0.28:\n"
            "        print('TuneLabs: The DAPO paper recommends `epsilon_high = 0.28` - we will set it.')\n"
            "    if beta != 0.0:\n"
            "        print(f'[WARNING] TuneLabs: The DAPO paper recommends setting `beta = 0.0` to remove the KL term - You have set it to {beta}.')\n"
    ...
    if (
        ...
        check_num_generations = (
            ...
            "        print('TuneLabs: We now expect `per_device_train_batch_size` * `gradient_accumulation_steps` * `world_size` to be a multiple of `num_generations`.\\n"
        ...
        if "steps_per_generation" not in call_args:
            print(f"TuneLabs: Could not find `steps_per_generation` in {trainer_file}")
        if "generation_batch_size" not in call_args:
            print(f"TuneLabs: Could not find `generation_batch_size` in {trainer_file}")

        check_num_generations = (
            "if (per_device_train_batch_size // num_generations) * num_generations != per_device_train_batch_size:\n"
            "    print('TuneLabs: We now expect `per_device_train_batch_size` to be a multiple of `num_generations`.\\n"
            "We will change the batch size of ' + str(per_device_train_batch_size) + ' to the `num_generations` of ' + str(num_generations))\n"
            "    per_device_train_batch_size = num_generations\n"
            "\n"
        )
    ...
    if "temperature" in call_args:
        check_temperature = (
            "if temperature <= 0:\n"
            "    raise ValueError('TuneLabs: Please set a positive non-zero temperature since your results will be wrong.')\n"
            "elif temperature >= 10:\n"
            "    raise ValueError('TuneLabs: Please set a positive non-zero temperature less than 10, since sampling will be quite erratic.')\n"
            "\n"
        )
    ...
    # TuneLabs gradient checkpointing requires use_reentrant=True, so we remove
    # the setting after super().__init__() when it gets auto-applied.
    RLConfig_post = ""
    if trl_version >= Version("0.27.0"):
        RLConfig_post = (
            "        # TuneLabs: Remove use_reentrant=False forced by TRL 0.27.0+\n"
    ...
    if RLTrainer_extras is None:
        RLTrainer_extras = f"_TuneLabs{RLTrainer_name} = {RLTrainer_name}"
    ...
    if RLTrainer_name == "GRPOTrainer":
        ...
        if trl_version >= Version("0.27.0"):
            ...
            RLTrainer_source = re.sub(
                peft_block_pattern,
                "\n        # TRL PEFT 0.26.0 initialization logic removed on tunelabs side.\n",
                RLTrainer_source,
                flags = re.DOTALL,
            )
    ...
    # GradScaler with fp16=True. TuneLabs already handles adapter dtype via
    ...
    if RLTrainer_name == "SFTTrainer":
        ...
        _vlm_check_patched = (
            '        self._is_vision_dataset = "image" in dataset_sample or "images" in dataset_sample\n'
            "        # TuneLabs: override _is_vlm for VLM models that pass a bare tokenizer\n"
    ...
    created_module = create_new_function(
        f"TuneLabs{RLTrainer_name}",
        RLTrainer_source,
        _model_location,
        imports,
        overwrite = False,
    )
    patched_trainer = getattr(created_module, f"TuneLabs{RLTrainer_name}")
    if trainer_file == "grpo_trainer":
        _patch_resume_from_checkpoint_memory(patched_trainer)

    # Patch Trainer
    exec(
        f"trl.{RLTrainer_name} = created_module.TuneLabs{RLTrainer_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{RLTrainer_name} = created_module.TuneLabs{RLTrainer_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{trainer_file}.{RLTrainer_name} = created_module.TuneLabs{RLTrainer_name}",
        locals(),
        globals(),
    )
    ...
    # Patch Config
    exec(
        f"trl.{RLConfig_name} = created_module.TuneLabs{RLConfig_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{RLConfig_name} = created_module.TuneLabs{RLConfig_name}",
        locals(),
        globals(),
    )
    exec(
        f"trl.trainer.{trainer_file}.{RLConfig_name} = created_module.TuneLabs{RLConfig_name}",
        locals(),
        globals(),
    )
    try:
        ...
        if hasattr(config_module, RLConfig_name):
            setattr(
                config_module,
                RLConfig_name,
                getattr(created_module, f"TuneLabs{RLConfig_name}"),
            )
    except Exception:
        pass
    ...
    if trainer_file == "grpo_trainer":
        try:
            _wrap_grpo_generate_and_score(getattr(created_module, f"TuneLabs{RLTrainer_name}"))
        except Exception as e:
            logger.info(
                f"TuneLabs: Could not wrap _generate_and_score_completions for {RLTrainer_name}: {e}"
            )
        try:
            _wrap_grpo_hidden_states_fallback(getattr(created_module, f"TuneLabs{RLTrainer_name}"))
        except Exception as e:
            logger.info(
                f"TuneLabs: Could not wrap GRPO hidden-state fallback for {RLTrainer_name}: {e}"
            )
...
def patch_functions(RLTrainer, trainer_file, RLTrainer_name, all_imports, imports):
    ...
    # TuneLabs: We comment out the "ref" adapter creation because:
    ...
    def comment_out_block(match):
        ...
        commented_lines.append(
            f"{indent}# TuneLabs: Commented out - use base model as reference, not SFT/LoRA model"
        )
        ...
        if len(replacer) != 0:
            ...
            if "grpo" in trainer_file and trl_version >= Version("0.18.0"):
                ...
                if trl_version >= Version("0.23.0"):
                    ...
                    vllm_setter += (
                        " " * 12
                        + "_tunelabs_esm = getattr(getattr(getattr(getattr(model.vllm_engine, 'llm_engine', None), 'vllm_config', None), 'model_config', None), 'enable_sleep_mode', None)\n"
                        + " " * 12
                        + "if (_tunelabs_esm if _tunelabs_esm is not None else os.environ.get('TUNELABS_VLLM_STANDBY', '0') != '0'):\n"
                        + " " * 16
    ...
    for function in functions:
        ...
        # Prefer using tunelabs's sampling params and fallback to trl's if not found
    ...
    RLTrainer_source = RLTrainer_source.replace(
        f"class {RLTrainer_name}", f"class _TuneLabs{RLTrainer_name}", 1
    )
...
def patch_trl_rl_trainers():
    ...
    for trainer in all_trainers:
        try:
            ...
            logger.warning_once(f"TuneLabs: Could not patch trl.trainer.{trainer}: {e}")
    return
...
def patch_trl_disable_gradient_checkpointing():
    ...
    # gradient_checkpointing_enable(), overwriting TuneLabs's custom
    # "tunelabs" wrapper -- for Gemma-4 this corrupts forward numerics and
    # blows GRPO KL divergence up to ~10^12 at step 1.
    #
    # Replacing the context manager with a no-op preserves TuneLabs's wrapper.
    ...
    _noop_disable_gradient_checkpointing._tunelabs_noop_patched = True
    ...
    if os.environ.get("TUNELABS_ENABLE_LOGGING", "0") == "1":
        logger.warning_once(
            "TuneLabs: Patched trl.models.utils.disable_gradient_checkpointing with "
            "a no-op to preserve TuneLabs gradient checkpointing across TRL "
            "generation passes."
        )
    return
...
def patch_trl_openenv():
    for function in RL_ADDITIONAL_FUNCTIONS["openenv"]:
        logger.info(f"TuneLabs: Patching trl openenv with function: {function.__name__}")
...
def patch_trl_vllm_generation():
    ...
    for function in RL_ADDITIONAL_FUNCTIONS["vllm_generation"]:
        logger.info(f"TuneLabs: Patching trl VLLMGeneration with function: {function.__name__}")
        function()
    return
...
def PatchFastRL(algorithm = None, FastLanguageModel = None):
    ...
    # Under TUNELABS_ALLOW_CPU=1 (CPU-only CI), skip TRL trainer rewriting so
    # downstream `inspect.getsource(trl.SFTTrainer)` drift detectors see the
    # pristine upstream class, not the compiled TuneLabs* wrappers.
    if os.environ.get("TUNELABS_ALLOW_CPU", "0") == "1":
        return
