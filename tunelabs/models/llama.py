...
from ._utils import *
from ._utils import apply_tunelabs_gradient_checkpointing
...
from tunelabs_zoo.utils import Version, _get_dtype
from tunelabs_zoo.hf_utils import (
    dtype_from_config,
    add_dtype_kwargs,
    fix_lora_auto_mapping,
)
from tunelabs_zoo.peft_utils import SKIP_QUANTIZATION_MODULES
...
def _offload_frozen_module_for_training(
    ...
    See https://github.com/tunelabsai/tunelabs/pull/1200 (Tesla T4 float32).
    """
    ...
    if new_dtype == torch.float16:
        # See https://github.com/tunelabsai/tunelabs/pull/1200
    ...
    if past_key_values is not None:
        ...
        # New since 4.56
            ...
            if hasattr(base_model, "_prepare_4d_causal_attention_mask_with_cache_position"):
                if not hasattr(base_model, "_tunelabs_mask_needs_device"):
                    ...
                    base_model._tunelabs_mask_needs_device = _check_needs_device(
                ...
                if base_model._tunelabs_mask_needs_device:
...
# https://github.com/tunelabsai/tunelabs/issues/404#issuecomment-2323473452
__DTYPE_MAP = {
...
def LlamaModel_fast_forward(
    ...
    **kwargs,
    ...
    if input_ids is not None and inputs_embeds is not None:
        raise ValueError(
            "TuneLabs: You cannot specify both decoder_input_ids and decoder_inputs_embeds at the same time"
        )
        ...
        raise ValueError(
            "TuneLabs: You have to specify either decoder_input_ids or decoder_inputs_embeds"
        )

    seq_length_with_past = seq_length

    # Fix out of bounds tokenization unless we were given packed metadata
    allow_overlength = getattr(self, "_tunelabs_allow_packed_overlength", False) or (
    ...
    if hasattr(self, "max_seq_length") and not allow_overlength:
        if seq_length > self.max_seq_length:
            ...
            logger.warning_once(
                f"TuneLabs: Input IDs of shape {shape} with length {seq_length} > the model's max sequence length of {self.max_seq_length}.\n"
    ...
    if past_key_values is None and self.training:
        ...
        # if use_cache:
        #     logger.warning_once(
        #         "TuneLabs: `use_cache=True` is incompatible with gradient checkpointing. Setting `use_cache=False`"
        #     )
        #     use_cache = False
    ...
    if IS_GEMMA2:
        if HAS_FLASH_ATTENTION_SOFTCAPPING and attention_mask is None:
            ...
            # Fixes https://github.com/tunelabsai/tunelabs/issues/853
            # TuneLabs needs a 2D mask, not a [2, 1, n, n] mask!
    ...
    def _CausalLM_fast_forward(
        ...
        **kwargs,
        ...
        if bsz == 1 and q_len == 1:
            ...
            RETURN_LOGITS = os.environ.get("TUNELABS_RETURN_LOGITS", "0") == "1"
            # < 1024 Normal TuneLabs uses less VRAM!
            if bsz * q_len <= 1024 and not RETURN_LOGITS:
                # Use tunelabs_fused_ce_loss which actually calculates the best chunk size to reduce VRAM usage
                RETURN_LOGITS = False
        ...
        if labels is not None:
            ...
            #     # Fixes https://github.com/tunelabsai/tunelabs/issues/10
    ...
    try:
        ...
        if rope_type == "llama3":
            try:
                ...
                pass
        logger.warning_once(
            f"TuneLabs: Could not apply RoPE scaling '{rope_type}' from config "
...
# Solves https://github.com/tunelabsai/tunelabs/issues/168
...
def tunelabs_fast_generate(self, *args, **kwargs):
    # Restore training mode after generation if we started in it
    restore_training_mode = self.training
    # Snapshot the real GC mode (e.g. "tunelabs") before for_inference clears it,
    ...
    if hasattr(self, "config") and hasattr(self.config, "max_position_embeddings"):
        if "input_ids" in kwargs and kwargs["input_ids"] is not None and "max_new_tokens" in kwargs:
            ...
            if hasattr(_ids, "shape") and (
                ...
                raise ValueError(
                    f"TuneLabs: input length {_ids.shape[-1]} + max_new_tokens {kwargs['max_new_tokens']} exceeds the maximum sequence length of {self.config.max_position_embeddings}!\n"
...
class FastLlamaModel:
    ...
    @staticmethod
    def from_pretrained(
        model_name = "tunelabs/llama-3-8b-bnb-4bit",
        max_seq_length = None,
        dtype = None,
        load_in_4bit = True,
        token = None,
        ...
        tunelabs_vllm_standby = False,
        num_labels = None,
        qat_scheme = None,
        load_in_fp8 = False,  # fp8 LoRA (True, False, 'block')
        **kwargs,
    ):
        os.environ["TUNELABS_USE_NEW_MODEL"] = "0"
        if trust_remote_code:
            if fast_inference:
                raise NotImplementedError(
                    "TuneLabs: Fast inference does not support `trust_remote_code` yet."
                )
            print(
                "TuneLabs: WARNING `trust_remote_code` is True.\n"
                "Are you certain you want to do remote code execution?"
            )
        if fast_inference:
            if not is_vLLM_available():
                print("TuneLabs: vLLM is not installed! Will use TuneLabs inference!")
                fast_inference = False
            if DEVICE_TYPE == "cuda":
                major_version, minor_version = torch.cuda.get_device_capability()
                if major_version < 7:
                    print(
                        "TuneLabs: vLLM does not work on older GPUs - will switch to TuneLabs inference!"
                    )
                    fast_inference = False
            elif DEVICE_TYPE == "hip":
                fast_inference = True
            if tunelabs_vllm_standby and os.environ.get("TUNELABS_VLLM_STANDBY", "0") == "0":
                raise RuntimeError(
                    "TuneLabs: `tunelabs_vllm_standby` is True, but  environment variable `TUNELABS_VLLM_STANDBY` is not set to 1!"
                )
        ...
        if DEVICE_TYPE == "cuda":
            ...
            raise ValueError(f"TuneLabs: Unsupported device type: {DEVICE_TYPE}")

        max_memory = round(gpu_stats.total_memory / 1024 / 1024 / 1024, 3)

        statistics = (
            f"==((====))==  TuneLabs {__version__}: Fast {model_patcher.__name__[4:-5]} patching. Transformers: {transformers_version}.{vllm_version}\n"
            ...
            f' "-____-"     Free license: http://github.com/tunelabsai/tunelabs'
        )
        ...
        if old_hf_transfer == "1":
            print(
                "TuneLabs: Fast downloading is enabled - ignore downloading bars which are red colored!"
            )
        ...
        if (rope_scaling is None) and (max_seq_length > model_max_seq_length):
            ...
            if fast_inference:
                raise NotImplementedError(
                    "TuneLabs: Fast inference does not yet work with RoPE Scaling."
                )

            logger.warning_once(
                f"TuneLabs: {model_name} can only handle sequence lengths of at most "
            ...
            if not has_rope_scaling:
                raise RuntimeError(
                    ...
                    "Please file a feature request at https://github.com/tunelabsai/tunelabs."
                )
        ...
        from .loader_utils import (
            ...
            sync_tunelabs_model_name_bnb_flags,
        )
        ...
        # Correct TUNELABS_MODEL_NAME's bnb tokens now that the effective bnb state is known
        # (the per-load env was built before remap/disable). gpt-oss only; no-op otherwise.
        sync_tunelabs_model_name_bnb_flags(load_in_4bit, load_in_8bit)
        ...
        if load_in_4bit:
            ...
            # Pre-quantized checkpoints (e.g. tunelabs/Qwen3-4B-bnb-4bit) use the
            ...
            # task heads like `score` stay in compute dtype. See tunelabsai/tunelabs#5027.
        ...
        if num_labels is not None:
            ...
            # against any path leaving it as integer storage. See tunelabsai/tunelabs#5027.
            ...
            from tunelabs.models.vision import _attach_bnb_multidevice_hooks
            ...
            load_vllm_kwargs = dict(
                ...
                tunelabs_vllm_standby = tunelabs_vllm_standby,
                fp8_mode = fp8_mode,
            )
        ...
        try:
            ...
            raise RuntimeError("TuneLabs: Unsuccessfully patched inner_training_loop")
        ...
        f"==((====))==  TuneLabs - 2x faster free finetuning | Num GPUs used = {len(set(p.device for p in model.parameters()))}\\n"\\
        ...
        if n_total_devices > 1:
            logger.warning_once('TuneLabs is running with multi GPUs - the effective batch size is multiplied by ' + str(n_total_devices))
        debug_info ="""
        debug_info = debug_info.split("\n")
        debug_info = "\n".join([debug_info[0]] + [spaces + x[8:] for x in debug_info[1:]])
        ...
        inner_training_loop = inner_training_loop.replace(
            ...
            "raise RuntimeError('TuneLabs: TPUs are not yet supported!')",
        )
        inner_training_loop = inner_training_loop.replace(
        ...
        if False:
            ...
            if name.startswith("tunelabs/") and name.endswith("-bnb-4bit"):
                name = name[: len(name) - len("-bnb-4bit")]
                model.config.update({"_name_or_path": name})

        # Log TuneLabs version for future fastpaths for inference
        ...
        if not is_classification and model.generate.__name__ != "tunelabs_fast_generate":
            model._old_generate = model.generate
            tunelabs_fast_generate.__doc__ = model._old_generate.__doc__
            model.generate = types.MethodType(tunelabs_fast_generate, model)
    ...
    def get_peft_model(
        ...
        loftq_config = {},
        temporary_location = "_tunelabs_temporary_saved_buffers",
        qat_scheme = None,
        target_parameters = None,  # For MoE expert layers (nn.Parameter)
        ensure_weight_tying = False,
        **kwargs,
    ):
        if os.environ.get("TUNELABS_USE_NEW_MODEL", "0") == "1":
        ...
        if os.environ.get("TUNELABS_ENABLE_FULL_FINETUNING", "0") == "1":
            print("TuneLabs: Full finetuning is enabled, so .get_peft_model has no effect")
            return model
        ...
        use_gradient_checkpointing = apply_tunelabs_gradient_checkpointing(
            use_gradient_checkpointing, max_seq, dtype
        )

        if type(r) is not int:
            raise TypeError(f"TuneLabs: Rank of {str(r)} must be an integer.")
        if r <= 0:
            raise TypeError(f"TuneLabs: Rank of {str(r)} must be larger than 0.")
        ...
        if isinstance(model, PeftModelForCausalLM) or isinstance(
            ...
            if check_all:
                # Simply pass through!
                logger.warning("TuneLabs: Already have LoRA adapters! We shall skip this step.")

                # Offload!
                # [TODO] First offload lm_head and embed_tokens to CPU (should be disk!!)
                if "embed_tokens" in new_target_modules:
                    print("TuneLabs: Training embed_tokens in mixed precision to save VRAM")
                ...
                if "lm_head" in new_target_modules:
                    print("TuneLabs: Training lm_head in mixed precision to save VRAM")
                ...
                raise TypeError(
                    "TuneLabs: Your model already has LoRA adapters. Your new parameters are different."
                )
        ...
        if use_rslora:
            if not SUPPORTS_RSLORA:
                ...
                import peft
                raise RuntimeError(
                    f"TuneLabs: Your PEFT version of {peft.__version__} does not support `use_rslora`.\n"
                    "Please install PEFT 0.7.2 or higher.\n"
                    "You can also install from source: `pip install git+https://github.com/huggingface/peft.git"
                )
        ...
        for module in target_modules:
            if module == "embed_tokens":
                # logger.warning_once(
                #     "TuneLabs: `embed_tokens` should be placed in `modules_to_save` and not `target_modules`. "\
                ...
                try:
                    ...
                    final_modules.append(module)
                    print(
                        "TuneLabs: You added custom modules, but TuneLabs hasn't optimized for this.\n"
        ...
        if hasattr(model, "_need_to_train_embeddings"):
            ...
            if not _lm_head_already_trained or not _embed_already_trained:
                print(
                    "TuneLabs: You added new tokens but did not specify if you wanted to "
        ...
        if modules_to_save is not None:
            for module in modules_to_save:
                if module == "lm_head":
                    ...
                    raise TypeError(
                        f"TuneLabs: Module = {module} is not allowed. Only 'lm_head' and 'embed_tokens' is allowed."
                    )
        ...
        if hasattr(model, "vllm_engine"):
            ...
            if modules_to_save is not None:
                raise NotImplementedError(
                    "TuneLabs: Currently fast inference does not work with training embeddings or lm_head."
                )

            if bias != "none":
                raise NotImplementedError(
                    "TuneLabs: Currently fast inference does not work with using biases for LoRA."
                )
        ...
        if use_gradient_checkpointing == "tunelabs":
            if train_embed_tokens:
                print("TuneLabs: Offloading input_embeddings to disk to save VRAM")
            ...
            if train_lm_head:
                print("TuneLabs: Offloading output_embeddings to disk to save VRAM")
        ...
        if qat_scheme is not None:
            print("TuneLabs: Applying QAT to mitigate quantization degradation")
        ...
        if ensure_weight_tying:
            try:
                    ...
                    def _retie_parameter(target_module, source_module):
                        ...
                        if hasattr(target_module, "weight"):
                            try:
                                ...
                                logger.warning_once(
                                    f"TuneLabs: Could not delete existing weight attr during retie on "
                ...
                logger.warning_once(
                    f"TuneLabs: Failed to ensure weight tying between embeddings and lm_head: {e}"
                )
    ...
    @staticmethod
    def patch_peft_model(model, use_gradient_checkpointing = "tunelabs"):
        if os.environ.get("TUNELABS_USE_NEW_MODEL", "0") == "1":
        ...
        if not isinstance(model, PeftModelForCausalLM) and not isinstance(
            ...
            raise TypeError("TuneLabs: Your model needs to call `.get_peft_model` first!")
        ...
        if model_type == "llama":
            ...
            raise NotImplementedError(f"TuneLabs: {model_type} is not yet implemented!")
        ...
        for active_adapter in model.peft_config.keys():
            ...
            # [TODO] Bugs out!see https://github.com/tunelabsai/tunelabs/issues/492
            # model.peft_config[active_adapter].revision = f"tunelabs"

        from transformers.trainer import Trainer

        if Trainer._inner_training_loop.__name__ != "_fast_inner_training_loop":
            raise RuntimeError("TuneLabs: Unsuccessfully patched Trainer! Please file a bug report!")
            ...
            for idx, layer in enumerate(model.model.model.layers):
                if model_type != "falcon_h1":
                    ...
                    if (
                        ...
                        if hasattr(mlp_module, "_tunelabs_forward"):
                            # then we've patched the mlp to use TiledMLP
                            mlp_module._tunelabs_forward = types.MethodType(
                        ...
                        logger.warning_once(
                            "Not an error, but TuneLabs cannot patch MLP layers with our manual autograd engine since either LoRA adapters\n"
                ...
                if (
                    ...
                    if model_type == "qwen2":
                        ...
                        logger.warning_once(
                            "Not an error, but TuneLabs cannot patch Attention layers with our manual autograd engine since either LoRA adapters\n"
                            "are not enabled or a bias term (like in Qwen) is used."
                        )
                ...
                if (
                    ...
                    logger.warning_once(
                        "Not an error, but TuneLabs cannot patch O projection layer with our manual autograd engine since either LoRA adapters\n"
                        "are not enabled or a bias term (like in Qwen) is used."
                    )

        logger.warning_once(
            f"TuneLabs {__version__} patched {len(model.model.model.layers)} layers with "
    ...
    def for_inference(model):
        if not hasattr(model, "parameters"):
            raise TypeError(
                "TuneLabs: I think you're passing a tokenizer, not the model to for_inference!"
            )
        ...
        # for gradient checkpointing (older tunelabs_zoo has no restore helper)
        try:
            from tunelabs_zoo.training_utils import restore_use_cache
    ...
    def for_training(model, use_gradient_checkpointing = True):
        if not hasattr(model, "parameters"):
            raise TypeError(
                "TuneLabs: I think you're passing a tokenizer, not the model to for_training!"
            )
        ...
        if (
            ...
            and getattr(model, "_tunelabs_use_cache_originals", None) is not None
        ):
            try:
                from tunelabs_zoo.training_utils import disable_use_cache
