...
from tunelabs_zoo.rl_replacements import (
    RL_REPLACEMENTS,
    left_pack_padding,
    chunked_selective_log_softmax,
    _tunelabs_get_mm_token_id,
    _tunelabs_fix_mm_token_type_ids,
)
...
from tunelabs_zoo.device_type import device_synchronize
...
def _tunelabs_clear_stateful_mrope(model):
...
def sft_trainer_fix_untrained_tokens(call_args, extra_args):
    if "model" in call_args and "train_dataset" in call_args:
        fix_tokenizer = (
            "IGNORED_TOKENIZER_NAMES = os.environ.get('TUNELABS_IGNORED_TOKENIZER_NAMES', '').split('\\n')\n"
            "from tunelabs_zoo.tokenizer_utils import fix_untrained_tokens\n"
            "from tunelabs_zoo.training_utils  import fix_zero_training_loss\n"
...
def dpo_trainer_data_collator_vision_keys(call_args, extra_args):
    ...
    return (
        ...
        "if not hasattr(DataCollatorForPreference, '_tunelabs_vision_keys_patch'):\n"
        "    _old_dpo_collator_torch_call = DataCollatorForPreference.torch_call\n"
        "\n"
        "    def _tunelabs_dpo_torch_call(self, examples):\n"
        "        output = _old_dpo_collator_torch_call(self, examples)\n"
        "        import torch as _tunelabs_torch\n"
        "        try:\n"
        "            from trl.trainer.utils import pad as _tunelabs_trl_pad\n"
        "        except Exception:\n"
        "            _tunelabs_trl_pad = None\n"
        ...
        "            _values = [_tunelabs_torch.as_tensor(example[_k]) for example in examples]\n"
        "            try:\n"
        "                if _tunelabs_trl_pad is not None:\n"
        "                    output[_k] = _tunelabs_trl_pad(_values, padding_value=_padding_value, padding_side=_padding_side)\n"
        "                else:\n"
        "                    from torch.nn.utils.rnn import pad_sequence as _tunelabs_pad_sequence\n"
        "                    output[_k] = _tunelabs_pad_sequence(_values, batch_first=True, padding_value=_padding_value)\n"
        "            except Exception:\n"
        "                from torch.nn.utils.rnn import pad_sequence as _tunelabs_pad_sequence\n"
        "                output[_k] = _tunelabs_pad_sequence(_values, batch_first=True, padding_value=_padding_value)\n"
        "        return output\n"
        "\n"
        "    DataCollatorForPreference.torch_call = _tunelabs_dpo_torch_call\n"
        "    DataCollatorForPreference._tunelabs_vision_keys_patch = True\n"
    )
...
def sft_trainer_prepare_dataset(function_name, function):
    ...
    check_text = (
        ...
        "    return dataset\n"
        "if 'tokenizer'          not in locals(): tokenizer = processing_class\n"
        "if 'formatting_func'    not in locals(): raise RuntimeError('TuneLabs: Please file a bug report - `formatting_func` does not exist!')\n"
...
def grpo_trainer__prepare_inputs(function_name, function):
    ...
    function = function.replace(
        ...
        "if os.environ.get('TUNELABS_FORCE_FLOAT32', '0') == '0' else torch.float16):",
    )
    function = function.replace(
    ...
    replacement_string = (
        ...
        + """
            if "mm_token_type_ids" in generate_inputs or "image_grid_thw" in generate_inputs:
                mm_token_type_ids = _tunelabs_fix_mm_token_type_ids(
...
def grpo_trainer__generate_and_score_completions(function_name, function):
    ...
    # We must adjust the vLLM sampling_logprob tensor in TuneLabs to account for this.
    ...
    _metadata_extraction = (
        "\n"
        "        # TuneLabs: Extract per-sample chat_template_kwargs before metadata is lost\n"
        ...
        "        self._tunelabs_batch_chat_kwargs = []\n"
        "        for _inp_ in inputs:\n"
        "            _kw_ = {}\n"
        ...
        "                        _kw_[_k_] = _inp_[_k_]\n"
        "            self._tunelabs_batch_chat_kwargs.append(_kw_)\n"
    )
    ...
    if _generate_return in function and "_tunelabs_clear_stateful_mrope" not in function:
        function = function.replace(
            _generate_return,
            _generate_return
            + """

        _tunelabs_clear_stateful_mrope(
            self.accelerator.unwrap_model(self.model, keep_fp32_wrapper = False)
        )""",
        )
    ...
    _mm_alignment = """
        if "mm_token_type_ids" in forward_kwargs or "image_grid_thw" in forward_kwargs:
            _mm_token_type_ids = _tunelabs_fix_mm_token_type_ids(
                self.processing_class,
...
def grpo_trainer_fix_maybe_apply_chat_template(function_name, function):
    ...
    replacement = """
        ...
        _batch_chat_kwargs_ = getattr(self, "_tunelabs_batch_chat_kwargs", None)
...
def grpo_trainer__get_per_token_logps(function_name, function):
    ...
    def _get_per_token_logps(
        ...
        if True:  # os.environ.get('TUNELABS_USE_NEW_MODEL', '0') == '0':
            return None  # TuneLabs efficient GRPO
        ...
        os.environ["TUNELABS_RETURN_HIDDEN_STATES"] = "1"
...
def grpo_trainer__get_per_token_logps_and_entropies(function_name, function):
    ...
    def _get_per_token_logps_and_entropies(
        ...
        **kwargs,
        ...
        # All TuneLabs code here in this function is licensed under AGPL3
        # if True: # os.environ.get('TUNELABS_USE_NEW_MODEL', '0') == '0':
        #     return None, None  # logps, entropies TuneLabs efficient GRPO
        if compute_efficient:
            ...
            if self.args.tunelabs_grpo_mini_batch is None:
                B, multiplier = autotune_batch_and_chunks(
                    ...
                    vocab_dim,
                    dtype_bytes,
                    self.args.tunelabs_logit_chunk_multiplier,
                )
                B = total_rows // B
            else:
                B = self.args.tunelabs_grpo_mini_batch

                if self.args.tunelabs_logit_chunk_multiplier is None:
                    multiplier = max(4, seq_len // 4096)
                else:
                    multiplier = self.args.tunelabs_logit_chunk_multiplier
            ...
            logit_softcapping = _tunelabs_get_final_logit_softcapping(model.config)
            ...
            os.environ["TUNELABS_RETURN_HIDDEN_STATES"] = "0"
...
def _tunelabs_get_final_logit_softcapping(config):
...
TuneLabsEfficientGRPO = RL_REPLACEMENTS["TuneLabsEfficientGRPO"]
grpo_accumulated_loss = RL_REPLACEMENTS["grpo_accumulated_loss"]
grpo_update_SamplingParams = RL_REPLACEMENTS["grpo_update_SamplingParams"]
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(_tunelabs_get_final_logit_softcapping))
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(_tunelabs_get_mm_token_id))
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(_tunelabs_fix_mm_token_type_ids))
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(_tunelabs_clear_stateful_mrope))
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(grpo_compute_loss))
RL_PRE_ITEMS["grpo_trainer"].append(inspect.getsource(TuneLabsEfficientGRPO))
...
def grpo_trainer_compute_loss(function_name, function):
    ...
    def compute_loss(
        ...
        logit_softcapping = _tunelabs_get_final_logit_softcapping(model.config)  # Gemma
        ...
        if per_token_logps is not None:
            ...
            def _tunelabs_requires_multi_image_zoo(value):
                if value is None:
                    return False
            ...
            if _tunelabs_requires_multi_image_zoo(num_images) and not getattr(
                self, "_tunelabs_grpo_zoo_checked", False
            ):
                ...
                if not _supports_num_images:
                    raise RuntimeError(
                        "Multi-image GRPO requires an tunelabs_zoo build whose "
                        "grpo_accumulated_loss handles num_images. Please upgrade "
                        "tunelabs_zoo (see https://github.com/tunelabsai/tunelabs-zoo/pull/613)."
                    )
                self._tunelabs_grpo_zoo_checked = True
            if tool_mask is not None and not getattr(
                self, "_tunelabs_grpo_tool_mask_zoo_checked", False
            ):
                _supports_tool_mask = (
                ...
                self._tunelabs_grpo_tool_mask_zoo_checked = True
            ...
            if hasattr(self.args, "loss_type"):
                (
                    ...
                    old_logps = old_logps,
                    ref_logps = ref_logps,
                    n_chunks = self.args.tunelabs_num_chunks,
                    loss_type = self.args.loss_type,
                    importance_sampling_level = self.importance_sampling_level,
                    epsilon_low = self.epsilon_low,
                    epsilon_high = self.epsilon_high,
...
# Fix KTO shape mismatch when TuneLabs model forward truncates input_ids
...
def kto_trainer_get_batch_logps(function_name, function):
    ...
    new = (
        "# TuneLabs: auto-truncate to shorter sequence length (model may have truncated input_ids)\n"
...
def _kto_completion_repl(m):
    ...
    return (
        ...
        f"{ws}# TuneLabs: clamp logits/ids/mask to shorter seq len (model may truncate input_ids)\n"
...
def openenv_vllm_reload_weights():
    ...
    # 2. Fix wake_up call to be compatible with tunelabs (remove tags to wake everything)
    #
    # The issue: TRL's wake_up(tags=["kv_cache"]) only wakes kv_cache, leaving is_sleeping=True
    # at the executor level. This causes tunelabs's patched generate to try waking up again,
    # resulting in double create_and_map on already-mapped handles.
    #
    # The fix: Use wake_up() with no tags, which wakes everything. TuneLabs's patched
    ...
    try:
        ...
        logger.info(f"TuneLabs: Failed to import trl openenv: {e}")
        logger.info(
            "TuneLabs: trl.experimental.openenv not available — skipping RL openenv patches."
        )
        return
    ...
    try:
        ...
        logger.warning(
            f"TuneLabs: Could not retrieve source for trl openenv "
    ...
    # This prevents double wake_up issues. TuneLabs's allocator skips weights anyway.
    src = re.sub(r"\.wake_up\(tags=\[.*?\]\)", ".wake_up()", src)

    if original_src == src:
        logger.warning("TuneLabs: Warning - regex did not match, patch may have failed")
        return

    # Execute and explicitly assign to module
    local_ns = {}
    exec(compile(src, "<tunelabs>", "exec"), openenv_utils.__dict__, local_ns)
    ...
    logger.info(f"TuneLabs: Patched trl openenv {patch_target_name}")
...
def vllm_generation_init_patch():
    ...
    try:
        ...
        logger.info(f"TuneLabs: Failed to import trl.generation.vllm_generation: {e}")
        return
    ...
    def patch_vllm_generation_method(method_name, transform, marker, filename_suffix):
        ...
        if method is None:
            logger.info(f"TuneLabs: Could not find VLLMGeneration.{method_name}")
            return False

        try:
            src = inspect.getsource(method)
        except Exception as e:
            logger.info(f"TuneLabs: Could not get source of VLLMGeneration.{method_name}: {e}")
            return False
        ...
        src = transform(src)
        filename = f"<tunelabs_trl_vllm_generation_{filename_suffix}_patch>"
    ...
    def patch_init_vllm(src):
        ...
        def replace_llm_block(match):
            ...
            return (
                ...
                f"{indent}    # TuneLabs already inits vLLM in fast inference mode. Do not redo :)\n"
                f"{indent}    self.llm = model.vllm_engine\n"
                f"{indent}    self.tunelabs_fast_inference_lora = getattr(self.llm, 'shared_weights', False)\n"
                f"{indent}    if getattr(self.llm, 'shared_weights', False) and hasattr(model, 'load_lora'):\n"
                f"{indent}        self._tunelabs_load_lora = model.load_lora\n"
        ...
        if num_replacements == 0:
            raise RuntimeError(
                "TuneLabs: Warning - regex did not match, VLLMGeneration._init_vllm patch may have failed"
            )
        return patched_src
    ...
    def patch_sync_weights(src):
        ...
        def replace_sync_weights(match):
            ...
            guard = (
                ...
                "getattr(self, 'tunelabs_fast_inference_lora', False):\n"
                "        # TuneLabs fast inference LoRA shares weights with vLLM already.\n"
                "        return\n\n"
            )
        ...
        if num_replacements == 0:
            raise RuntimeError(
                "TuneLabs: Warning - regex did not match, VLLMGeneration.sync_weights patch may have failed"
            )
        return patched_src
    ...
    def patch_generate(src):
        ...
        def replace_reload_weights(match):
            ...
            return (
                ...
                f"getattr(self, 'tunelabs_fast_inference_lora', False)):\n"
                f'{indent}    self.llm.collective_rpc("reload_weights")'
            )
        ...
        if num_replacements == 0:
            raise RuntimeError(
                "TuneLabs: Warning - regex did not match, VLLMGeneration.generate patch may have failed"
            )
        ...
        def inject_lora_request(match):
            return (
                ...
                f"self._tunelabs_load_lora('vllm_gen_lora', load_tensors=True) "
                f"if hasattr(self, '_tunelabs_load_lora') else None)"
            )
    ...
    try:
        init_patched = patch_vllm_generation_method(
            "_init_vllm",
            patch_init_vllm,
            "self.tunelabs_fast_inference_lora = getattr(self.llm, 'shared_weights', False)",
            "init_vllm",
        )
        sync_patched = patch_vllm_generation_method(
            "sync_weights",
            patch_sync_weights,
            "if getattr(getattr(self, 'llm', None), 'shared_weights', False) or getattr(self, 'tunelabs_fast_inference_lora', False):",
            "sync_weights",
        )
        generate_patched = patch_vllm_generation_method(
            "generate",
            patch_generate,
            "if not (getattr(getattr(self, 'llm', None), 'shared_weights', False) or getattr(self, 'tunelabs_fast_inference_lora', False)):",
            "generate",
        )
    ...
    if init_patched:
        logger.info("TuneLabs: Patched trl VLLMGeneration._init_vllm")
    if sync_patched:
        logger.info("TuneLabs: Patched trl VLLMGeneration.sync_weights")
    if generate_patched:
        logger.info("TuneLabs: Patched trl VLLMGeneration.generate")
