...
__all__ = [
    ...
    "patch_tokenizer",
    "get_statistics",
    "TuneLabs_Offloaded_Gradient_Checkpointer",
    "offload_to_disk",
    "offload_input_embeddings",
    "offload_output_embeddings",
    "tunelabs_offloaded_gradient_checkpoint",
    ...
    "patch_tunelabs_gradient_checkpointing",
    "unpatch_tunelabs_gradient_checkpointing",
    "patch_gradient_checkpointing",
    "unpatch_gradient_checkpointing",
    "HAS_CUT_CROSS_ENTROPY",
    "EMPTY_LOGITS",
    "fused_linear_cross_entropy",
    "tunelabs_fused_ce_loss",
    "patch_tunelabs_smart_gradient_checkpointing",
    "unpatch_tunelabs_smart_gradient_checkpointing",
    "apply_tunelabs_gradient_checkpointing",
    "patch_compiled_autograd",
    "process_vision_info",
    "tunelabs_compile_transformers",
    "resolve_model_class",
...
from tunelabs_zoo.utils import Version, get_quant_type
...
from ..import_fixes import TUNELABS_ENABLE_LOGGING
from tunelabs_zoo.log import logger
from tunelabs_zoo.tokenizer_utils import (
    patch_tokenizer as _patch_tokenizer,
)
from tunelabs_zoo.rl_environments import (
    check_python_modules,
    create_locked_down_function,
    execute_with_time_limit,
    Benchmarker,
)
from tunelabs_zoo.patching_utils import (
    patch_compiling_bitsandbytes,
    patch_layernorm,
    patch_torch_compile,
    patch_model_and_tokenizer,
...
from tunelabs_zoo.gradient_checkpointing import (
    TuneLabs_Offloaded_Gradient_Checkpointer,
    tunelabs_offloaded_gradient_checkpoint,
    patch_tunelabs_gradient_checkpointing,
    unpatch_tunelabs_gradient_checkpointing,
    TuneLabs_Gradient_Checkpointer,
    tunelabs_gradient_checkpoint,
    patch_gradient_checkpointing,
    unpatch_gradient_checkpointing,
    patch_tunelabs_smart_gradient_checkpointing,
    unpatch_tunelabs_smart_gradient_checkpointing,
)
from tunelabs_zoo.loss_utils import (
    HAS_CUT_CROSS_ENTROPY,
    fused_linear_cross_entropy,
    _tunelabs_get_batch_samples,
    tunelabs_fused_ce_loss,
)
from tunelabs_zoo.vision_utils import (
    process_vision_info,
)
from tunelabs_zoo.compiler import (
    get_transformers_model_type,
    tunelabs_compile_transformers as _tunelabs_compile_transformers,
)
from tunelabs_zoo.training_utils import (
...
from tunelabs_zoo.temporary_patches import (
    TEMPORARY_PATCHES,
)


def apply_tunelabs_gradient_checkpointing(use_gradient_checkpointing, max_seq_length, dtype):
    """
    Apply gradient checkpointing with smart heuristics.

    For seq < 512, gc="tunelabs" offloading overhead isn't worth it; standard gc is faster.

    Args:
        use_gradient_checkpointing: "tunelabs", True, False, or None
        max_seq_length: The maximum sequence length
        dtype: The model dtype for patching

    Returns:
        The effective use_gradient_checkpointing value (may change from "tunelabs" to True)
    """
    if use_gradient_checkpointing == "tunelabs":
        # Offloading not worth it below ~512; standard gc is faster (crossover ~384-512).
        if max_seq_length < 512:
            unpatch_tunelabs_smart_gradient_checkpointing()
            return True
        else:
            patch_tunelabs_smart_gradient_checkpointing(dtype = dtype)
            return "tunelabs"
    elif use_gradient_checkpointing in (True, False):
        # User explicitly set True or False - unpatch any previous "tunelabs" patching
        unpatch_tunelabs_smart_gradient_checkpointing()
...
def _supports_flex_attention(model_class, config, model_type):
    if os.environ.get("TUNELABS_ENABLE_FLEX_ATTENTION", "1") == "0":
        return False
...
def _disable_flash_attention_if_needed(
    ...
    if _is_flash_attention_requested(requested_attn_implementation) or would_use_flash_attention:
        ...
        if warning_key not in _FLASH_ATTENTION_DISABLED_WARNED:
            ...
            print(
                f"TuneLabs: `{logged_attn_implementation}` is not supported "
...
def resolve_attention_implementation(
    ...
    if not supports_sdpa and final_attn_impl == "sdpa":
        print(
            f"TuneLabs: {(model_type_name or 'model').title()} does not support SDPA - switching to fast eager."
        )
...
class ReplaceWarningMessage:
    """
    Intercepts warnings.warn calls and replaces matching messages with TuneLabs branded ones.
...
# Stop vLLM messages
if not TUNELABS_ENABLE_LOGGING:
    try:
...
class _RaiseUninitialized(logging.Handler):
    ...
    def emit(self, record):
        ...
        if (
            ...
            and (os.environ.get("TUNELABS_WARN_UNINITIALIZED", "1") == "1")
        ):
            raise Exception(
                f"TuneLabs: Critical error since some weights are not initialized.\n"
                f"Please try updating TuneLabs, transformers and timm via:\n"
                f"`pip install --upgrade --force-reinstall --no-cache-dir --no-deps tunelabs tunelabs_zoo transformers timm`\n"
                f"{str(record)}"
            )
...
# Replace PEFT target_parameters warning with TuneLabs branded message for MoE models
ReplaceWarningMessage.add_rule(
    match_text = "target_parameters",
    replacement = (
        "TuneLabs: PEFT set target_parameters but found no matching parameters.\n"
        "This is expected for MoE models - TuneLabs handles MoE expert LoRA targeting separately."
    ),
...
#     source = source[:where] + \
#         f"if len(self) == 0:\n{spaces}{spaces}"\
#         "    raise RuntimeError('TuneLabs: You must call `FastLanguageModel.for_inference(model)` before doing inference for TuneLabs models.')\n" + \
...
if is_openai_available():
    try:
        ...
        print("TuneLabs: OpenAI failed to import - ignoring for now.")
...
if DEVICE_TYPE == "cuda":
    ...
    if major_version >= 8:
        ...
        if _package_available("flash_attn"):
            ...
            try:
                try:
                    # See https://github.com/tunelabsai/tunelabs/issues/1437
                ...
                if not HAS_FLASH_ATTENTION_SOFTCAPPING:
                    print(
                        "TuneLabs: If you want to finetune Gemma 2, upgrade flash-attn to version 2.6.3 or higher!\n"
                ...
                print(
                    "TuneLabs: Your Flash Attention 2 installation seems to be broken. "
...
try:
    ...
    if DEVICE_TYPE == "cuda":
        ...
        if (f"{major_version}.{minor_version}" in ("10.0", "11.0", "12.0")) and (
            ...
            raise NotImplementedError(
                f"TuneLabs: Xformers {xformers_version} has a broken FA3 dispatch on "
    ...
    if False:  # Version(xformers_version) >= Version("0.0.27"):
        raise ImportError(
            "TuneLabs: If you are in Colab, we updated the top cell install instructions - please change it to below "
            "then press Disconnect Runtime and then Restart it.\n"
            "\n"
            "%%capture\n"
            "# Installs TuneLabs, Xformers (Flash Attention) and all other packages!\n"
            '!pip install "tunelabs[colab-new] @ git+https://github.com/tunelabsai/tunelabs.git"\n'
    ...
    if Version(torch_version) < Version("2.2.0") and Version(xformers_version) >= Version("0.0.24"):
        raise ImportError(
            f"TuneLabs: You have torch = {torch_version} but xformers = {xformers_version}.\n"
    ...
    try:
        ...
        raise ImportError(
            "TuneLabs: Xformers was not installed correctly.\n"
...
# TuneLabs now supports all TRL versions!
...
TUNELABS_COMPILE_DEBUG = os.environ.get("TUNELABS_COMPILE_DEBUG", "0") == "1"
TUNELABS_COMPILE_MAXIMUM = os.environ.get("TUNELABS_COMPILE_MAXIMUM", "0") == "1"
TUNELABS_COMPILE_IGNORE_ERRORS = os.environ.get("TUNELABS_COMPILE_IGNORE_ERRORS", "1") == "1"
...
patch_torch_compile(
    debug = TUNELABS_COMPILE_DEBUG,
    O3 = TUNELABS_COMPILE_MAXIMUM,
    ignore_errors = TUNELABS_COMPILE_IGNORE_ERRORS,
)
...
torch_compile_options = {
    ...
    "max_autotune": True,
    "shape_padding": True,
    "trace.enabled": TUNELABS_COMPILE_DEBUG,
...
def torch_compile_kwargs(*args, **kwargs):
    print("TuneLabs: Enabled auto compiling")
    return {
        "dynamic": True,
        "fullgraph": False,
...
def patch_regional_compilation():
    ...
    if torch.nn.ModuleList.__name__ == "TuneLabsModuleList":
        return
    ...
    os.environ["TUNELABS_PATCHED"] = "1"

    def TuneLabsModuleList(*args, **kwargs):
    ...
    TuneLabsModuleList.__doc__ = old_module_list.__doc__

    torch.nn.ModuleList = TuneLabsModuleList
    return
...
if Version(peft_version) < Version("0.12.0"):
    ...
    try:
        ...
        logger.warning_once(
            "TuneLabs unsuccessfully patched LoraLayer.update_layer. Please file a bug report.\n"
...
USE_MODELSCOPE = os.environ.get("TUNELABS_USE_MODELSCOPE", "0") == "1"
if USE_MODELSCOPE:
...
def _get_statistics(statistics = None, force_download = True):
    ...
    if statistics is not None:
        ...
        from tunelabs_zoo.rl_environments import execute_with_time_limit
        ...
        if has_internet():
            ...
            try:
                ...
                raise TimeoutError(
                    "TuneLabs: HuggingFace seems to be down after trying for 120 seconds :(\n"
                    "Check https://status.huggingface.co/ for more details.\n"
                    "As a temporary measure, use modelscope with the same model name ie:\n"
                    "```\n"
                    "pip install modelscope\n"
                    "import os; os.environ['TUNELABS_USE_MODELSCOPE'] = '1'\n"
                    "from tunelabs import FastLanguageModel\n"
                    "model = FastLanguageModel.from_pretrained('tunelabs/gpt-oss-20b')\n"
                    "```"
                )
            except Exception:
                logger.debug("TuneLabs: stats_check failed with an exception.")
...
def get_statistics(local_files_only = False):
    ...
    # We simply download a README.md file from HF - all data is made public.
    # This is simply so we can check if some envs are broken or not.
    # You can disable this by setting TUNELABS_DISABLE_STATISTICS
    import os

    if (
        "TUNELABS_DISABLE_STATISTICS" in os.environ
        or os.environ.get("TUNELABS_USE_MODELSCOPE", "0") == "1"
    ):
        return
    if local_files_only:
        return
...
def offload_input_embeddings(model, temporary_location: str = "_tunelabs_temporary_saved_buffers"):
...
def offload_output_embeddings(model, temporary_location: str = "_tunelabs_temporary_saved_buffers"):
    offloaded_W = offload_to_disk(
...
def _tunelabs_pre_compute_loss(self, model, inputs, *args, **kwargs):
    ...
    if (
        ...
        logger.warning_once(
            f"TuneLabs: Not an error, but {name} does not accept `num_items_in_batch`.\n"
            "Using gradient accumulation will be very slightly less accurate.\n"
            "Read more on gradient accumulation issues here: https://tunelabs.ai/blog/gradient"
        )
...
def patch_gradient_accumulation_fix(Trainer):
    ...
    # Fixes Output 0 of TuneLabsFusedLossBackward is a view and is being modified inplace.
    import inspect

    if hasattr(Trainer, "get_batch_samples"):
        if Trainer.get_batch_samples.__name__ == "_tunelabs_get_batch_samples":
            return
        if (
            ...
            raise NotImplementedError("TuneLabs: Please make a Github issue immediately!!")
        else:
            if Trainer.get_batch_samples.__name__ != "_tunelabs_get_batch_samples":
                Trainer.get_batch_samples = _tunelabs_get_batch_samples

            # Also fix passing in num_items_in_batch
            if not hasattr(Trainer, "_old_compute_loss"):
                # Fix transformers 4.57.0 causing `Output 0 of TuneLabsFusedLossBackward is a view and is being modified inplace.`
                ...
                Trainer.compute_loss = _tunelabs_pre_compute_loss
    else:
        logger.warning_once(
            "TuneLabs: We fixed a gradient accumulation bug, "
            "but it seems like you don't have the latest transformers version!\n"
            "Please update transformers, TRL and tunelabs via:\n"
            "`pip install --upgrade --no-cache-dir --no-deps tunelabs transformers git+https://github.com/huggingface/trl.git`"
        )

    # Also fix up loss scaling ie negate loss *= self.args.gradient_accumulation_steps
    if not (
        Trainer.training_step.__name__ == "_tunelabs_training_step"
        ...
        function = function.replace("def training_step", "def _tunelabs_training_step", 1)
        ...
        exec(function, globals())
        Trainer.training_step = _tunelabs_training_step
    ...
    if not getattr(Trainer, "_tunelabs_init_wrapped_for_accelerate_gas", False):
        _original_trainer_init = Trainer.__init__

        def _tunelabs_trainer_init(self, *args, **kwargs):
        ...
        _tunelabs_trainer_init.__wrapped__ = _original_trainer_init
        Trainer.__init__ = _tunelabs_trainer_init
        Trainer._tunelabs_init_wrapped_for_accelerate_gas = True


def _tunelabs_compile_cache_leaves():
    # Accepts `TUNELABS_COMPILE_LOCATION` overrides (the env var tunelabs_zoo honors).
    leaves = {"tunelabs_compiled_cache", "tunelabs_cache", "tunelabs_compiled"}
    loc = os.environ.get("TUNELABS_COMPILE_LOCATION", "") or ""
    loc = loc.rstrip("/\\")
    if loc:
...
def _forward_is_tunelabs_compiled(model):
    # True iff forward was installed from the TuneLabs compile cache directory.
    # __module__ stays as the transformers module, so check co_filename.
    leaves = _tunelabs_compile_cache_leaves()
...
def apply_accepts_loss_kwargs_fix(model):
    ...
    if _forward_is_tunelabs_compiled(model):
        _shadow_accepts_loss_kwargs(model, True)
        return "True (TuneLabs compiled forward)"
...
def patch_tokenizer(model, tokenizer):
    ...
    if model is not None:
        model.config.update({"tunelabs_version": __version__})
...
def tunelabs_compile_transformers(
    dtype,
    model_name,
    model_types,
    token = None,
    ...
    disable = False,
    return_logits = False,
    tunelabs_force_compile = False,
):
    if Version(torch_version) < Version("2.4.0"):
        print(
            "="
            * 30
            + "TuneLabs: Unfortunately TuneLabs vision and other newer optimized models need Torch 2.4 or later.\n"
        ...
        return
    if trust_remote_code and tunelabs_force_compile == False:
        print(
            "TuneLabs: We can't trace models if `trust_remote_code = True`, "
    ...
    for model_type in model_types:
        _tunelabs_compile_transformers(
            model_type,
...
LOGITS_ERROR_STRING = (
    "TuneLabs: Logits are empty from 2024.11 onwards. To get raw logits again, please "
    'set the environment variable `TUNELABS_RETURN_LOGITS` to `"1" BEFORE starting to train ie before `trainer.train()`. For example:\n'
    "```\nimport os\n"
    "os.environ['TUNELABS_RETURN_LOGITS'] = '1'\n"
    "trainer.train()\n```\n"
    "No need to restart your console - just add `os.environ['TUNELABS_RETURN_LOGITS'] = '1'` before trainer.train() and re-run the cell!"
)
...
def validate_loftq_config(loftq_config, lora_dropout, bias, init_lora_weights, model):
    ...
    if lora_dropout != 0:
        logger.warning_once(
            f"TuneLabs: Dropout = 0 is supported for fast patching. You are using dropout = {lora_dropout}.\n"
            f"TuneLabs will patch all other layers, except LoRA matrices, causing a performance hit."
        )

    if bias != "none":
        logger.warning_once(
            f"TuneLabs: bias = `none` is supported for fast patching. You are using bias = {bias}.\n"
            f"TuneLabs will patch all other layers, except LoRA matrices, causing a performance hit."
        )
    ...
    if not (
        ...
        raise ValueError(
            'TuneLabs: `init_lora_weights` must be either [True, False, "gaussian", "loftq", "corda"].'
        )
    ...
    if init_lora_weights == "loftq":
        if not SUPPORTS_LOFTQ:
            import peft
            raise RuntimeError(
                f"TuneLabs: Your PEFT version of {peft.__version__} does not support LoftQ init.\n"
        ...
        if loftq_config == {}:
            ...
            logger.warning_once(
                "TuneLabs: init_lora_weights = `loftq` is set, but `loftq_config` is None.\n"
        ...
        if hasattr(model.config, "quantization_config"):
            raise ValueError(
                "TuneLabs: You are using `loftq` init, yet `load_in_4bit = True` was set.\n"
...
def fast_inference_setup(model_name, model_config):
    ...
    if not is_vLLM_available():
        logger.warning_once("TuneLabs: vLLM is not installed! Will use TuneLabs inference!")
        fast_inference = False
    from tunelabs_zoo.vllm_utils import (
        patch_vllm,
        vllm_dynamic_quant_supported,
    )

    patch_vllm()
    if model_name.endswith("tunelabs-bnb-4bit"):
        if not vllm_dynamic_quant_supported(model_name, model_config):
            # Instead use -bnb-4bit variant
            logger.warning_once(
                f"TuneLabs: Switching from TuneLabs dynamic quant to normal quant since\n"
                f"we do not yet support fast inference for {model_name}"
            )
            model_name = model_name[: -len("tunelabs-bnb-4bit")] + "bnb-4bit"
...
def patch_peft_fast_inference(model):
    ...
    if vllm_engine is not None:
        ...
        from tunelabs_zoo.vllm_utils import save_lora, load_lora
...
def error_out_no_vllm(*args, **kwargs):
    raise NotImplementedError(
        "TuneLabs: vLLM is not yet supported for fast inference for this model! Please use `.generate` instead"
    )
...
try:
    ...
    try:
        ...
        print("TuneLabs: TorchAO changed `torchao.quantization.Int4WeightOnlyConfig`")
...
def _prepare_model_for_qat(
    ...
    if qat_scheme == "int8-int4":
        ...
        if is_gemma3:
            print(
                "TuneLabs: Gemma3 has a large vocabulary causing int8 embedding issues. "
    ...
    if not isinstance(qat_scheme, TorchAOConfig):
        ...
        if qat_scheme == "fp8-int4":
            ...
            if skipped_cactus_layers:
                ...
                warnings.warn(
                    f"TuneLabs: qat_scheme='cactus' uses PerGroup({group_size}) "
...
def verify_fp8_support_if_applicable(model_config):
    ...
    if quant_method in ["fbgemm_fp8", "fp8"] and DEVICE_TYPE != "cuda":
        raise ValueError(
            f"TuneLabs: FP8 quantization is only supported on CUDA GPUs. You are using {DEVICE_TYPE}."
        )
    ...
    if DEVICE_TYPE == "cuda":
        ...
        if quant_method == "fbgemm_fp8" and major_version < 9:
            ...
            raise ValueError(
                f"TuneLabs: FBGEMM FP8 quantization is only supported on H100 and higher GPUs. L4 is not supported. You are using {torch.cuda.get_device_name()}. Refer to https://developer.nvidia.com/cuda-gpus for more details."
            )
        if quant_method == "fp8" and major_version * 10 + minor_version < 89:
            ...
            raise ValueError(
                f"TuneLabs: FP8 quantization is only supported on L4 and higher GPUs with compute capability 8.9 or higher. You are using {torch.cuda.get_device_name()}. Refer to https://developer.nvidia.com/cuda-gpus for more details."
            )
...
def _resolve_moe_parameter_name(model, default_name: str, alternate_name: str) -> str:
    ...
    Most current TuneLabs MoE models expose expert weights under
...
def get_moe_target_parameters(model, target_modules = None) -> Optional[List[str]]:
    ...
    if moe_params:
        print(
            f"TuneLabs: Detected MoE model with {num_experts = } and {target_modules = }. Enabling LoRA on MoE parameters: {moe_params}"
        )
        return moe_params
...
def make_fast_generate_wrapper(original_generate):
    ...
    def _fast_generate_wrapper(*args, **kwargs):
        ...
        if "sampling_params" in kwargs:
            raise ValueError(
                "TuneLabs: `sampling_params` is only supported when `fast_inference=True` (vLLM). "
        ...
        if "lora_request" in kwargs:
            raise ValueError(
                "TuneLabs: `lora_request` is only supported when `fast_inference=True` (vLLM). "
        ...
        if len(args) > 0:
            ...
            if is_string_input:
                raise ValueError(
                    "TuneLabs: Passing text strings to `fast_generate` is only supported "
...
# Dynamic quant checkpoints (eg gemma-3-4b-it-tunelabs-bnb-4bit) encode skip paths as
...
# Ref: https://github.com/tunelabsai/tunelabs/issues/4208
