...
from tunelabs.models._utils import TorchAOConfig
...
BAD_MAPPINGS = {
    "tunelabs/Qwen3-32B-tunelabs-bnb-4bit".lower(): "tunelabs/Qwen3-32B-bnb-4bit".lower(),  # 32B dynamic quant is way too big
    "tunelabs/Qwen3-30B-A3B-tunelabs-bnb-4bit".lower(): "tunelabs/Qwen3-30B-A3B".lower(),  # HF loads MoEs too slowly
    "tunelabs/Qwen3-30B-A3B-bnb-4bit".lower(): "tunelabs/Qwen3-30B-A3B".lower(),  # We rather do it on the fly
    "tunelabs/Qwen3-30B-A3B-Base-tunelabs-bnb-4bit".lower(): "tunelabs/Qwen3-30B-A3B-Base".lower(),  # HF loads MoEs too slowly
    "tunelabs/Qwen3-30B-A3B-Base-bnb-4bit".lower(): "tunelabs/Qwen3-30B-A3B-Base".lower(),  # We rather do it on the fly
}


def _get_torchao_fp8_config(fp8_mode):
    # Lazy import so a broken optional vLLM install doesn't break `import tunelabs`.
    from tunelabs_zoo.vllm_utils import _get_torchao_fp8_config as _impl
...
def __get_model_name(
    ...
    if load_in_fp8 != False:
        if load_in_fp8 == True and (os.environ.get("TUNELABS_HAS_FBGEMM", "0") == "1"):
        ...
        print(
            f"TuneLabs: Your transformers version of {transformers_version} does not support native "
        ...
        # logger.warning_once(
        #     f"TuneLabs: You passed in `{model_name}` which is a 4bit model, yet you set\n"\
        #     f"`load_in_4bit = False`. We shall load `{new_model_name}` instead."
        # )
        return new_model_name

    elif not load_in_4bit and lower_model_name in MAP_TO_TUNELABS_16bit:
        new_model_name = MAP_TO_TUNELABS_16bit[lower_model_name]
        return new_model_name
        ...
        # logger.warning_once(
        #     f"TuneLabs: You passed in `{model_name}` and `load_in_4bit = True`.\n"\
...
def _get_new_mapper():
    try:
        ...
        new_mapper = (
            "https://raw.githubusercontent.com/tunelabsai/tunelabs/main/tunelabs/models/mapper.py"
        )
        ...
        new_mapper = (
            ...
            .replace("MAP_TO_TUNELABS_16bit", "NEW_MAP_TO_TUNELABS_16bit")
        )
...
def _resolve_with_mappers(
    model_name, load_in_4bit, load_in_fp8, int_to_float, float_to_int, map_to_tunelabs_16bit
):
    return __get_model_name(
        ...
        MAP_TO_TUNELABS_16bit = map_to_tunelabs_16bit,
...
def get_model_name(
    ...
    if new_model_name is None and model_name.count("/") == 1 and model_name[0].isalnum():
        # Try checking if a new TuneLabs version allows it!
        NEW_INT_TO_FLOAT_MAPPER, NEW_FLOAT_TO_INT_MAPPER, NEW_MAP_TO_TUNELABS_16bit = (
            _get_new_mapper()
        )
        upgraded_model_name = _resolve_with_mappers(
            ...
            map_to_tunelabs_16bit = NEW_MAP_TO_TUNELABS_16bit,
        )
        if upgraded_model_name is not None:
            raise NotImplementedError(
                f"TuneLabs: {model_name} is not supported in your current TuneLabs version! Please update TuneLabs via:\n\n"
                "pip uninstall tunelabs tunelabs_zoo -y\n"
                'pip install --upgrade --no-cache-dir "tunelabs[colab-new] @ git+https://github.com/tunelabsai/tunelabs.git"\n'
                'pip install --upgrade --no-cache-dir "git+https://github.com/tunelabsai/tunelabs-zoo.git"\n'
            )
...
def _offline_quantize_to_fp8(
    ...
    *,
    ...
    print(f"TuneLabs: Quantizing '{model_name}' to fp8, using model_name='{new_model_name}' instead")
...
def check_and_disable_bitsandbytes_loading(
    ...
    if load_in_4bit or load_in_8bit:
        if verbose:
            print(
                f"TuneLabs: Model already quantized with {quant_method}. "
...
def sync_tunelabs_model_name_bnb_flags(load_in_4bit, load_in_8bit):
    """Make TUNELABS_MODEL_NAME's `_load_in_4bit_`/`_load_in_8bit_` tokens match the EFFECTIVE bnb
    ...
    name = os.environ.get("TUNELABS_MODEL_NAME", "")
    ...
    os.environ["TUNELABS_MODEL_NAME"] = name
...
def _get_fp8_mode_and_check_settings(
    ...
    if fp8_mode not in ["row", "block"]:
        raise ValueError(f"TuneLabs: `load_in_fp8` can only be 'row' or 'block', got '{fp8_mode}'")
    if full_finetuning:
        raise ValueError("TuneLabs: `load_in_fp8` is not compatible with full finetuning")
    if load_in_4bit or load_in_8bit or load_in_16bit:
        raise ValueError(
            "TuneLabs: `load_in_fp8` is not compatible with `load_in_4bit`, `load_in_8bit` or `load_in_16bit`",
        )
    ...
    if not (
        ...
        raise ValueError(
            "TuneLabs: On the fly `load_in_fp8` requires H100 GPUs or after. Try `tunelabs/Qwen3-8B` instead."
        )

    # Check if torch >= 2.9.0
    if Version(torch.__version__) < Version("2.9.0"):
        raise ValueError(
            "TuneLabs: On the fly `load_in_fp8` requires torch 2.9.0+. Try `tunelabs/Qwen3-8B` instead."
        )
    ...
    if importlib.util.find_spec("torchao") is None:
        raise ValueError(
            "TuneLabs: Please install torchao for on the fly float8 to work! Try `tunelabs/Qwen3-8B` instead."
        )
    import torchao

    error_message = (
        "TuneLabs: `load_in_fp8` requires torchao 0.15.0+ (or nightly).\n"
    ...
    if (
        ...
        if Version(fbgemm_gpu.__version__) < Version("1.4.1"):
            ...
            logger.info(
                f"TuneLabs: fbgemm_gpu_genai=={fbgemm_gpu.__version__} is old for FP8 loading. "
