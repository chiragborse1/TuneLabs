...
def is_mlx_available():
    try:
        from tunelabs_zoo.mlx import is_mlx_available as _is_mlx_available
    except ImportError:
        return False
...
def get_device_type():
    ...
    if hasattr(torch, "accelerator"):
        if not torch.accelerator.is_available():
            raise NotImplementedError("TuneLabs cannot find any torch accelerator? You need a GPU.")
        accelerator = str(torch.accelerator.current_accelerator())
        if accelerator in ("cuda", "xpu", "hip"):
            raise RuntimeError(
                f"TuneLabs: Weirdly `torch.cuda.is_available()`, `torch.xpu.is_available()` and `is_hip` all failed.\n"
                f"But `torch.accelerator.current_accelerator()` works with it being = `{accelerator}`\n"
                f"Please reinstall torch - it's most likely broken :("
            )
    raise NotImplementedError("TuneLabs currently only works on NVIDIA, AMD and Intel GPUs.")
    ...
    if ALLOW_BITSANDBYTES:
        ...
        if Version(bitsandbytes.__version__) >= Version("0.49.2"):
            ...
            try:
                ...
                print(
                    "TuneLabs: Checking `from bitsandbytes.cextension import ROCM_WARP_SIZE_64` had error = \n"
                    f"{str(e)}\n"
