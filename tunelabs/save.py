...
from tunelabs_zoo.utils import Version
from importlib.metadata import version as importlib_version
from tunelabs_zoo.hf_utils import dtype_from_config, HAS_TORCH_DTYPE
...
# H4: Defensive imports -- these were added in tunelabs-zoo PR #526
# and may not exist on older versions
try:
    from tunelabs_zoo.llama_cpp import LLAMA_CPP_DEFAULT_DIR, IS_WINDOWS
except ImportError:
    import sys
...
def _quantize_q2_k_l(
    ...
    # "Q2_K_L" is an TuneLabs preset, not a native llama.cpp ftype: q2_k with
    ...
    if print_output:
        print(
            "TuneLabs: Quantizing as Q2_K_L preset "
    ...
    if print_output:
        ...
        print(f"TuneLabs: Successfully quantized to {output_gguf} (size: {file_size_gb:.2f}GB)")
    return str(output_gguf)


def check_if_sentencepiece_model(model, temporary_location = "_tunelabs_sentencepiece_temp"):
...
def _preserve_sentencepiece_tokenizer_assets(
        ...
        if desired_added_tokens_decoder:
            ...
            if tokenizer_config.get("added_tokens_decoder") != desired_added_tokens_decoder:
                ...
                with open(tokenizer_config_path, "w", encoding = "utf-8") as file:
                    ...
                    file.write("\n")
                logger.warning_once(
                    f"TuneLabs: Restored added_tokens_decoder metadata in "
    ...
    if not os.path.isfile(tokenizer_model) and downloaded_path is not None:
        ...
        logger.warning_once(
            f"TuneLabs: Preserved sentencepiece asset `tokenizer.model` in " f"{save_directory}."
        )
...
def _free_cached_model(model):
    ...
    for cached_repo in cached_repos:
        if cached_repo.repo_id == model.config._name_or_path:
            ...
            logger.warning_once(
                "TuneLabs: Will remove a cached repo with size "
    ...
    if isinstance(layer, (Bnb_Linear4bit, Peft_Linear4bit, Peft_Linear)):
        ...
        if A is not None:
            ...
            if not torch.isfinite(maximum_element).item():
                raise ValueError(f"TuneLabs: Merge failed.\n{name} has some elements = infinity.")
        W = W.t().to(dtype)
    else:
        W = layer.weight
    return W, bias
...
def _preserve_tokenizer_eos_token(
    ...
    try:
        ...
        logger.warning_once(
            f"TuneLabs: Could not preserve tokenizer eos_token in {tokenizer_config}: {error}"
        )
...
def tunelabs_save_model(
    ...
    commit_message: Optional[str] = "Trained with TuneLabs",
    private: Optional[bool] = None,
    create_pr: bool = False,
    revision: str = None,
    commit_description: str = "Upload model trained with TuneLabs 2x faster",
    tags: List[str] = None,
    # Our functions
    temporary_location: str = "_tunelabs_temporary_saved_buffers",
    ...
    if commit_message is None:
        commit_message = ""
    if "TuneLabs" not in commit_message:
        commit_message += " (Trained with TuneLabs)"
    commit_message = commit_message.lstrip()

    if commit_description is None:
        commit_description = "Upload model trained with TuneLabs 2x faster"
    elif "TuneLabs 2x faster" not in commit_description:
        commit_description += " (Trained with TuneLabs 2x faster)"

    if save_method == "merged_4bit":
        raise RuntimeError(
            "TuneLabs: Merging into 4bit will cause your model to lose accuracy if you plan\n"
    ...
    if push_to_hub:
        ...
        try:
            ...
            raise RuntimeError(
                "TuneLabs: Please supply a token!\nGo to https://huggingface.co/settings/tokens"
            )
    ...
    if save_method != "lora" and save_method != "merged_16bit" and save_method != "merged_4bit":
        raise RuntimeError(
            "TuneLabs: You must select one of 3 options when saving models:\n"
    ...
    if save_method == "merged_4bit":
        print("TuneLabs: Merging 4bit and LoRA weights to 4bit...")
    ...
    if ((save_method == "lora") or (save_method == "merged_4bit")) and push_to_hub:
        if token is None:
            raise RuntimeError(
                "TuneLabs: Pushing to HF requires a token. Pass `token = 'hf_....'`\n"
                "Go to https://huggingface.co/settings/tokens."
            )

        if save_method == "lora":
            print("TuneLabs: Saving LoRA adapters. Please wait...")
        elif save_method == "merged_4bit":
            print("TuneLabs: Saving 4bit Bitsandbytes model. Please wait...")
    ...
    if (
        ...
        if tokenizer is not None:
            print("TuneLabs: Saving tokenizer...", end = "")
        ...
        print("TuneLabs: Saving model...", end = "")
        if save_method != "lora":
    ...
    if push_to_hub and "/" in save_directory:
        ...
        if IS_KAGGLE_ENVIRONMENT:
            ...
            logger.warning_once(
                "TuneLabs: You are pushing to hub in Kaggle environment.\n"
                f"To save memory, we shall move {save_directory} to {new_save_directory}"
            )
        else:
            logger.warning_once(
                f"TuneLabs: You are pushing to hub, but you passed your HF username = {username}.\n"
    ...
    print("TuneLabs: Merging 4bit and LoRA weights to 16bit...")
    ...
    if safe_serialization is None:
        ...
        logger.warning_once(
            f"TuneLabs: You have {n_cpus} CPUs. Using `safe_serialization` is 10x slower.\n"
    ...
    if IS_KAGGLE_ENVIRONMENT or IS_COLAB_ENVIRONMENT:
        ...
        logger.warning_once(
            "TuneLabs: Kaggle/Colab has limited disk space. We need to delete the downloaded\n"
    ...
    print("TuneLabs: Saving model... This might take 5 minutes ...")
    ...
    for key, value in state_dict.items():
        ...
        if type(value) is not torch.Tensor:
            logger.warning_once(f"TuneLabs: {key} is not a Tensor but a {type(value)}.")
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        print(f"TuneLabs: Saving to organization with address {new_save_directory}")
    ...
    if save_pretrained_settings["push_to_hub"] and (username != actual_username):
        ...
        print("TuneLabs: Uploading all files... Please wait...")
...
def install_llama_cpp_make_non_blocking():
    ...
    if check == 0:
        ...
        if check != 0:
            raise RuntimeError(
                f"*** TuneLabs: Failed compiling llama.cpp using os.system(...) with error {check}. Please report this ASAP!"
            )
        ...
        with subprocess.Popen(
            ...
            for line in sp.stdout:
                ...
                if "undefined reference" in line:
                    raise RuntimeError(
                        f"*** TuneLabs: Failed compiling llama.cpp with {line}. Please report this ASAP!"
                    )
...
def install_llama_cpp_old(version = -10):
    ...
    if os.path.exists("llama.cpp"):
        print(
            ...
            "TuneLabs will DELETE the broken directory and install a new one.\n"
    ...
    if not (
        ...
        raise RuntimeError(
            "TuneLabs: The file 'llama.cpp/llama-quantize' or `llama.cpp/quantize` does not exist.\n"
...
def save_to_gguf(
    ...
    model_directory: str = "tunelabs_finetuned_model",
    ...
    """
    # print_output True only if TUNELABS_ENABLE_LOGGING=1
    if os.environ.get("TUNELABS_ENABLE_LOGGING", "0") == "1":
        print_output = True
    else:
        print_output = False
    ...
    if isinstance(quantization_method, list):
        ...
        raise TypeError("TuneLabs: quantization_method can only be a string or a list of strings")

    # Check if bfloat16 is supported
    if model_dtype == "bf16" and not torch.cuda.is_bf16_supported():
        logger.warning(
            "TuneLabs: Cannot convert to bf16 GGUF since your computer doesn't support it.\n"
    ...
    for quant_method in quantization_method:
        if quant_method.startswith("iq2"):
            raise RuntimeError(
                "TuneLabs: Currently iq2 type quantizations aren't supported yet - sorry!"
            )
    ...
    for quant_method in quantization_method:
        ...
        if quant_method not in ALLOWED_QUANTS.keys():
            error = f"TuneLabs: Quant method = [{quant_method}] not supported. Choose from below:\n"
            for key, value in ALLOWED_QUANTS.items():
                error += f"[{key}] => {value}\n"
    ...
    if is_gpt_oss:
        print("TuneLabs: GPT-OSS model detected - using special conversion settings")
    ...
    if first_conversion == "bf16" and not torch.cuda.is_bf16_supported():
        logger.warning("TuneLabs: Switching bf16 to f16 due to hardware limitations")
        first_conversion = "f16"

    first_conversion_dtype = "" if first_conversion == "None" else first_conversion
    # Print conversion info
    print_info = (
        f"==((====))==  TuneLabs: Conversion from HF to GGUF information\n"
    ...
    try:
        ...
        print("TuneLabs: llama.cpp found in the system. Skipping installation.")
    except:
        print("TuneLabs: Installing llama.cpp. This might take 3 minutes...")
        if IS_KAGGLE_ENVIRONMENT:
    ...
    print("TuneLabs: Preparing converter script...")
    with use_local_gguf():
        ...
        print(f"TuneLabs: [1] Converting model into {first_conversion_dtype} GGUF format.")
    ...
    for file in initial_files:
        if not os.path.exists(file):
            if IS_KAGGLE_ENVIRONMENT:
                raise RuntimeError(
                    f"TuneLabs: Conversion failed for {file}\n"
    ...
    print(f"TuneLabs: Initial conversion completed! Files: {initial_files}")
    ...
    if not is_gpt_oss:
        ...
        for quant_method in quantization_method:
            if quant_method != first_conversion:
                print(
                    f"TuneLabs: [2] Converting GGUF {first_conversion_dtype} into {quant_method}. This might take 10 minutes..."
                )
                ...
                try:
                    if quant_method == "q2_k_l":
                        ...
                        # Use tunelabs-zoo's standard quantization for all other methods
                    ...
                    if IS_KAGGLE_ENVIRONMENT:
                        raise RuntimeError(
                            f"TuneLabs: Quantization failed for {output_location}\n"
        ...
        print("TuneLabs: Model files cleanup...")
        ...
        print("TuneLabs: GPT-OSS model - skipping additional quantizations")
        want_full_precision = True

    print(f"TuneLabs: All GGUF conversions completed successfully!")
    print(f"Generated files: {all_saved_locations}")

    return all_saved_locations, want_full_precision, is_vlm


def tunelabs_save_pretrained_merged(
    self,
    save_directory: Union[str, os.PathLike],
    tokenizer = None,
    ...
    if tokenizer is None:
        logger.warning_once(
            "TuneLabs: You're not saving a tokenizer as well?\n"
    ...
    del arguments["self"]
    tunelabs_save_model(**arguments)
    for _ in range(3):
        gc.collect()


def tunelabs_push_to_hub_merged(
    self,
    repo_id: str,
    tokenizer = None,
    save_method: str = "merged_16bit",  # ["lora", "merged_16bit", "merged_4bit"]
...
This {model_type} model was trained 2x faster with [TuneLabs](https://github.com/tunelabsai/tunelabs)
...
def _determine_username(save_directory, old_username, token):
    ...
    if "/" not in save_directory:
        ...
        try:
            ...
            raise RuntimeError(f"TuneLabs: {save_directory} is not a Huggingface directory.")
    else:
    ...
    try:
        ...
        if datasets:
            try:
                ...
                logger.warning_once(
                    f"TuneLabs: Could not update datasets metadata for {save_directory}: {e}"
                )
...
def upload_to_huggingface(
    ...
    if file_location is not None:
        ...
        if len(ftevent_files) > 0:
            print(
                "TuneLabs: Uploading tensorboard files... Please wait...",
        ...
        if create_config:
            ...
            with open("_temporary_tunelabs_config.json", "w", encoding = "utf-8") as file:
                json.dump({"model_type": model.config.model_type}, file, indent = 4)
            hf_api.upload_file(
                path_or_fileobj = "_temporary_tunelabs_config.json",
                path_in_repo = "config.json",
                repo_id = save_directory,
                repo_type = "model",
                commit_message = "(Trained with TuneLabs)",
            )
            os.remove("_temporary_tunelabs_config.json")
    return username
...
def fix_tokenizer_bos_token(tokenizer):
    ...
    if tokenizer("A").input_ids[0] == getattr(tokenizer, "bos_token_id", None):
        if chat_template is not None and (
            ...
            fix_bos_token = True
            logger.warning(
                "TuneLabs: ##### The current model auto adds a BOS token.\n"
                "TuneLabs: ##### Your chat template has a BOS token. We shall remove it temporarily."
            )
...
def create_ollama_modelfile(tokenizer, base_model_name, model_location):
    ...
    if not ollama_template_name:
        print(
            f"TuneLabs: No Ollama template mapping found for model '{base_model_name}'. Skipping Ollama Modelfile"
        )
        return None
    ...
    # Fixes https://github.com/tunelabsai/tunelabs/issues/1087
...
@_normalize_tied_weights_keys_for_save
def tunelabs_save_pretrained_gguf(
    self,
    save_directory: Union[str, os.PathLike],
    tokenizer = None,
    ...
    """
    if tokenizer is None:
        raise ValueError("TuneLabs: Saving to GGUF must have a tokenizer.")
    if isinstance(tokenizer, (PreTrainedTokenizerBase, ProcessorMixin)):
        tokenizer = patch_saving_functions(tokenizer)
    ...
    if push_to_hub:
        raise ValueError(
            "TuneLabs: Please use .push_to_hub_gguf() instead of .save_pretrained_gguf() with push_to_hub=True"
        )
    ...
    if is_gpt_oss:
        if quantization_method is not None:
            ...
            if _ignored:
                logger.warning_once(
                    f"TuneLabs: GPT-OSS does not support GGUF quantization "
    ...
    if is_peft_model:
        print(f'TuneLabs: Merging model weights to {"mxfp4" if is_gpt_oss else "16-bit"} format...')
        try:
            # Call tunelabs_generic_save directly (it's in the same file)
            tunelabs_generic_save(**arguments)
        ...
        if original_path and os.path.isdir(original_path):
            print(
                f"TuneLabs: Model is not a PEFT model. Using existing checkpoint at {original_path}"
            )
            ...
            print("TuneLabs: Model is not a PEFT model. Saving directly without LoRA merge...")
    ...
    try:
        ...
        if type(model_dtype) is str:
            ...
            raise TypeError("TuneLabs: Model dtype can only be float16 or bfloat16")
    except Exception as e:
        # Fallback if dtype_from_config fails
        print(f"TuneLabs: Could not determine dtype ({e}), defaulting to float16")
        model_dtype = "float16"

    # Step 8: Convert to GGUF format
    print("TuneLabs: Converting to GGUF format...")
    ...
    if quantization_method is not None:
        ...
        if isinstance(quantization_method, list):
            ...
            raise TypeError(
                "TuneLabs: quantization_method can only be a string or a list of strings"
            )
    ...
    try:
        ...
        logger.warning(f"TuneLabs: fix_sentencepiece_gguf skipped ({type(e).__name__}): {e}")
    ...
    try:
        ...
        if IS_KAGGLE_ENVIRONMENT:
            raise RuntimeError(
                f"TuneLabs: GGUF conversion failed in Kaggle environment.\n"
                f"This is likely due to the 20GB disk space limit.\n"
                f"Try saving to /tmp directory or use a smaller model.\n"
                f"Error: {e}"
            )
        else:
            raise RuntimeError(f"TuneLabs: GGUF conversion failed: {e}")
    ...
    if fix_bos_token:
        logger.warning(
            "TuneLabs: ##### The current model auto adds a BOS token.\n"
            "TuneLabs: ##### We removed it in GGUF's chat template for you."
        )
    ...
    if is_vlm_update:
        print("\n")
        print(
            f"TuneLabs: example usage for Multimodal LLMs: {os.path.join(_bin_dir, 'llama-mtmd-cli' + _exe)} -m {all_file_locations[0]} --mmproj {all_file_locations[-1]}"
        )
        print("TuneLabs: load image inside llama.cpp runner: /image test_image.jpg")
        print("TuneLabs: Prompt model to describe the image")
    else:
        print(
            f'TuneLabs: example usage for text only LLMs: {os.path.join(_bin_dir, "llama-cli" + _exe)} --model {all_file_locations[0]} -p "why is the sky blue?"'
        )

    if ollama_success:
        print(f"TuneLabs: Saved Ollama Modelfile to {modelfile_location}")
        print(
            f"TuneLabs: convert model to ollama format by running - ollama create model_name -f {modelfile_location}"
        )
...
def tunelabs_push_to_hub_gguf(
    ...
    if use_temp_dir or use_temp_dir is None:
        ...
        temp_dir = tempfile.mkdtemp(prefix = "tunelabs_gguf_")
    ...
    print(f"TuneLabs: Converting model to GGUF format...")

    try:
        # Call save_pretrained_gguf - it returns all the info we need
        result = tunelabs_save_pretrained_gguf(
            self = self,
            save_directory = save_directory,
            tokenizer = tokenizer,
    ...
    print("TuneLabs: Uploading GGUF to Huggingface Hub...")
    ...
    try:
        ...
        for file_location in all_file_locations:
            ...
            if cleanup_temp and "tunelabs_gguf_" in original_name:
...
This model was finetuned and converted to GGUF format using [TuneLabs](https://github.com/tunelabsai/tunelabs).
...
"""
        ...
        readme_content += (
            "This was trained 2x faster with [TuneLabs](https://github.com/tunelabsai/tunelabs)\n"
            '[<img src="https://raw.githubusercontent.com/tunelabsai/tunelabs/main/images/tunelabs%20made%20with%20love.png" width="200"/>](https://github.com/tunelabsai/tunelabs)\n'
        )
        ...
        print(f"TuneLabs: Successfully uploaded GGUF to https://huggingface.co/{full_repo_id}")

        # Add tags
        if tags is None:
            tags = []
        tags.extend(["gguf", "llama-cpp", "tunelabs"])
        if is_vlm:
        ...
        if datasets:
            try:
                ...
                logger.warning_once(
                    f"TuneLabs: Could not update datasets metadata for {full_repo_id}: {e}"
                )
        ...
        if cleanup_temp:
            print("TuneLabs: Cleaning up temporary files...")
            for d in [save_directory, f"{save_directory}_gguf"]:
                if os.path.exists(d):
                    try:
                        shutil.rmtree(d)
...
def save_lora_to_custom_dir(model, tokenizer, save_directory):
    ...
    # Call the tunelabs_save_model function with the custom directory
    tunelabs_save_model(
        model,
        tokenizer,
        save_directory = save_directory,
...
def tunelabs_convert_lora_to_ggml_and_push_to_hub(
    self,
    tokenizer,
    repo_id: str,
    use_temp_dir: Optional[bool] = None,
    commit_message: Optional[str] = "Converted LoRA to GGML with TuneLabs",
    private: Optional[bool] = None,
    token: Union[bool, str, None] = None,
    create_pr: bool = False,
    revision: str = None,
    commit_description: str = "Convert LoRA to GGML format using TuneLabs",
    temporary_location: str = "_tunelabs_temporary_saved_buffers",
    maximum_memory_usage: float = 0.85,
):
    ...
    print(f"TuneLabs: Converting auto-saved LoRA adapters at {lora_directory_push} to GGML format.")
    ...
    print(f"TuneLabs: Conversion completed! Output file: {output_file}")

    print("TuneLabs: Uploading GGML file to Hugging Face Hub...")
    ...
    print(
        "\nThis GGML making function was made by Maheswar. Ping him @Maheswar on the TuneLabs Discord or on HuggingFace (@mahiatlinux) if you like this!"
    )


def tunelabs_convert_lora_to_ggml_and_save_locally(
    self,
    ...
    print(f"TuneLabs: Converting auto-saved LoRA adapters at {save_directory} to GGML format.")
    print(f"The output file will be {output_file}")
...
from .models.loader_utils import get_model_name
from tunelabs_zoo.saving_utils import (
...
def save_to_gguf_generic(
    ...
    if repo_id is not None and token is None:
        raise RuntimeError("TuneLabs: Please specify a token for uploading!")

    if not os.path.exists(os.path.join("llama.cpp", "tunelabs_convert_hf_to_gguf.py")):
...
@_normalize_tied_weights_keys_for_save
@torch.inference_mode
def tunelabs_generic_save(
    model,
    tokenizer,
    save_directory: Union[str, os.PathLike] = "tunelabs_finetuned_merge",
    save_method: str = "lora",  # ["lora", "merged_16bit", "merged_4bit"]
    push_to_hub: bool = False,
    token: Optional[Union[str, bool]] = None,
    is_main_process: bool = True,
    ...
    if not _is_peft:
        ...
        if push_to_hub:
            print(f"TuneLabs: Pushing full fine-tuned model to '{save_directory}' ...")
            model.push_to_hub(
                repo_id = save_directory,
                token = token,
                private = private,
            ...
            print(f"TuneLabs: Saving full fine-tuned model to '{save_directory}' ...")
        ...
        print(f"TuneLabs: Model saved successfully to '{save_directory}'")
    else:
        merge_and_overwrite_lora(
            get_model_name,
            model = model,
...
def tunelabs_generic_save_pretrained_merged(
    self,
    save_directory: Union[str, os.PathLike],
    tokenizer = None,
    save_method: str = "merged_16bit",  # ["lora", "merged_16bit", "merged_4bit"]
...
def tunelabs_generic_push_to_hub_merged(
    self,
    repo_id: str,
    tokenizer = None,
    save_method: str = "merged_16bit",  # ["lora", "merged_16bit", "merged_4bit"]
...
def _tunelabs_save_torchao_with_attached_config(
    model,
    save_directory: Union[str, os.PathLike],
    tokenizer,
    push_to_hub: bool = False,
    ...
    if isinstance(model, PeftModelForCausalLM):
        _tunelabs_save_torchao_with_given_config(
            model = model,
            save_directory = save_directory,
            tokenizer = tokenizer,
...
def _tunelabs_save_torchao_with_given_config(
    model,
    save_directory: Union[str, os.PathLike],
    tokenizer,
    torchao_config,
    ...
    if push_to_hub:
        assert token is not None, "TuneLabs: Please specify a token for uploading!"

    assert (
        torchao_config is not None
    ), "TuneLabs: Please specify a torchao_config for post-training quantization!"
...
def tunelabs_save_pretrained_torchao(
    self,
    save_directory: Union[str, os.PathLike],
    tokenizer = None,
    torchao_config = None,
    ...
    if torchao_config is not None:
        ...
        assert not has_qat_config, (
            "TuneLabs: You passed `torchao_config` but this model was trained with `qat_scheme`. "
        ...
        assert has_qat_config, (
            "TuneLabs: No `torchao_config` provided and model was not trained with `qat_scheme`. "
            "Either train with `qat_scheme` parameter, or provide a `torchao_config` for "
            "post-training quantization."
        )
        _tunelabs_save_torchao_with_attached_config(
            model = self,
            save_directory = save_directory,
            tokenizer = tokenizer,
            push_to_hub = push_to_hub,
...
def not_implemented_save(*args, **kwargs):
    raise NotImplementedError("TuneLabs: Sorry GGUF is currently not supported for vision models!")
...
def patch_saving_functions(model, vision = False):
    ...
    if model.push_to_hub.__name__ == "tunelabs_push_to_hub":
    ...
    push_to_hub_text = f'''def tunelabs_push_to_hub(self, {signature}:
    """
    {docs}
    """
    arguments = dict(locals())
    ...
    if "tags" in arguments and arguments["tags"] is not None:
        ...
        arguments["tags"] = list(arguments["tags"]) + ["tunelabs",]
    elif "tags" in arguments:
        arguments["tags"] = ["tunelabs",]
    elif hasattr(self, "add_model_tags"):
        self.add_model_tags(["tunelabs",])
    ...
    if "commit_message" in arguments:
        ...
        if commit_message is not None:
            ...
            commit_message = "Upload model trained with TuneLabs"
    ...
    if "commit_description" in arguments:
        ...
        if commit_description is not None:
            ...
            if "TuneLabs" not in commit_description:
                commit_description += "(Trained with TuneLabs 2x faster)"
        else:
            commit_description = "Upload model trained with TuneLabs 2x faster"
    ...
    def tunelabs_tokenizer_save_pretrained(
        self,
        save_directory,
        legacy_format = None,
        filename_prefix = None,
    ...
    if (
        ...
        and model.save_pretrained.__name__ != "tunelabs_tokenizer_save_pretrained"
    ):
        model.original_save_pretrained = model.save_pretrained
        model.save_pretrained = types.MethodType(tunelabs_tokenizer_save_pretrained, model)
    ...
    while True:
        ...
        if (
            ...
            and original_model.push_to_hub.__name__ != "tunelabs_push_to_hub"
        ):
            original_model.original_push_to_hub = original_model.push_to_hub
            original_model.push_to_hub = types.MethodType(tunelabs_push_to_hub, original_model)
    ...
    if not vision:
        if hasattr(model, "config"):
            ...
            model.push_to_hub_merged = types.MethodType(tunelabs_generic_push_to_hub_merged, model)
            model.save_pretrained_merged = types.MethodType(
                tunelabs_generic_save_pretrained_merged, model
            )
            model.push_to_hub_gguf = types.MethodType(tunelabs_push_to_hub_gguf, model)
            model.save_pretrained_gguf = types.MethodType(tunelabs_save_pretrained_gguf, model)
            model.save_pretrained_torchao = types.MethodType(tunelabs_save_pretrained_torchao, model)
            model.push_to_hub_ggml = types.MethodType(
                tunelabs_convert_lora_to_ggml_and_push_to_hub, model
            )
            model.save_pretrained_ggml = types.MethodType(
                tunelabs_convert_lora_to_ggml_and_save_locally, model
            )
    else:
