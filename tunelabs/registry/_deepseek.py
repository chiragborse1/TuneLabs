from tunelabs.registry.registry import ModelInfo, ModelMeta, QuantType, _register_models
...
DeepseekR1DistillLlamaMeta = ModelMeta(
    ...
    model_info_cls = DeepseekR1ModelInfo,
    is_multimodal = False,
    quant_types = {"8": [QuantType.TUNELABS, QuantType.GGUF], "70": [QuantType.GGUF]},
)
...
DeepseekR1DistillQwenMeta = ModelMeta(
    ...
    is_multimodal = False,
    quant_types = {
        "1.5": [QuantType.TUNELABS, QuantType.BNB, QuantType.GGUF],
        "7": [QuantType.TUNELABS, QuantType.BNB],
        "14": [QuantType.TUNELABS, QuantType.BNB, QuantType.GGUF],
...
def _list_deepseek_r1_distill_models():
    from tunelabs.utils.hf_hub import ModelInfo as HfModelInfo
    from tunelabs.utils.hf_hub import list_models

    models: list[HfModelInfo] = list_models(author = "tunelabs", search = "Distill", limit = 1000)
    distill_models = []
    for model in models:
        model_id = model.id
...
if __name__ == "__main__":
    from tunelabs.registry.registry import MODEL_REGISTRY, _check_model_info
