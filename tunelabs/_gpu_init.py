...
# freeze Hub's cache constants with the un-redirected paths. tunelabs_zoo
...
# triggering the full tunelabs_zoo package init this early; the zoo's own
# call later is an idempotent no-op. Older tunelabs_zoo without hf_cache.py
# is skipped silently.
try:
    import importlib.util as _importlib_util
    from pathlib import Path as _Path

    _zoo_spec = _importlib_util.find_spec("tunelabs_zoo")
    if _zoo_spec is not None and _zoo_spec.origin:
        ...
        if _hf_cache_file.is_file():
            _hf_cache_spec = _importlib_util.spec_from_file_location(
                "tunelabs_zoo._early_hf_cache", _hf_cache_file
            )
...
configure_amdgpu_asic_id_table_path()
# Must precede `import tunelabs_zoo` below, which imports bnb on ROCm.
fix_bitsandbytes_rocm_arch_detection()
disable_broken_causal_conv1d()
disable_broken_vllm()
fix_message_factory_issue()
...
# TuneLabs patches these libraries at import time; if imported first, the
# unoptimized versions run, risking OOM or slower training.
if already_imported:
    # stacklevel=2 points the warning at the user's import line
    warnings.warn(
        f"WARNING: TuneLabs should be imported before [{', '.join(already_imported)}] "
        ...
        f"Please restructure your imports with 'import tunelabs' at the top of your file.",
        stacklevel = 2,
    )
del already_imported, critical_modules

# Pin BNB_ROCM_VERSION before bitsandbytes is first imported (`import
# tunelabs_zoo` below pulls it in on ROCm hosts).
...
# Fixes https://github.com/tunelabsai/tunelabs/issues/1266
...
# Check for tunelabs_zoo
try:
    tunelabs_zoo_version = importlib_version("tunelabs_zoo")
    if Version(tunelabs_zoo_version) < Version("2026.5.2"):
        print(
            "TuneLabs: Please update TuneLabs and TuneLabs-Zoo to the latest version!\n"
            "Do this via `pip install --upgrade --force-reinstall --no-cache-dir --no-deps tunelabs tunelabs_zoo`"
        )
        # if os.environ.get("TUNELABS_DISABLE_AUTO_UPDATES", "0") == "0":
        #     try:
        #         os.system("pip install --upgrade --no-cache-dir --no-deps tunelabs_zoo")
        #     except:
        #         try:
        #             os.system("pip install --upgrade --no-cache-dir --no-deps --user tunelabs_zoo")
        #         except:
        #             raise ImportError("TuneLabs: Please update tunelabs_zoo via `pip install --upgrade --no-cache-dir --no-deps tunelabs_zoo`")
    import tunelabs_zoo
except PackageNotFoundError:
    raise ImportError(
        f"TuneLabs: Please install tunelabs_zoo via `pip install tunelabs_zoo` then retry!"
    )
except:
    raise
...
try:
    ...
    raise ImportError(
        "TuneLabs: Pytorch is not installed. Go to https://pytorch.org/.\n"
        "We have some installation instructions on our Github page."
    )
except:
    raise

from tunelabs_zoo.device_type import (
    is_hip,
    get_device_type,
    DEVICE_TYPE,
    DEVICE_TYPE_TORCH,
...
if DEVICE_TYPE == "cuda":
    ...
    try:
        ...
        print(
            "TuneLabs: `bitsandbytes` is not installed - 4bit QLoRA unallowed, but 16bit and full finetuning works!"
        )
        bnb = None
    try:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            warnings.warn("TuneLabs: Running `ldconfig /usr/lib64-nvidia` to link CUDA.")
            ...
            try:
                ...
                warnings.warn(
                    "TuneLabs: CUDA is not linked properly.\n"
                    "Try running `python -m bitsandbytes` then `python -m xformers.info`\n"
                    "We tried running `ldconfig /usr/lib64-nvidia` ourselves, but it didn't work.\n"
                    "You need to run in your terminal `sudo ldconfig /usr/lib64-nvidia` yourself, then import TuneLabs.\n"
                    "Also try `sudo ldconfig /usr/local/cuda-xx.x` - find the latest cuda version.\n"
                    "TuneLabs will still run for now, but maybe it might crash - let's hope it works!"
                )
...
# TUNELABS_ALLOW_CPU=1 (CPU-only CI): rebinding trl.SFTTrainer.__init__
