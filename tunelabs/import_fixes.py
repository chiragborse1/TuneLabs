...
# We cannot do from tunelabs_zoo.log import logger since FBGEMM might cause seg faults.
TUNELABS_ENABLE_LOGGING = os.environ.get("TUNELABS_ENABLE_LOGGING", "0") in (
    "1",
    "True",
    "true",
)
...
def Version(version):
    try:
        ...
        raise RuntimeError(
            f"TuneLabs: Could not get version for `{version}`\n"
...
if not TUNELABS_ENABLE_LOGGING:
    ...
    # bnb-4bit / TuneLabs paths don't use torchao's cpp kernels, so drop only that record rather
...
def fix_message_factory_issue():
    try:
        ...
        if not hasattr(google.protobuf.message_factory, "MessageFactory"):
            logger.info("TuneLabs: Patching protobuf.MessageFactory as it doesn't exist")
            ...
            logger.info("TuneLabs: Patching protobuf.MessageFactory.GetPrototype")
        pass
    except:
        pass
            ...
            if cutlass.exists():
                with open(cutlass, "r+", encoding = "utf-8") as f:
                    ...
                    if "num_splits_key=-1," in text:
                        ...
                        f.write(text)
                        f.truncate()
                        logger.info("TuneLabs: Patching Xformers to fix some performance issues.")
        except Exception as e:
            logger.info(f"TuneLabs: Failed patching Xformers with error = {str(e)}")
...
def patch_vllm_for_notebooks():
    ...
    logger.info(
        "TuneLabs: Notebook detected - Patching sys.stdout.fileno for newer `vllm>=0.12.0` versions"
    )
            ...
            if ovis_config.exists():
                ...
                backbone_config = AutoConfig.for_model(model_type,
                    ...
                    backbone_config = AIMv2Config(**backbone_config)""",
                        ...
                        f.truncate()
                        logger.info(
                            "TuneLabs: Patching vLLM to fix `'aimv2' is already used by a Transformers config, pick another name.`"
                        )
        except Exception as e:
            logger.info(f"TuneLabs: Failed patching vLLM with error = {str(e)}")


# vLLM >= 0.22 (PR #35024) deleted `vllm.transformers_utils.tokenizer`, but an
# older tunelabs_zoo still imports it unguarded and crashes (issue #6385). Supply
...
_VLLM_TOKENIZER_STUB_SENTINEL = "__tunelabs_vllm_tokenizer_stub__"


def _tunelabs_return_no_lora_tokenizer(*args, **kwargs):
    # None -> vLLM uses the base tokenizer for LoRA (matches tunelabs_zoo).
    return None
...
class _VllmLoraTokenizerStubLoader(importlib.abc.Loader):
    ...
    def create_module(self, spec):
        ...
        module.__file__ = f"<tunelabs stub: {self.module_name}>"
        module.__package__ = self.module_name.rpartition(".")[0]
        setattr(module, _VLLM_TOKENIZER_STUB_SENTINEL, True)
        module.get_lora_tokenizer = _tunelabs_return_no_lora_tokenizer
        module.get_lora_tokenizer_async = _tunelabs_return_no_lora_tokenizer
        return module
...
def fix_vllm_lora_tokenizer_module():
    ...
    logger.info(
        "TuneLabs: Installed `vllm.transformers_utils.tokenizer` compatibility "
...
def patch_ipykernel_hf_xet():
    ...
    if (
        ...
        print(
            "#### TuneLabs: `hf_xet==1.1.10` and `ipykernel==7.0.0` or `ipykernel==7.0.1` breaks progress bars. Using ASCII progress bars.\n"
            "#### TuneLabs: To re-enable progress bars, please upgrade to `ipykernel>=7.1.0` or wait for a fix to\n"
...
def patch_trackio():
    ...
    # See https://github.com/tunelabsai/notebooks/pull/110
    os.environ["TRACKIO_LOGO_LIGHT_URL"] = (
        "https://raw.githubusercontent.com/tunelabsai/tunelabs/main/images/tunelabs%20logo%20black%20text.png"
    )
    os.environ["TRACKIO_LOGO_DARK_URL"] = (
        "https://raw.githubusercontent.com/tunelabsai/tunelabs/main/images/tunelabs%20logo%20white%20text.png"
    )
...
def patch_datasets():
    ...
    if (datasets_version <= Version("4.5.0")) and (datasets_version >= Version("4.4.0")):
        raise NotImplementedError(
            f"#### TuneLabs: Using `datasets = {str(datasets_version)}` will cause recursion errors.\n"
...
def check_fbgemm_gpu_version():
    ...
    if Version(fbgemm_gpu_version) < Version("1.4.0"):
        os.environ["TUNELABS_HAS_FBGEMM"] = "0"
        logger.info(
            f"TuneLabs: fbgemm_gpu_genai=={fbgemm_gpu_version} is old and may cause issues. "
            f"Disabling FBGEMM - using Triton kernels instead."
        )
        return

    logger.info(f"TuneLabs: fbgemm_gpu_genai=={fbgemm_gpu_version} detected.")
...
def patch_enable_input_require_grads():
    ...
    logger.info("TuneLabs: Patched enable_input_require_grads for vision model compatibility")
...
def patch_unsafe_trainer_rng_load():
    ...
    if load_rng_state is None or getattr(load_rng_state, "_tunelabs_safe_rng_load", False):
        return
    try:
    ...
    try:
        ...
        def check_torch_load_is_safe():
            if TrueVersion(torch.__version__.split("+")[0]) < TrueVersion("2.6"):
                raise RuntimeError(
                    "TuneLabs: refusing to load checkpoint RNG state on torch < 2.6 "
    ...
    if not getattr(torch.load, "_tunelabs_rng_guard", False):
        ...
        _guarded_torch_load._tunelabs_rng_guard = True
        _guarded_torch_load._tunelabs_rng_flag = _rng_active
        torch.load = _guarded_torch_load
    _rng_active = torch.load._tunelabs_rng_flag

    @functools.wraps(load_rng_state)
    def _tunelabs_safe_load_rng_state(self, checkpoint):
        _rng_active.on = True
        try:
    ...
    _tunelabs_safe_load_rng_state._tunelabs_safe_rng_load = True
    Trainer._load_rng_state = _tunelabs_safe_load_rng_state
    logger.info("TuneLabs: Hardened Trainer._load_rng_state rng loading (CVE-2026-1839).")
...
def torchvision_compatibility_check():
    ...
    if os.environ.get("TUNELABS_SKIP_TORCHVISION_CHECK", "0").lower() in ("1", "true"):
        return

    if importlib.util.find_spec("torch") is None:
        raise ImportError("TuneLabs: torch not found. Please install torch first.")
    ...
    if tv_v >= Version(required_tv_str):
        logger.info(
            f"TuneLabs: torch=={torch_version_raw} and "
    ...
    message = (
        f"TuneLabs: torch=={torch_version_raw} requires "
    ...
    if is_custom or is_prerelease:
        ...
        logger.warning(
            ...
            f"Set TUNELABS_SKIP_TORCHVISION_CHECK=1 to silence this."
        )
        return
    ...
    try:
        with open(openenv, "r+", encoding = "utf-8") as f:
            ...
            if bad + "\n" + "\n" in text and replace_with not in text:
                ...
                f.write(text)
                f.truncate()
                logger.info("TuneLabs: Patching TRL OpenEnv to fix SamplingParams not defined")
    except Exception as e:
        logger.info(f"TuneLabs: Failed patching TRL OpenEnv with error = {str(e)}")
    ...
    try:
        ...
        with open(executorch, "r+", encoding = "utf-8") as f:
            ...
            if bad in text and what not in text:
                ...
                f.write(text)
                f.truncate()
                logger.info("TuneLabs: Patching Executorch to fix get_mapped_key")
    except Exception as e:
        logger.info(f"TuneLabs: Failed Executorch with error = {str(e)}")
...
def fix_triton_compiled_kernel_missing_attrs():
    ...
    logger.info(
        "TuneLabs: Patched triton CompiledKernel with num_ctas/cluster_dims "
...
def patch_trunc_normal_precision_issue():
    ...
    if getattr(torch.nn.init, "_tunelabs_trunc_normal_patched", False):
        return

    original_trunc_normal = torch.nn.init.trunc_normal_
    if getattr(original_trunc_normal, "__tunelabs_trunc_normal_patched__", False):
        torch.nn.init._tunelabs_trunc_normal_patched = True
        return
    ...
    _patched_trunc_normal_.__tunelabs_trunc_normal_patched__ = True
    _patched_trunc_normal_._tunelabs_original = original_trunc_normal
    torch.nn.init._tunelabs_trunc_normal_original = original_trunc_normal
    torch.nn.init.trunc_normal_ = _patched_trunc_normal_
    torch.nn.init._tunelabs_trunc_normal_patched = True
    logger.info("TuneLabs: Patched torch.nn.init.trunc_normal_ for fp16/bf16 stability.")
...
def check_vllm_torch_sm100_compatibility():
    ...
    raise RuntimeError(
        f"TuneLabs: Incompatible configuration detected.\n\n"
...
def fix_vllm_pdl_blackwell():
    ...
    try:
        ...
        if vllm_version >= Version(VLLM_PDL_FIX_VERSION):
            logger.info(
                f"TuneLabs: SM100 ({sm100_gpu_name}) detected but vLLM {vllm_version} "
                f"should include PDL fix - skipping workaround"
            )
            return
    except Exception as e:
        logger.debug(f"TuneLabs: vLLM version check failed ({e}), applying PDL workaround.")
    ...
    if patched:
        logger.info(
            f"TuneLabs: Applied PDL fix for SM100 ({sm100_gpu_name}) - "
            f"patched: {', '.join(patched)}"
        )
    else:
        # Just set the env var - vLLM might be an older version without supports_pdl
        logger.info(f"TuneLabs: Set TRITON_DISABLE_PDL=1 for SM100 ({sm100_gpu_name})")
...
def patch_openspiel_env_async():
    ...
    try:
        ...
        try:
            import nest_asyncio
            nest_asyncio.apply()
            logger.info("TuneLabs: Applied nest_asyncio for OpenEnv EnvClient async compatibility")
        except ImportError:
            logger.info(
                "TuneLabs: nest_asyncio not installed, OpenEnv async methods may need manual wrapping"
            )
...
def patch_torchcodec_audio_decoder():
    """Call tunelabs_zoo's AudioDecoder patch."""
    try:
        from tunelabs_zoo.dataset_utils import patch_torchcodec_audio_decoder as _patch
        _patch()
...
def disable_broken_wandb():
    ...
    try:
        ...
        logger.info(
            "TuneLabs: wandb is installed but broken (likely a protobuf version mismatch). "
...
# parties can introspect ``__tunelabs_stub__`` to detect our patch.
_TUNELABS_STUB_SENTINEL = "__tunelabs_stub__"
...
def _make_peft_stub_module(fullname):
    ...
    mod.__file__ = f"<tunelabs stub: {fullname}>"
    mod.__package__ = fullname.rpartition(".")[0]
    setattr(mod, _TUNELABS_STUB_SENTINEL, True)
    return mod
...
def _install_transformers_conversion_mapping_stub():
    ...
    if existing is not None and getattr(existing, _TUNELABS_STUB_SENTINEL, False):
        return existing
...
def _install_transformers_core_model_loading_stub():
    ...
    class ConversionOps:
        def convert(self, *args, **kwargs):  # pragma: no cover - inert stub
            raise NotImplementedError(
                "tunelabs stub: transformers.core_model_loading.ConversionOps "
...
def fix_peft_transformers_weight_conversion_import():
    ...
    logger.info(
        "TuneLabs: stubbed transformers.conversion_mapping / "
...
def patch_peft_weight_converter_compatibility():
    ...
    if getattr(twc, "_tunelabs_weight_converter_compat_patch", False):
        return
    ...
    twc._tunelabs_weight_converter_compat_patch = True


def _patch_peft_moe_target_conversion(twc):
    """Keep PEFT 0.19 MoE conversion from rewriting explicit TuneLabs targets."""
    if getattr(twc, "_tunelabs_moe_target_conversion_patch", False):
        return
    ...
    def _convert_peft_config_moe_tunelabs(peft_config, model_type: str) -> None:
    ...
    twc._convert_peft_config_moe = _convert_peft_config_moe_tunelabs
    twc._tunelabs_moe_target_conversion_patch = True


CAUSAL_CONV1D_BROKEN = False
_CAUSAL_CONV1D_PREFIX = "causal_conv1d"
_CAUSAL_CONV1D_BLOCKER_SENTINEL = "_tunelabs_causal_conv1d_blocker"
VLLM_BROKEN = False
_VLLM_PREFIX = "vllm"
_VLLM_BLOCKER_SENTINEL = "_tunelabs_vllm_blocker"
_ROCM_ENV_HINT_KEYS = (
    "ROCM_PATH",
    "ROCM_HOME",
    "HIP_PATH",
...
def _is_rocm_torch_build() -> bool:
    ...
    try:
        ...
        if "rocm" in torch_version_raw:
            _log_rocm_detection("TuneLabs: ROCm detection matched torch version tag (+rocm).")
            return True
    except Exception:
        pass
    ...
    for key in _ROCM_ENV_HINT_KEYS:
        ...
        if isinstance(value, str) and value.strip():
            _log_rocm_detection(f"TuneLabs: ROCm detection matched environment key `{key}`.")
            return True
    ...
    for path in _ROCM_PATH_HINTS:
        try:
            if path.exists():
                _log_rocm_detection(f"TuneLabs: ROCm detection matched filesystem hint `{path}`.")
                return True
        except Exception:
            continue

    _log_rocm_detection("TuneLabs: ROCm detection did not match any known hints.")
    return False
    ...
    for candidate in _iter_amdgpu_asic_id_table_candidates():
        try:
            if candidate.is_file():
                ...
                if TUNELABS_ENABLE_LOGGING:
                    logger.info(f"TuneLabs: Set {_AMDGPU_ASIC_ID_TABLE_PATH_ENV}={candidate}")
...
# them. Must run before `import tunelabs_zoo` (imports bnb on ROCm).
# ---------------------------------------------------------------------------

_BNB_CUDA_SPECS_MODULE = "bitsandbytes.cuda_specs"
_BNB_ROCM_FIX_FINDER_SENTINEL = "_tunelabs_bnb_rocm_fix_finder"
_BNB_ROCM_FIX_FUNCTION_FLAG = "__tunelabs_bnb_rocm_fix__"
...
def _run_hipinfo(hipinfo_path):
    ...
    try:
        ...
        _log_rocm_detection(f"TuneLabs: `{hipinfo_path}` failed: {e}")
        return ""


def _tunelabs_get_rocm_gpu_arch():
    ...
    _log_rocm_detection(
        "TuneLabs: Could not detect the ROCm GPU architecture - bitsandbytes will see `unknown`."
    )
    return "unknown"


def _tunelabs_get_rocm_warpsize():
    ...
    _log_rocm_detection(
        "TuneLabs: Could not detect the ROCm warp size - defaulting to 64 "
        "(bitsandbytes' own default)."
    )
    return 64


setattr(_tunelabs_get_rocm_gpu_arch, _BNB_ROCM_FIX_FUNCTION_FLAG, True)
setattr(_tunelabs_get_rocm_warpsize, _BNB_ROCM_FIX_FUNCTION_FLAG, True)
...
def _patch_bnb_cuda_specs_module(module):
    ...
    for attribute_name, replacement in (
        ("get_rocm_gpu_arch", _tunelabs_get_rocm_gpu_arch),
        ("get_rocm_warpsize", _tunelabs_get_rocm_warpsize),
    ):
        ...
        patched = True
        logger.info(
            f"TuneLabs: Patched bitsandbytes.cuda_specs.{attribute_name} - "
...
class _BnbCudaSpecsPatchLoader(importlib.abc.Loader):
    ...
    def exec_module(self, module):
        ...
        try:
            ...
            _log_rocm_detection(f"TuneLabs: bitsandbytes ROCm detection patch failed: {e}")
...
def _repair_imported_bitsandbytes_rocm_constants():
    """bnb imported before tunelabs: noise already fired, but fix detectors
    ...
    for module_name, module in list(sys.modules.items()):
        ...
        try:
            ...
            continue
    logger.info("TuneLabs: Repaired bitsandbytes ROCm arch / warp-size constants in place.")
...
def fix_bitsandbytes_rocm_arch_detection():
    ...
    upstream-fixed bnb. Idempotent. Opt out: TUNELABS_DISABLE_BNB_ROCM_FIX=1."""
    if os.environ.get("TUNELABS_DISABLE_BNB_ROCM_FIX", "0") == "1":
        return
    ...
    _log_rocm_detection("TuneLabs: Installed the bitsandbytes ROCm arch detection patch hook.")
...
def _get_vllm_cuda_mismatch_message(error):
    ...
    return (
        f"TuneLabs: vLLM was built for CUDA {wanted_cuda} but this system has "
...
def _patch_find_spec_for_causal_conv1d():
    ...
    if getattr(current_find_spec, "_tunelabs_causal_conv1d_find_spec_patch", False):
        return
    ...
    _blocked_find_spec._tunelabs_causal_conv1d_find_spec_patch = True
    _blocked_find_spec._tunelabs_original_find_spec = current_find_spec
    importlib.util.find_spec = _blocked_find_spec


def _patch_find_spec_for_vllm():
    current_find_spec = importlib.util.find_spec
    if getattr(current_find_spec, "_tunelabs_vllm_find_spec_patch", False):
        return
    ...
    _blocked_find_spec._tunelabs_vllm_find_spec_patch = True
    _blocked_find_spec._tunelabs_original_find_spec = current_find_spec
    importlib.util.find_spec = _blocked_find_spec
...
def disable_broken_vllm(error = None):
    ...
    if cuda_msg:
        ...
        logger.warning(
            "TuneLabs: Detected broken vLLM binary extension; "
            "disabling vLLM imports and continuing import.\n"
            "Please reinstall via `uv pip install tunelabs vllm torchvision torchaudio "
            "--torch-backend=auto`."
        )
    return True
...
def disable_broken_causal_conv1d():
    ...
    This mirrors TuneLabs's FlashAttention fallback behavior: if importing causal_conv1d
    ...
    _disable_transformers_causal_conv1d()
    print(
        "TuneLabs: Detected broken causal_conv1d binary; "
...
def maybe_set_windows_rocm_bnb_version():
    ...
    (marked ``TUNELABS_BNB_ROCM_VERSION_SOURCE=sitecustomize``) are
    redetectable defaults, not overrides; ``TUNELABS_SKIP_BNB_ROCM_VERSION=1``
    opts out and drops a seeded default. Returns the value set, else None.
    """
    if sys.platform != "win32":
        return None
    if os.environ.get("TUNELABS_SKIP_BNB_ROCM_VERSION") == "1":
        # Real opt-out: drop our seeded default (marker present); explicit
        # user values carry no marker and are kept.
        if os.environ.get("TUNELABS_BNB_ROCM_VERSION_SOURCE") == "sitecustomize":
            os.environ.pop("BNB_ROCM_VERSION", None)
            os.environ.pop("TUNELABS_BNB_ROCM_VERSION_SOURCE", None)
        return None
    if "BNB_ROCM_VERSION" in os.environ and (
        os.environ.get("TUNELABS_BNB_ROCM_VERSION_SOURCE") != "sitecustomize"
    ):
        return None
    ...
    if TUNELABS_ENABLE_LOGGING:
        logger.info(
            f"TuneLabs: set BNB_ROCM_VERSION={version} "
...
def patch_accelerate_recursively_apply():
    """
    Make Accelerate's recursive utilities tolerate TuneLabs's EmptyLogits
    ...
    if original_recursively_apply is not None and not getattr(
        original_recursively_apply, "__tunelabs_patched__", False
    ):
        ...
        _patched_recursively_apply.__tunelabs_patched__ = True
    ...
    if original_find_device is not None and not getattr(
        original_find_device, "__tunelabs_patched__", False
    ):
        ...
        _patched_find_device.__tunelabs_patched__ = True
