...
# Legacy Windows consoles (cp1252) can't encode TuneLabs's emoji/box-drawing
...
def _is_mlx_available():
    ...
    # tunelabs_zoo until tunelabs_zoo.mlx is import-safe on GPU hosts. Then this
    # can collapse back to the centralized zoo runtime call below.
    if (
        os.environ.get("TUNELABS_FORCE_GPU_PATH", "0") == "1"
...
if _IS_MLX:
    try:
        ...
        raise ImportError(
            "TuneLabs: MLX support requires `tunelabs-zoo` with MLX modules. "
            "Reinstall with `pip install tunelabs-zoo` or rerun install.sh."
        ) from _e
    # An older tunelabs-zoo satisfies `import tunelabs_zoo` but lacks the
    ...
    try:
        ...
        raise ImportError(
            "TuneLabs: MLX support requires an tunelabs-zoo build that includes "
            "`tunelabs_zoo.mlx.trainer` and `tunelabs_zoo.mlx.loader`. Upgrade with "
            "`pip install -U tunelabs-zoo` or rerun install.sh."
        ) from _e
    ...
    _raw_text_spec = importlib.util.spec_from_file_location("tunelabs._mlx_raw_text", _raw_text_path)
    if _raw_text_spec is None or _raw_text_spec.loader is None:
        raise ImportError("TuneLabs: could not load MLX raw_text dataprep helpers.")
    ...
    __version__ = tunelabs_zoo.__version__
    DEVICE_TYPE = "mlx"
    ...
    class FastSentenceTransformer:
        ...
        def from_pretrained(*args, **kwargs):
            raise NotImplementedError(
                "TuneLabs: FastSentenceTransformer is not yet supported on MLX."
            )
    ...
    class TuneLabsVisionDataCollator:
        def __init__(self, *args, **kwargs):
            raise NotImplementedError(
                "TuneLabs: TuneLabsVisionDataCollator is not used on MLX. "
