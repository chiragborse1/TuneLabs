...
# why: bypass partially-initialised tunelabs ns during _gpu_init load
from .models._utils import is_bfloat16_supported
from tunelabs.utils import (
...
from tunelabs_zoo.training_utils import (
    tunelabs_train as _tunelabs_train,
)
from tunelabs_zoo.vision_utils import (
    TuneLabsVisionDataCollator as _TuneLabsVisionDataCollatorBase,
)
from tunelabs.models.vision import check_dataset_for_missing_videos
from tunelabs_zoo.hf_utils import get_transformers_model_type
from tunelabs_zoo.utils import Version
import dataclasses

__all__ = [
    "TuneLabsTrainingArguments",
    "TuneLabsTrainer",
    "tunelabs_train",
    "_patch_trl_trainer",
    "TuneLabsVisionDataCollator",
    "QGaloreConfig",
...
class TuneLabsVisionDataCollator(_TuneLabsVisionDataCollatorBase):
    """
...
_AUTO_PADDING_FREE_ENV_DISABLED = os.environ.get(
    "TUNELABS_DISABLE_AUTO_PADDING_FREE", ""
...
def _should_pack(config) -> bool:
    if config is None or not getattr(config, "packing", False):
        return False
    return not getattr(config, "_tunelabs_disable_auto_packing", False)
...
def _disable_sample_packing(config):
    ...
    setattr(config, "_tunelabs_disable_auto_packing", True)
...
if Version(transformers_version) > Version("4.45.2"):
    ...
    def tunelabs_train(trainer, *args, **kwargs):
        if len(args) != 0 or len(kwargs) != 0:
            raise RuntimeError(
                "TuneLabs: Our custom gradient accumulation fixed trainer does not support other arguments.\n"
                "If you want to use our fix inside of HF, please update `transformers` to the latest version via:\n"
                "`pip uninstall transformers -y && pip install --upgrade --no-cache-dir transformers`"
            )
        print(
            "TuneLabs: Using our custom gradient accumulation fixed trainer, which is not feature complete.\n"
            "If you want to use our fix inside of HF, please update `transformers` to the latest version via:\n"
            "`pip uninstall transformers -y && pip install --upgrade --no-cache-dir transformers`"
        )
        return _tunelabs_train(trainer)
...
class QGaloreConfig:
    ...
    Pass an instance of this class to ``TuneLabsTrainingArguments`` (via
...
class TuneLabsTrainingArguments(TrainingArguments):
    def __init__(
        self,
...
def _create_tunelabs_optimizer(
    model,
    optimizer_cls,
    optimizer_kwargs,
    embedding_lr = 5e-5,
    ...
    for name, param in model.named_parameters():
        ...
        if name.endswith("modules_to_save.default.weight"):
            ...
            print(
                f"TuneLabs: Setting lr = {embedding_lr:.2e} instead of {lr:.2e} for {partial_name}."
            )
...
class TuneLabsTrainer(SFTTrainer):
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            self.optimizer = _create_tunelabs_optimizer(
                self.model,
                optimizer_cls,
                optimizer_kwargs,
    ...
    def _create_q_galore_optimizer(
        ...
        from tunelabs.optimizers.q_galore_adamw import (
            QGaLoreAdamW8bit,
...
def _patch_sft_trainer_auto_packing(trl_module):
    ...
    if sft_trainer is None:
        return
    if getattr(sft_trainer, "_tunelabs_auto_packing_wrapped", False):
        return
    ...
    def new_init(self, *args, **kwargs):
        ...
        blocked = (
            ...
            or (
                os.environ.get("TUNELABS_RETURN_LOGITS", "0") == "1"
        ...
        if blocked and requested_pack:
            ...
            message = f"TuneLabs: Sample packing skipped ({reason} detected)."
            print(message)
        ...
        if _should_pack(config_arg) and not blocked:
            ...
            packing_active = True
            logger.info("TuneLabs: Sample packing enabled for SFTTrainer instance.")
        ...
        if not blocked:
            if padding_free_requested:
                ...
                logger.info("TuneLabs: Padding-free batching auto-enabled for SFTTrainer instance.")
        ...
        try:
            ...
            if packing_active and _should_skip_auto_packing_error(exc):
                logger.info(
                    "TuneLabs: Auto sample packing failed because trainer reported an incompatible setup (%s).",
                    exc,
                )
        ...
        if not blocked and trainer_packing and (packing_active or _should_pack(trainer_args)):
            ...
            print("🦥 TuneLabs: Packing enabled - training is >2x faster and uses less VRAM!")
        elif not blocked and trainer_padding_free:
            enable_padding_free_metadata(self.model, self)
            message = (
                "🦥 TuneLabs: Padding-free auto-enabled, enabling faster training."
                if auto_padding_free_active
                else "🦥 TuneLabs: Padding-free enabled, enabling faster training."
            )
            print(message)

    sft_trainer.__init__ = new_init
    sft_trainer._tunelabs_auto_packing_wrapped = True


def _patch_trl_trainer():
    import trl

    if hasattr(trl, "__TUNELABS_BACKWARDS_COMPATIBLE__"):
        return
    ...
    trl.__TUNELABS_BACKWARDS_COMPATIBLE__ = True
