...
"""Utilities for enabling packed (padding-free) batches across TuneLabs."""
...
def mark_allow_overlength(module):
    ...
    if hasattr(module, "max_seq_length"):
        setattr(module, "_tunelabs_allow_packed_overlength", True)
...
def enable_sample_packing(
    ...
    *,
    ...
    if collator is None or not hasattr(collator, "torch_call"):
        return
    if getattr(collator, "_tunelabs_packing_wrapped", False):
        return
    ...
    collator._tunelabs_packing_wrapped = True
...
def enable_padding_free_metadata(model, trainer):
    ...
    if (
        collator is None
        or getattr(collator, "_tunelabs_padding_free_lengths_wrapped", False)
    ...
    collator._tunelabs_padding_free_lengths_wrapped = True
