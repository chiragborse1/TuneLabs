...
def _get_cache_file_path(cache_key: str) -> str:
    ...
    cache_dir = os.path.expanduser("~/.cache/tunelabs/moe_autotune")
...
def get_or_autotune_moe_kernels(
    ...
    if os.environ.get("TUNELABS_MOE_DISABLE_AUTOTUNE", "0") == "1":
        logger.info(
            f"TUNELABS_MOE_DISABLE_AUTOTUNE=1: Using Heuristic (Safe) MoE kernel configs for SM{device_capability[0]}{device_capability[1]}"
        )
    ...
    try:
        ...
        if "AttributeError" in str(e) and "_experimental_make_tensor_descriptor" in str(e):
            logger.warning(
                "TuneLabs: Your Triton version might be incompatible with TMA features. Falling back to default configs."
            )
