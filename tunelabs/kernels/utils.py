...
if DEVICE_TYPE == "xpu" and Version(torch.__version__) < Version("2.6.0"):
    raise RuntimeError("Intel xpu currently supports tunelabs with torch.version >= 2.6.0")
...
if importlib.util.find_spec("torchao") is not None:
    try:
        ...
        if Version(torchao.__version__) >= Version("0.15.0"):
            print(
                f"TuneLabs: `from torchao.quantization import Float8Tensor` failed on version={torchao.__version__}"
            )
