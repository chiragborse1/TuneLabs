...
from tunelabs_zoo.temporary_patches.common import torch_compile
...
try:
    ...
    FP8Linear = None
    logger.info(
        "TuneLabs: FP8 models need importing FP8Linear from `transformers.integrations.finegrained_fp8` but we don't see it."
    )
...
try:
    ...
    logger.info(
        "TuneLabs: FP8 models need importing FbgemmFP8Linear from `transformers.integrations.fbgemm_fp8` but we don't see it."
    )
...
try:
    ...
    logger.info(
        "TuneLabs: Could not find fbgemm_gpu.experimental.gemm.triton_gemm.fp8_gemm.triton_quantize_fp8_block"
    )
...
try:
    ...
    logger.info(
        "TuneLabs: Could not find torchao.prototype.blockwise_fp8_inference.blockwise_quantization.blockwise_fp8_gemm"
    )
...
def test_has_fbgemm():
    ...
    try:
        ...
        if is_cutlass_cuda_error:
            print("TuneLabs: FBGEMM on the current GPU cannot load - will switch to Triton kernels")
        else:
            print(
                f"TuneLabs: FBGEMM on the current GPU cannot load with error = {e} - will switch to Triton kernels"
            )
        has_fbgemm = False
    del block_scale, xq
...
if "TUNELABS_HAS_FBGEMM" not in os.environ:
...
try:
    ...
    if Version(fbgemm_gpu.__version__) >= Version("1.4.0"):
        ...
        from tunelabs.import_fixes import suppress_cuda_printf
        with suppress_cuda_printf():
            _has_fbgemm = test_has_fbgemm()
        if _has_fbgemm:
            os.environ["TUNELABS_HAS_FBGEMM"] = "1"
