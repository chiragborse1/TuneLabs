...
def patch_loss_functions(torch_compile = True):
    ...
    # (e.g. ForConditionalGeneration for Qwen3.5, Csm...). tunelabs_zoo also
    # does this; remove once the floor pin passes tunelabsai/tunelabs-zoo#656.
    try:
        import transformers.loss.loss_utils as _lu
        _tunelabs_loss = _lu.LOSS_MAPPING.get("ForCausalLM")
        if _tunelabs_loss is not None:
            for _key, _fn in list(_lu.LOSS_MAPPING.items()):
                if getattr(_fn, "__name__", "") == "ForCausalLMLoss":
                    _lu.LOSS_MAPPING[_key] = _tunelabs_loss
