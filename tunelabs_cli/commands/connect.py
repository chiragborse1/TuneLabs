...
"""`tunelabs connect` — launch a coding agent against a running Studio server."""
...
from tunelabs_cli._inference import (
    _USER_AGENT,
    _studio_token,
...
_CODEX_PROFILE = "tunelabs_api"
_CODEX_ENV_KEY = "TUNELABS_STUDIO_AUTH_TOKEN"
_HERMES_ENV_KEY = "TUNELABS_API_KEY"
_HERMES_PROVIDER = "tunelabs"
...
_KEY_OPTION = typer.Option(
    ...
    help = (
        ...
        "(or TUNELABS_API_KEY); it is remembered for next time."
    ),
)
...
def _require_studio() -> str:
    ...
    if base is None:
        expected = os.environ.get("TUNELABS_STUDIO_URL", "http://127.0.0.1:8888")
        _fail(
            f"No running Studio server found at {expected}. Start one with "
            "`tunelabs studio`, or point TUNELABS_STUDIO_URL at a remote server."
        )
    return base
...
def _agent_api_key(base: str, explicit: Optional[str]) -> str:
    ...
    if token is None:
        _fail(
            ...
            "or set TUNELABS_API_KEY."
        )
    key = _http_json(
        "POST",
        f"{base}/api/auth/api-keys",
        token,
        {"name": "Coding agents (tunelabs connect)"},
...
def _require_gguf_for_codex(base: str, key: str, model_id: str) -> None:
    ...
    _fail(
        ...
        f"the transformers backend. Try: tunelabs connect codex --model {hint}"
    )
...
def _merge_codex_config(existing: str, base: str) -> str:
    ...
    # Drop the provider table and any stale [model_providers.tunelabs_api.*] subtables.
...
def write_openclaw_config(base: str, key: str, model: dict) -> None:
    ...
    if config is None:
        typer.echo(
            f"Warning: couldn't parse {path} — add an 'tunelabs' provider there "
    ...
    _subdict(models, "providers")["tunelabs"] = {
        "baseUrl": f"{base}/v1",
        "apiKey": key,
    ...
    _subdict(defaults, "model")["primary"] = f"tunelabs/{model['id']}"
...
def write_opencode_config(base: str, key: str, model: dict) -> None:
    ...
    _subdict(config, "provider")["tunelabs"] = {
    ...
    config["model"] = f"tunelabs/{model['id']}"
    if json.dumps(config, sort_keys = True) != before:
        _write_private_json(path, config)
        typer.echo(f"Updated {path}")
