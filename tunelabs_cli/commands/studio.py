...
studio_app = typer.Typer(help = "TuneLabs Studio commands.")
...
def _enable_verbose_access_logs() -> None:
    ...
    os.environ["TUNELABS_STUDIO_ACCESS_LOG_DEDUP_MS"] = "0"
    os.environ["TUNELABS_STUDIO_ACCESS_LOG_POLL_DEDUP_MS"] = "0"


# Resolve install root: TUNELABS_STUDIO_HOME, then STUDIO_HOME alias, then
# sys.prefix inference (so a direct call to <root>/bin/tunelabs resolves after
# the installer's env var has expired), then legacy ~/.tunelabs/studio.
# TUNELABS_STUDIO_HOME wins when both env vars are set.
def _looks_like_installer_managed_studio_home(candidate: Path) -> bool:
    ...
    tunelabs_studio is not misidentified as a custom Studio root.
    """
    shim_name = "tunelabs.exe" if platform.system() == "Windows" else "tunelabs"
...
def _resolve_studio_home() -> tuple[Path, bool]:
    ...
    try:
        ...
        if prefix.name == "tunelabs_studio":
            ...
            legacy = (Path.home() / ".tunelabs" / "studio").resolve()
            if inferred != legacy and _looks_like_installer_managed_studio_home(inferred):
                return inferred, True
    except (OSError, ValueError):
        pass
    return Path.home() / ".tunelabs" / "studio", False
...
def _ensure_studio_env_exported() -> None:
    """Re-export TUNELABS_STUDIO_HOME / TUNELABS_LLAMA_CPP_PATH only for real
    ...
    if not _STUDIO_HOME_IS_CUSTOM:
        return
    # Truthy-check (not setdefault) so a blank TUNELABS_STUDIO_HOME= does not
    # suppress the inferred custom root.
    if not os.environ.get("TUNELABS_STUDIO_HOME"):
        os.environ["TUNELABS_STUDIO_HOME"] = str(STUDIO_HOME)
    # When override == legacy default, llama.cpp stays at ~/.tunelabs/llama.cpp.
    try:
        _legacy_studio = (Path.home() / ".tunelabs" / "studio").resolve()
        _is_legacy = STUDIO_HOME.resolve() == _legacy_studio
    except (OSError, ValueError):
        _is_legacy = STUDIO_HOME == (Path.home() / ".tunelabs" / "studio")
    if _is_legacy:
        _llama_dir = Path.home() / ".tunelabs" / "llama.cpp"
    else:
        _llama_dir = STUDIO_HOME / "llama.cpp"
    if not os.environ.get("TUNELABS_LLAMA_CPP_PATH"):
        os.environ["TUNELABS_LLAMA_CPP_PATH"] = str(_llama_dir)
...
# __file__ is tunelabs_cli/commands/studio.py -- two parents up is the package root
...
def _studio_venv_python() -> Optional[Path]:
    ...
    if platform.system() == "Windows":
        p = STUDIO_HOME / "tunelabs_studio" / "Scripts" / "python.exe"
    else:
        p = STUDIO_HOME / "tunelabs_studio" / "bin" / "python"
...
def _find_run_py() -> Optional[Path]:
    ...
    site-packages after pip install, right next to tunelabs_cli/.
    """
    ...
    for pattern in (
        ...
        for match in (STUDIO_HOME / "tunelabs_studio").glob(pattern):
            return match
    return None
...
def _load_run_module():
    ...
    run_py = _find_run_py()
    if run_py is None:
        raise ImportError("Could not find studio/backend/run.py. Re-run: tunelabs studio setup")
...
_PARALLEL_MIN = 1
_PARALLEL_MAX = 64
_PARALLEL_DEFAULT_RUN = 4  # pre-PR hardcoded for `tunelabs studio run`
_PARALLEL_DEFAULT_PLAIN = 1  # pre-PR effective for plain `tunelabs studio`
...
def _find_frontend_dist() -> Optional[Path]:
    ...
    Fixes the silent 404 when another `tunelabs` on PATH shadows the
...
# ── helpers for `tunelabs studio run` ────────────────────────────────
...
def _create_api_key_inprocess(name: str) -> str:
    ...
    CLI already has filesystem access to ``~/.tunelabs/studio``.
    """
...
def studio_default(
    ...
    parallel: int = typer.Option(
        ...
        help = (
            ...
            f"Default {_PARALLEL_DEFAULT_PLAIN}; `tunelabs studio run` "
    ...
    """Launch the TuneLabs Studio server."""
    ...
    if ctx.invoked_subcommand is not None:
        ...
        # `tunelabs studio --parallel N run ...` would silently drop N.
        if parallel != _PARALLEL_DEFAULT_PLAIN:
            typer.echo(
                f"Error: --parallel on `tunelabs studio` applies to the "
                f"plain-server path only. For `tunelabs studio "
                f"{ctx.invoked_subcommand}`, put the flag after the "
                f"subcommand: `tunelabs studio {ctx.invoked_subcommand} "
                f"--parallel {parallel} ...`",
                err = True,
            )
            raise typer.Exit(2)
        ...
        if not cloudflare:
            typer.echo(
                f"Error: --no-cloudflare on `tunelabs studio` applies to the "
                f"plain-server path only. For `tunelabs studio "
                f"{ctx.invoked_subcommand}`, put it after the subcommand: "
                f"`tunelabs studio {ctx.invoked_subcommand} --no-cloudflare ...`",
                err = True,
            )
            raise typer.Exit(2)
        ...
        if secure:
            typer.echo(
                f"Error: --secure on `tunelabs studio` applies to the "
                f"plain-server path only. For `tunelabs studio "
                f"{ctx.invoked_subcommand}`, put it after the subcommand: "
                f"`tunelabs studio {ctx.invoked_subcommand} --secure ...`",
                err = True,
            )
            raise typer.Exit(2)
        ...
        if verbose:
            typer.echo(
                f"Error: --verbose on `tunelabs studio` applies to the "
                f"plain-server path only. For `tunelabs studio "
                f"{ctx.invoked_subcommand}`, put it after the subcommand: "
                f"`tunelabs studio {ctx.invoked_subcommand} --verbose ...`",
                err = True,
            )
            raise typer.Exit(2)
        ...
        if enable_tools is not None:
            ...
            typer.echo(
                f"Error: {_tool_flag} on `tunelabs studio` applies to the "
                f"plain-server path only. For `tunelabs studio "
                f"{ctx.invoked_subcommand}`, put it after the subcommand: "
                f"`tunelabs studio {ctx.invoked_subcommand} {_tool_flag} ...`",
                err = True,
            )
            raise typer.Exit(2)
        return
    ...
    studio_venv_dir = STUDIO_HOME / "tunelabs_studio"
    ...
    if not in_studio_venv:
        ...
        if studio_python and run_py:
            if not silent:
                typer.echo("Launching TuneLabs Studio... Please wait...")
            args = [
                str(studio_python),
                str(run_py),
                "--host",
    ...
    if not silent:
        ...
        typer.echo(f"Starting TuneLabs Studio on http://{display_host}:{port}")
...
def run(
    ...
    Example:
        ...
        tunelabs studio run --model tunelabs/Qwen3-1.7B-GGUF --top-k 20 --seed 42 --parallel 8
        tunelabs studio run --model some-model --chat-template-file /path/to/tpl.jinja
        tunelabs studio run --model tunelabs/Qwen3-27B-GGUF --gguf-variant Q8_0 --tensor-parallel
    """
    ...
    from tunelabs_cli._tool_policy import is_external_host, resolve_tool_policy
    ...
    if not in_studio_venv:
        ...
        if not studio_python:
            typer.echo("Studio not set up. Run install.sh first.")
            raise typer.Exit(1)
        # Re-exec via the studio venv's `tunelabs` console-script.
        studio_bin = studio_python.parent / "tunelabs"
        if not studio_bin.is_file():
            typer.echo("Studio venv missing 'tunelabs' entry point. Re-run: tunelabs studio setup")
            raise typer.Exit(1)
        args = [
            str(studio_bin),
            "studio",
    ...
    try:
        ...
        if not silent:
            typer.echo("Starting TuneLabs Studio...")
    ...
    if not silent:
        ...
        typer.echo("=" * 56)
        if secure and _cf_url:
            typer.echo(f"  TuneLabs Studio running (secure) at {_cf_url}")
            typer.echo(f"  On this machine only: {base_url}")
        else:
            typer.echo(f"  TuneLabs Studio running at {base_url}")
            if _cf_url:
...
@studio_app.command()
def stop():
    """Stop a running TuneLabs Studio server.

    Reads the PID from ~/.tunelabs/studio/studio.pid and sends SIGTERM
...
def _run_setup_script(*, verbose: bool = False) -> None:
    ...
    env = {**os.environ, "TUNELABS_VERBOSE": "1"} if verbose else None
    ...
    if platform.system() == "Windows":
        ...
        # stdout is a pipe, e.g. `tunelabs studio update --local 2>&1 | tee`.
...
_INSTALLER_URL_BASH = "https://tunelabs.ai/install.sh"
_INSTALLER_URL_PWSH = "https://tunelabs.ai/install.ps1"
    ...
    if is_windows:
        ...
        try:
            request = urllib.request.Request(
                installer_url, headers = {"User-Agent": "tunelabs-studio-update"}
            )
        ...
        # install.ps1 auto-invokes `Install-TuneLabsStudio @args` at EOF; over
        # stdin `$args` is empty so that triggers the full installer flow
        # (deps, venv, prompts) before our shortcuts-only call. Strip it.
        installer = re.sub(
            r"(?m)^[ \t]*Install-TuneLabsStudio[ \t]+@args[ \t]*\r?\n?",
            "",
            installer,
        )
        # stdin-piped scripts have empty $args, so call Install-TuneLabsStudio explicitly.
        marker_args = " ".join(args)
        wrapper = installer + f"\nInstall-TuneLabsStudio {marker_args}\n"
        ...
        ps1_fd, ps1_path = tempfile.mkstemp(
            prefix = "tunelabs-studio-refresh-",
            suffix = ".ps1",
        )
        try:
...
def update(
    ...
    package: str = typer.Option(
        "tunelabs", "--package", help = "Package name to install/update (for testing)"
    ),
    verbose: bool = typer.Option(
        False,
        "--verbose",
    ...
    """Update TuneLabs Studio dependencies and rebuild."""
    # Re-export TUNELABS_STUDIO_HOME for env-mode installs so the refresh
    ...
    try:
        ...
        # Restore tunelabs.exe from .deleteme if setup failed before pip
    ...
    # tunelabs.exe; on next update os.replace would overwrite it anyway,
    ...
    if os.environ.get("TUNELABS_TAURI_UPDATE") == "1":
        if verbose:
...
def _release_self_exe_lock_windows() -> None:
    """Rename running tunelabs.exe so pip can replace it. setup.ps1 also retries."""
    if platform.system() != "Windows":
        return
    try:
...
def _restore_self_exe_lock_windows() -> None:
    """If setup failed before pip wrote a working tunelabs.exe, restore .deleteme."""
    if platform.system() != "Windows":
        return
    try:
        venv_scripts = Path(sys.executable).resolve().parent
...
def _cleanup_self_exe_lock_windows() -> None:
    ...
    try:
        ...
        return
    stale = (venv_scripts / "tunelabs.exe").with_suffix(".exe.deleteme")
    try:
...
# ── tunelabs studio reset-password ────────────────────────────────────
...
def desktop_capabilities(
    ...
    try:
        ...
        payload["version"] = package_version("tunelabs")
    except Exception:
        pass
...
def reset_password():
    ...
    typer.echo("Auth database deleted. Restart TuneLabs Studio to get a new password.")
