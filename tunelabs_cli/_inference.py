...
class ChatBackend:
    """Uniform stream()/close() over the llama-server and TuneLabs backends."""

    def __init__(self, kind: str, backend) -> None:
        self._kind = kind  # "gguf" | "tunelabs"
...
def load_chat_backend(
    ...
    *,
    ...
    if not backend.load_model(
        ...
        typer.echo("Model load failed", err = True)
        raise typer.Exit(code = 1)
    return ChatBackend("tunelabs", backend)


def _loopback_candidate_bases(base: str) -> list:
    """For a bare ``localhost`` base, the concrete IP bases to try, IPv4
    127.0.0.1 first (where ``tunelabs studio`` binds by default). Pinning to one
...
def find_studio_server(timeout: float = 3.0) -> Optional[str]:
    ...
    base = os.environ.get("TUNELABS_STUDIO_URL", "http://127.0.0.1:8888").rstrip("/")
...
def connect_studio_server(model: str, *, hf_token, max_seq_length, load_in_4bit):
    ...
    # Explicit server (TUNELABS_STUDIO_URL) we can't safely attach to -> fail loudly;
    # opportunistic local discovery just falls back to a local load.
    explicit = bool(os.environ.get("TUNELABS_STUDIO_URL"))
    ...
    def _refuse(reason: str):
        ...
        typer.echo(
            ...
            "on this machine, or unset TUNELABS_STUDIO_URL to load the model locally.",
            err = True,
        )
        raise typer.Exit(code = 1)
