...
"""Tests for the `tunelabs chat` / `tunelabs inference` CLI — fakes only, no model loads."""
...
import tunelabs_cli.commands.chat as chatmod
...
def test_chatbackend_routes_compare_to_adapter_control():
    fake = _FakeBackend()
    backend = ChatBackend("tunelabs", fake)
...
def test_find_studio_server_none_when_not_running(monkeypatch):
    ...
    from tunelabs_cli import _inference
...
def test_find_studio_server_prefers_ipv4_loopback_for_localhost(monkeypatch):
    ...
    monkeypatch.setenv("TUNELABS_STUDIO_URL", "http://localhost:8888")
    monkeypatch.setattr(
        socket,
        "getaddrinfo",
        lambda *a, **k: [
