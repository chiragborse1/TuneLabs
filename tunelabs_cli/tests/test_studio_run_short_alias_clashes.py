...
`tunelabs studio run` passes unknown flags through to llama-server.
...
def _studio_mod():
    from tunelabs_cli.commands import studio as _s
    return _s
...
def _install_capture(monkeypatch):
    ...
    fake_bin = Path("/fake/studio/venv/tunelabs_studio/bin/tunelabs")
    ...
    from tunelabs_cli import _tool_policy as _tp
...
def test_dash_hf_documented_alias_still_works(monkeypatch):
    ...
    captured = _invoke(
        monkeypatch,
        ["-hf", "tunelabs/gemma-4-26B-A4B-it-GGUF:UD-Q4_K_XL"],
    )
    assert len(captured) == 1
    argv = captured[0]
    # `_split_repo_variant` peels the `:variant` suffix before re-exec.
    assert argv[argv.index("--model") + 1] == ("tunelabs/gemma-4-26B-A4B-it-GGUF"), argv
...
@pytest.mark.parametrize(
    ...
    [
        (["-m", "tunelabs/Qwen3-1.7B-GGUF"], "tunelabs/Qwen3-1.7B-GGUF"),
        (["-m=tunelabs/Qwen3-1.7B-GGUF"], "tunelabs/Qwen3-1.7B-GGUF"),
        (["-hfr", "tunelabs/Qwen3-1.7B-GGUF"], "tunelabs/Qwen3-1.7B-GGUF"),
        (["-hfr=tunelabs/Qwen3-1.7B-GGUF"], "tunelabs/Qwen3-1.7B-GGUF"),
    ],
)
...
def test_legacy_m_with_repo_variant_syntax(monkeypatch):
    ...
    captured = _invoke(
        monkeypatch,
        ["-m", "tunelabs/Qwen3-1.7B-GGUF:UD-Q4_K_XL"],
    )
    assert len(captured) == 1
    argv = captured[0]
    assert argv[argv.index("--model") + 1] == "tunelabs/Qwen3-1.7B-GGUF", argv
    assert argv[argv.index("--gguf-variant") + 1] == "UD-Q4_K_XL", argv
...
def test_legacy_m_inline_value_form(monkeypatch):
    ...
    captured = _invoke(monkeypatch, ["-m=tunelabs/Qwen3-1.7B-GGUF"])
    assert len(captured) == 1
    argv = captured[0]
    assert argv[argv.index("--model") + 1] == "tunelabs/Qwen3-1.7B-GGUF", argv
...
def test_expand_np_rewrites_attached_form(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        ["tunelabs", "studio", "run", "--model", "X", "-np8"],
    )
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "studio", "run", "--model", "X", "-np", "8"]


@pytest.mark.parametrize("value", ["1", "8", "64", "999"])
def test_expand_np_rewrites_all_digit_values(monkeypatch, value):
    monkeypatch.setattr(sys, "argv", ["tunelabs", "studio", "run", f"-np{value}"])
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "studio", "run", "-np", value]


def test_expand_np_leaves_space_form_alone(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["tunelabs", "run", "-np", "8"])
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "run", "-np", "8"]
...
def test_expand_np_leaves_non_digit_suffix_alone(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["tunelabs", "run", "-npfoo"])
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "run", "-npfoo"]


def test_expand_np_leaves_bare_np_alone(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["tunelabs", "run", "-np"])
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "run", "-np"]
...
def test_expand_np_handles_multiple_occurrences(monkeypatch):
    monkeypatch.setattr(
        sys,
        "argv",
        ["tunelabs", "run", "-np8", "-np16"],
    )
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "run", "-np", "8", "-np", "16"]
...
def test_expand_np_handles_signed_attached_forms(monkeypatch, attached, expected):
    ...
    monkeypatch.setattr(sys, "argv", ["tunelabs", "run", attached])
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "run", "-np", expected]
...
def test_expand_np_rewrites_numeric_prefix_even_with_junk(monkeypatch, attached, expected_suffix):
    ...
    monkeypatch.setattr(sys, "argv", ["tunelabs", "run", attached])
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "run", "-np", expected_suffix]
...
# Gate isolation: importing tunelabs_cli from a third-party script must
...
def test_third_party_importers_do_not_trigger_np_rewrite(monkeypatch, third_party_argv0):
    """Only the `tunelabs` / `tunelabs.exe` console-script may run the
    ...
    monkeypatch.delitem(sys.modules, "tunelabs_cli", raising = False)
    importlib.import_module("tunelabs_cli")
    assert sys.argv == starting_argv, (
        f"third-party argv[0]={third_party_argv0!r} triggered the "
        f"tunelabs -np canonicaliser; sys.argv was mutated to {sys.argv}"
    )
...
def test_attached_np8_no_longer_silently_sets_port(monkeypatch):
    ...
    _studio_mod()._expand_attached_np_short()
    captured = _invoke(monkeypatch, sys.argv[2:])  # drop "tunelabs studio"
...
def test_expand_np_stops_at_double_dash(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["tunelabs", "run", "--model", "X", "--", "-np8"],
    )
    _studio_mod()._expand_attached_np_short()
    assert sys.argv == ["tunelabs", "run", "--model", "X", "--", "-np8"]
