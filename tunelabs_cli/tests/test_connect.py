...
"""Tests for `tunelabs connect` — config merging and launch env, no network."""
...
import tunelabs_cli.commands.connect as connect

BASE = "http://127.0.0.1:8888"
MODEL = {"id": "tunelabs/gemma-4-26B-A4B-it-GGUF", "context_length": 131072}
...
def test_merge_codex_config_fresh():
    ...
    assert parsed["oss_provider"] == "tunelabs_api"
    provider = parsed["model_providers"]["tunelabs_api"]
...
def test_merge_codex_config_replaces_stale_block():
    existing = (
        'model = "gpt-5"\n'
        "\n"
        "[model_providers.tunelabs_api]\n"
        'base_url = "http://old-host:9999/v1"\n'
        'wire_api = "chat"\n'
        "\n"
        "[model_providers.tunelabs_api.http_headers]\n"
        'x-old = "1"\n'
        "\n"
    ...
    assert parsed["model_providers"]["tunelabs_api"]["base_url"] == f"{BASE}/v1"
    assert parsed["model_providers"]["tunelabs_api"]["wire_api"] == "responses"
    assert "http_headers" not in parsed["model_providers"]["tunelabs_api"]
...
def test_write_codex_config_profile(tmp_path, monkeypatch):
    ...
    profile = _parse_toml((tmp_path / "tunelabs_api.config.toml").read_text())
    assert profile["oss_provider"] == "tunelabs_api"
    assert profile["model_provider"] == "tunelabs_api"
    assert profile["model"] == MODEL["id"]
    assert profile["model_context_window"] == 131072
    config = _parse_toml((tmp_path / "config.toml").read_text())
    assert config["model_providers"]["tunelabs_api"]["env_key"] == "TUNELABS_STUDIO_AUTH_TOKEN"
...
def fake_studio(tmp_path, monkeypatch, claude_settings):
    ...
    def http_json(
        ...
        if url.endswith("/api/auth/api-keys"):
            return {"key": "sk-tunelabs-feedfacefeedface"}
    ...
    monkeypatch.delenv("TUNELABS_API_KEY", raising = False)
    return calls
...
def test_connect_claude_no_launch(fake_studio, claude_settings):
    ...
    _assert_env_set(result.output, "ANTHROPIC_AUTH_TOKEN", "sk-tunelabs-feedfacefeedface")
...
def test_connect_claude_launch_scrubs_conflicting_auth_env(fake_studio, monkeypatch):
    ...
    assert captured["env"]["ANTHROPIC_AUTH_TOKEN"] == "sk-tunelabs-feedfacefeedface"
...
def test_connect_codex_no_launch(fake_studio, tmp_path):
    ...
    assert result.exit_code == 0, result.output
    _assert_env_set(result.output, "TUNELABS_STUDIO_AUTH_TOKEN", "sk-tunelabs-feedfacefeedface")
    assert "codex --oss --profile tunelabs_api" in result.output
    assert (tmp_path / "codex" / "config.toml").exists()
    assert (tmp_path / "codex" / "tunelabs_api.config.toml").exists()
...
def test_connect_key_minted_once_then_cached(fake_studio, tmp_path):
    ...
    assert cached["servers"][BASE]["minted"] == ["sk-tunelabs-feedfacefeedface"]


def test_connect_explicit_key_remembered_for_keyless_runs(fake_studio, tmp_path):
    CliRunner().invoke(
        connect.connect_app,
        ["claude", "--no-launch", "--api-key", "sk-tunelabs-deadbeefdeadbeef"],
    )
    result = CliRunner().invoke(connect.connect_app, ["claude", "--no-launch"])
    assert result.exit_code == 0, result.output
    # Reused, not re-minted (a mint would return the feedface stand-in).
    _assert_env_set(result.output, "ANTHROPIC_AUTH_TOKEN", "sk-tunelabs-deadbeefdeadbeef")
    cached = json.loads((tmp_path / "agent_api_key.json").read_text())
    # An explicit key is remembered as "saved" so it replays without the handshake.
    assert cached["servers"][BASE]["saved"] == ["sk-tunelabs-deadbeefdeadbeef"]
...
def test_connect_skips_cached_keys_the_server_rejects(fake_studio, tmp_path, monkeypatch):
    ...
    cache.write_text(
        json.dumps(
            {"servers": {BASE: {"minted": ["sk-tunelabs-stale", "sk-tunelabs-feedfacefeedface"]}}}
        )
    )
    ...
    def http_json(
        ...
        if url.endswith("/v1/models") and token == "sk-tunelabs-stale":
    ...
    assert cached["servers"][BASE]["minted"] == ["sk-tunelabs-feedfacefeedface", "sk-tunelabs-stale"]
...
def test_connect_legacy_unscoped_cache_not_replayed(fake_studio, tmp_path):
    ...
    (tmp_path / "agent_api_key.json").write_text(json.dumps({"key": "sk-tunelabs-oldformat"}))
    result = CliRunner().invoke(connect.connect_app, ["claude", "--no-launch"])
    assert result.exit_code == 0, result.output
    _assert_env_set(result.output, "ANTHROPIC_AUTH_TOKEN", "sk-tunelabs-feedfacefeedface")
    cached = json.loads((tmp_path / "agent_api_key.json").read_text())
...
def test_connect_model_flag_loads_on_server(fake_studio):
    result = CliRunner().invoke(
        connect.connect_app, ["claude", "--no-launch", "--model", "tunelabs/Qwen3.5-35B-A3B"]
    )
    assert result.exit_code == 0, result.output
    loads = [c for c in fake_studio if c[1].endswith("/api/inference/load")]
    assert loads == [
        ("POST", f"{BASE}/api/inference/load", {"model_path": "tunelabs/Qwen3.5-35B-A3B"})
    ]
    _assert_env_set(result.output, "ANTHROPIC_MODEL", "tunelabs/Qwen3.5-35B-A3B")
...
def test_connect_model_flag_matches_canonical_id(fake_studio, monkeypatch):
    ...
    requested = "TuneLabs/Qwen3.5-35B-A3B"
    canonical = "tunelabs/Qwen3.5-35B-A3B"
    inner = connect._http_json
...
def test_connect_no_model_loaded_errors(fake_studio, monkeypatch):
    monkeypatch.setattr(
        ...
        lambda method, url, token, payload = None, timeout = 30, error = None: (
            {"key": "sk-tunelabs-feedfacefeedface"}
            if url.endswith("/api/auth/api-keys")
...
def test_connect_requested_model_not_loaded_fails(fake_studio, monkeypatch):
    ...
    monkeypatch.setattr(connect, "_http_json", http_json)
    result = CliRunner().invoke(
        connect.connect_app, ["claude", "--no-launch", "--model", "tunelabs/Missing-7B"]
    )
    assert result.exit_code == 1
    assert "tunelabs/Missing-7B" in result.output
...
def test_connect_codex_rejects_non_gguf_model(fake_studio, monkeypatch):
    ...
    def http_json(
        ...
        if url.endswith("/api/inference/status"):
            return {"is_gguf": False, "model_identifier": "tunelabs/Qwen3-0.6B"}
        return inner(method, url, token, payload, timeout, error)
...
def test_connect_nonloopback_explicit_key_is_allowed(fake_studio, monkeypatch):
    ...
    result = CliRunner().invoke(
        connect.connect_app,
        ["opencode", "--no-launch", "--api-key", "sk-tunelabs-deadbeefdeadbeef"],
    )
    assert result.exit_code == 0, result.output
...
def test_connect_nonloopback_replays_saved_key(fake_studio, tmp_path, monkeypatch):
    ...
    (tmp_path / "agent_api_key.json").write_text(
        json.dumps({"servers": {remote: {"saved": ["sk-tunelabs-deadbeefdeadbeef"]}}})
    )
    result = CliRunner().invoke(connect.connect_app, ["claude", "--no-launch"])
    assert result.exit_code == 0, result.output
    _assert_env_set(result.output, "ANTHROPIC_AUTH_TOKEN", "sk-tunelabs-deadbeefdeadbeef")
...
def test_connect_studio_server_errors_on_explicit_remote(monkeypatch):
    # A user who pointed TUNELABS_STUDIO_URL at a remote Studio should get an
    # error, not a silent local model load (which they did not ask for).
    import typer

    import tunelabs_cli._inference as inference

    monkeypatch.setenv("TUNELABS_STUDIO_URL", "http://studio.example:8888")
    monkeypatch.setattr(
...
def test_connect_studio_server_falls_back_locally_on_default_discovery(monkeypatch):
    # Opportunistic local discovery (no TUNELABS_STUDIO_URL): if the loopback
    # server can't be verified, fall back to a local load rather than erroring.
    import tunelabs_cli._inference as inference

    monkeypatch.delenv("TUNELABS_STUDIO_URL", raising = False)
...
def test_connect_replays_saved_key_without_identity_check(fake_studio, tmp_path, monkeypatch):
    ...
    cache = tmp_path / "agent_api_key.json"
    cache.write_text(json.dumps({"servers": {BASE: {"saved": ["sk-tunelabs-deadbeefdeadbeef"]}}}))
    monkeypatch.setattr(connect, "verify_studio_identity", lambda base: False)
    result = CliRunner().invoke(connect.connect_app, ["claude", "--no-launch"])
    assert result.exit_code == 0, result.output
    _assert_env_set(result.output, "ANTHROPIC_AUTH_TOKEN", "sk-tunelabs-deadbeefdeadbeef")
...
def test_connect_minted_cache_requires_identity_check(fake_studio, tmp_path, monkeypatch):
    ...
    cache = tmp_path / "agent_api_key.json"
    cache.write_text(json.dumps({"servers": {BASE: {"minted": ["sk-tunelabs-feedfacefeedface"]}}}))
    monkeypatch.setattr(connect, "verify_studio_identity", lambda base: False)
    result = CliRunner().invoke(connect.connect_app, ["claude", "--no-launch"])
    assert result.exit_code == 1
    assert "--api-key" in result.output
...
def test_write_openclaw_config_fresh(tmp_path, monkeypatch):
    ...
    connect.write_openclaw_config(BASE, "sk-tunelabs-abc", MODEL)
    config = json.loads(path.read_text())
    provider = config["models"]["providers"]["tunelabs"]
    assert provider["baseUrl"] == f"{BASE}/v1"
    assert provider["apiKey"] == "sk-tunelabs-abc"
    ...
    assert config["agents"]["defaults"]["model"]["primary"] == f"tunelabs/{MODEL['id']}"
...
def test_write_openclaw_config_preserves_and_idempotent(tmp_path, monkeypatch):
    ...
    assert config["models"]["providers"]["tunelabs"]["baseUrl"] == f"{BASE}/v1"
...
def test_connect_openclaw_no_launch(fake_studio, tmp_path, monkeypatch):
    ...
    config = json.loads(path.read_text())
    assert config["models"]["providers"]["tunelabs"]["apiKey"] == "sk-tunelabs-feedfacefeedface"
    assert config["agents"]["defaults"]["model"]["primary"] == f"tunelabs/{MODEL['id']}"
...
def test_write_opencode_config_fresh(tmp_path, monkeypatch):
    ...
    connect.write_opencode_config(BASE, "sk-tunelabs-abc", MODEL)
    config = json.loads(path.read_text())
    provider = config["provider"]["tunelabs"]
    assert provider["npm"] == "@ai-sdk/openai-compatible"
    assert provider["options"] == {"baseURL": f"{BASE}/v1", "apiKey": "sk-tunelabs-abc"}
    assert provider["models"] == {MODEL["id"]: {"name": MODEL["id"]}}
    assert config["model"] == f"tunelabs/{MODEL['id']}"
...
def test_write_opencode_config_preserves_and_idempotent(tmp_path, monkeypatch):
    ...
    assert config["provider"]["tunelabs"]["options"]["baseURL"] == f"{BASE}/v1"
    before = path.read_text()
    connect.write_opencode_config(BASE, "sk-tunelabs-abc", MODEL)
    assert path.read_text() == before
...
def test_connect_opencode_no_launch(fake_studio, tmp_path, monkeypatch):
    ...
    config = json.loads(path.read_text())
    assert config["provider"]["tunelabs"]["options"]["apiKey"] == "sk-tunelabs-feedfacefeedface"
    assert config["model"] == f"tunelabs/{MODEL['id']}"
    assert not any(c[1].endswith("/api/inference/status") for c in fake_studio)
...
def test_write_hermes_config_fresh(hermes_config):
    ...
    assert config["model"]["provider"] == "custom:tunelabs"
    assert config["model"]["default"] == MODEL["id"]
    assert config["model"]["api_mode"] == "openai"
    provider = config["providers"]["tunelabs"]
    assert provider["base_url"] == f"{BASE}/v1"
    assert provider["api_mode"] == "openai"
    assert provider["key_env"] == "TUNELABS_API_KEY"
    # The key is resolved from the launch env, never written to disk.
    assert "sk-tunelabs" not in hermes_config.read_text()
...
def test_write_hermes_config_preserves_and_idempotent(hermes_config):
    ...
    assert config["providers"]["tunelabs"]["base_url"] == f"{BASE}/v1"
...
def test_connect_hermes_no_launch(fake_studio, hermes_config):
    ...
    assert result.exit_code == 0, result.output
    _assert_env_set(result.output, "TUNELABS_API_KEY", "sk-tunelabs-feedfacefeedface")
