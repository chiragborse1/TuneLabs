...
Pins the typer Option (default on) on both `tunelabs studio` and
`tunelabs studio run`, and that the chosen polarity reaches the re-exec'd
...
def _studio():
    from tunelabs_cli.commands import studio as _studio_mod
    return _studio_mod


_BASE = ["--model", "tunelabs/Qwen3-1.7B-GGUF"]
...
# ── re-exec forwarding: `tunelabs studio run` ─────────────────────────
...
def _install_run_reexec_capture(monkeypatch, *, platform = "linux"):
    ...
    fake_venv = Path("/fake/studio/venv/tunelabs_studio")
    monkeypatch.setattr(studio_mod, "_studio_venv_python", lambda: fake_venv / "bin" / "python")
    fake_bin = fake_venv / "bin" / "tunelabs"
    ...
    from tunelabs_cli import _tool_policy as _tp_mod
...
# ── re-exec forwarding: plain `tunelabs studio` ───────────────────────
...
def test_studio_default_rejects_no_cloudflare_with_subcommand(monkeypatch):
    # `tunelabs studio --no-cloudflare run ...` would not reach the subcommand,
