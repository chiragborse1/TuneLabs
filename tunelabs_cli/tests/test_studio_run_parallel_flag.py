...
"""Tests for the `tunelabs studio run --parallel` CLI flag.
...
def _load_run_command():
    ...
    are lazy inside run()."""
    from tunelabs_cli.commands import studio as _studio
    return _studio
...
def test_parallel_default_is_four():
    """Default must stay at 4 so plain `tunelabs studio run` is unchanged."""
    studio_mod = _load_run_command()
    import inspect
...
def test_studio_default_rejects_parallel_when_subcommand_invoked():
    """`tunelabs studio --parallel 8 run ...` would silently drop the 8
...
def test_studio_default_exposes_parallel_option():
    """Plain `tunelabs studio` exposes --parallel too so the API-only
