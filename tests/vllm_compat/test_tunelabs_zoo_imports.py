...
"""CPU-only smoke imports for tunelabs_zoo modules touching vLLM / GRPO +
...
Cross-references (tunelabs_zoo commits that fixed bugs surfaced here):
  ...
  fa82dcc2 / 664e52ea (TUNELABS_VLLM_STANDBY hard-error on vLLM 0.10/0.14).
"""
...
# Apply the consolidated CPU spoof at import time, before any tunelabs import.
...
# Some tunelabs_zoo modules read pynvml at import; stub it for the runner.
...
def test_rl_replacements_imports_without_vllm():
    """tunelabs_zoo.rl_replacements must NOT pull in vllm at import time."""
    sys.modules.pop("tunelabs_zoo.rl_replacements", None)
    rl = importlib.import_module("tunelabs_zoo.rl_replacements")
    # A transitive vllm import crashes GRPOTrainer construction on Colab.
    assert "vllm" not in sys.modules, (
        "tunelabs_zoo.rl_replacements imported vllm transitively; this breaks "
...
def test_empty_model_imports_without_vllm():
    sys.modules.pop("tunelabs_zoo.empty_model", None)
    em = importlib.import_module("tunelabs_zoo.empty_model")
    assert (
        "vllm" not in sys.modules
    ), "tunelabs_zoo.empty_model imported vllm transitively; expected to be vllm-free"
    assert (
...
@pytest.mark.skipif(
    not (_has_tunelabs_zoo() and _has_vllm()), reason = "vllm not installed on this runner"
)
def test_vllm_lora_request_imports():
    sys.modules.pop("tunelabs_zoo.vllm_lora_request", None)
    importlib.import_module("tunelabs_zoo.vllm_lora_request")
...
def test_vllm_lora_worker_manager_imports():
    sys.modules.pop("tunelabs_zoo.vllm_lora_worker_manager", None)
    mod = importlib.import_module("tunelabs_zoo.vllm_lora_worker_manager")
...
def test_vllm_utils_imports():
    sys.modules.pop("tunelabs_zoo.vllm_utils", None)
    mod = importlib.import_module("tunelabs_zoo.vllm_utils")
    assert callable(
        getattr(mod, "patch_vllm", None)
    ), "tunelabs_zoo.vllm_utils must expose patch_vllm()"
