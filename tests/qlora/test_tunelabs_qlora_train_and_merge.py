...
def get_tunelabs_model_and_tokenizer(
    model_name: str,
    max_seq_length: int,
    load_in_4bit: bool,
    fast_inference: bool,
...
if __name__ == "__main__":
    ...
    tunelabs_merged_path = "tunelabs_merged_16bit"

    model, tokenizer = get_tunelabs_model_and_tokenizer(
        model_name,
        max_seq_length = 512,
        load_in_4bit = True,
    ...
    model.save_pretrained_merged(
        tunelabs_merged_path,
        tokenizer,
        save_method = "merged_16bit",
    )
    merged_model_tunelabs, tokenizer = get_tunelabs_model_and_tokenizer(
        tunelabs_merged_path,
        max_seq_length = 512,
        load_in_4bit = False,
        fast_inference = False,
    ...
    responses = sample_responses(
        merged_model_tunelabs,
        tokenizer,
        prompt = prompt,
        **generation_args,
    )
    with header_footer_context("Responses after tunelabs merge to 16bit"):
        check_responses(responses, answer = ANSWER, prompt = prompt)
