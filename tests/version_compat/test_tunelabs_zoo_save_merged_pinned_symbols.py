...
"""Pinned-symbol canary for tunelabs-zoo save_pretrained_merged guards
(tunelabsai/tunelabs-zoo#647 / tunelabsai/tunelabs#5410). Skips until #647
...
def _fetch_saving_utils() -> str:
    src = fetch_text("tunelabsai/tunelabs-zoo", ZOO_TAG, "tunelabs_zoo/saving_utils.py")
    if src is None:
        pytest.skip("tunelabs_zoo/saving_utils.py not fetchable")
    return src


def _fetch_merge_tests() -> str:
    src = fetch_text(
        "tunelabsai/tunelabs-zoo",
        ZOO_TAG,
        "tests/test_tunelabs_zoo_lora_merge.py",
    )
    if src is None:
        pytest.skip("tests/test_tunelabs_zoo_lora_merge.py not fetchable")
    return src
...
def _skip_until_pr_647_lands(src: str) -> None:
    if not any(
        ...
        pytest.skip(
            "tunelabsai/tunelabs-zoo#647 has not yet merged into main; "
...
def test_zoo_lora_merge_tests_have_standard_layout_coverage():
    ...
    if "test_merge_moe_gate_expert_standard_layout" not in src:
        pytest.skip("tunelabsai/tunelabs-zoo#647 not yet merged; coverage appears later.")
    for name in (
...
def test_tunelabs_save_pretrained_merged_entry_point_exists():
    import pathlib

    save_py = pathlib.Path(__file__).resolve().parents[2] / "tunelabs" / "save.py"
    if not save_py.is_file():
        pytest.skip(f"{save_py} not present")
    text = save_py.read_text(encoding = "utf-8", errors = "replace")
    assert "save_pretrained_merged" in text, "entry point removed from tunelabs/save.py."
    assert "merge_and_overwrite_lora" in text, "no dispatch into tunelabs_zoo merge; #647 bypassed."
