...
external server (``TUNELABS_E2E_BASE_URL``/``TUNELABS_E2E_API_KEY``) for fast
iteration, or a fixture-managed server started/torn down per session for CI.
Model/variant for the managed mode resolve from ``--tunelabs-model`` /
``--tunelabs-gguf-variant``, then env vars, then ``test_studio_api.py`` defaults.
"""
...
def pytest_addoption(parser):
    group = parser.getgroup(
        "tunelabs-e2e",
        "TuneLabs Studio end-to-end test options",
    )
    group.addoption(
        "--tunelabs-model",
        action = "store",
        ...
        help = (
            ...
            "Ignored if TUNELABS_E2E_BASE_URL is set. Overrides "
            "TUNELABS_E2E_MODEL env var. Defaults to test_studio_api.py's "
            "DEFAULT_MODEL."
        ),
    )
    group.addoption(
        "--tunelabs-gguf-variant",
        action = "store",
        default = None,
        help = (
            "GGUF variant used when starting a server for e2e tests. "
            "Ignored if TUNELABS_E2E_BASE_URL is set. Overrides "
            "TUNELABS_E2E_VARIANT env var. Defaults to test_studio_api.py's "
            "DEFAULT_VARIANT."
        ),
    )
...
def studio_server(request):
    ...
    Uses ``TUNELABS_E2E_BASE_URL`` (requires ``TUNELABS_E2E_API_KEY``) if set,
    ...
    """
    external_url = os.environ.get("TUNELABS_E2E_BASE_URL")
    if external_url:
        api_key = os.environ.get("TUNELABS_E2E_API_KEY")
        if not api_key:
            pytest.skip(
                "TUNELABS_E2E_BASE_URL is set but TUNELABS_E2E_API_KEY is "
    ...
    model = (
        request.config.getoption("--tunelabs-model")
        or os.environ.get("TUNELABS_E2E_MODEL")
        or _e2e.DEFAULT_MODEL
    )
    variant = (
        request.config.getoption("--tunelabs-gguf-variant")
        or os.environ.get("TUNELABS_E2E_VARIANT")
...
def rag_home(tmp_path, monkeypatch):
    """Isolate the RAG database under a fresh TUNELABS_STUDIO_HOME per test.
