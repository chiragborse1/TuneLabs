...
shadow dirs (a `tunelabs`/`tunelabs_zoo` dir with no __init__.py on sys.path)
before `from tunelabs import ...`. Order matters: `tunelabs.__init__` runs its
ROCm/Windows bnb fixes before importing tunelabs_zoo, so the guard must import
tunelabs first. Each test runs the real guard (ast-extracted from source, no
...
# ── fake package bodies ──────────────────────────────────────────────
# Real `tunelabs` sets a sentinel (mimics _gpu_init's pre-zoo fixes) then
# imports tunelabs_zoo, which records whether that sentinel was set first.

_REAL_TUNELABS_INIT = textwrap.dedent(
    """
    import os
    with open(os.environ["GUARD_ORDER_FILE"], "a") as _f:
        _f.write("tunelabs\\n")
    # mimic _gpu_init: pre-zoo ROCm/Windows fixes run before importing zoo
    os.environ["TUNELABS_GPU_INIT_RAN"] = "1"
    import tunelabs_zoo  # noqa: F401
    REAL = True
    """
)
...
_REAL_ZOO_INIT = textwrap.dedent(
    ...
    import os
    with open(os.environ["GUARD_ORDER_FILE"], "a") as _f:
        _f.write("tunelabs_zoo\\n")
    with open(os.environ["GUARD_SENTINEL_FILE"], "w") as _f:
        _f.write(os.environ.get("TUNELABS_GPU_INIT_RAN", "0"))
    REAL = True
    """
)
...
_DRIVER = textwrap.dedent(
    ...
    # without importing the heavy module or its `from tunelabs import ...` line.
    ...
    if real_root:
        class _RealFinder(importlib.abc.MetaPathFinder):
            def find_spec(self, fullname, path=None, target=None):
                if fullname.split(".")[0] not in ("tunelabs", "tunelabs_zoo"):
                    return None
    ...
    result["tunelabs_real"] = _real("tunelabs")
    result["tunelabs_zoo_real"] = _real("tunelabs_zoo")
...
def _make_real_pkg(root: Path) -> None:
    (root / "tunelabs").mkdir(parents = True, exist_ok = True)
    (root / "tunelabs" / "__init__.py").write_text(_REAL_TUNELABS_INIT)
    (root / "tunelabs_zoo").mkdir(parents = True, exist_ok = True)
    (root / "tunelabs_zoo" / "__init__.py").write_text(_REAL_ZOO_INIT)
...
def test_only_zoo_shadowed_imports_tunelabs_first(tmp_path):
    """Core concern: a tunelabs_zoo shadow must not make the guard import
    tunelabs_zoo before tunelabs -- that would skip _gpu_init's pre-zoo fixes."""
    shadow = tmp_path / "shadow"
    _make_namespace_shadow(shadow, "tunelabs_zoo")
    res = _run(tmp_path, shadow_roots = [shadow], real = True)

    assert res["error"] is None
    assert res["order"] == ["tunelabs", "tunelabs_zoo"], res
    assert res["sentinel_when_zoo_imported"] == "1", res
    assert res["tunelabs_real"] and res["tunelabs_zoo_real"], res
    assert res["path_restored"], res


def test_both_shadowed_imports_tunelabs_first(tmp_path):
    shadow = tmp_path / "shadow"
    _make_namespace_shadow(shadow, "tunelabs")
    _make_namespace_shadow(shadow, "tunelabs_zoo")
    res = _run(tmp_path, shadow_roots = [shadow], real = True)
...
def test_only_tunelabs_shadowed_recovers(tmp_path):
    shadow = tmp_path / "shadow"
    _make_namespace_shadow(shadow, "tunelabs")
    res = _run(tmp_path, shadow_roots = [shadow], real = True)
...
def test_multiple_shadow_entries_all_removed(tmp_path):
    ...
    _make_namespace_shadow(shadow_a, "tunelabs_zoo")
    _make_namespace_shadow(shadow_b, "tunelabs_zoo")
    ...
    # namespace shadow and tunelabs_zoo_real would be False
    assert res["tunelabs_zoo_real"], res
    assert res["order"] == ["tunelabs", "tunelabs_zoo"], res
    assert res["path_restored"], res
