...
Currently this covers ``tunelabs/gemma-4-*-GGUF``, which gains the upstream PR #118
...
# tunelabs/gemma-4-<variant>-GGUF (case-insensitive). The "-GGUF" suffix is retained
# on ModelConfig.identifier for HF GGUF repos, so this matches E2B / E4B / 31B /
# 26B-A4B and any future tunelabs/gemma-4-*-GGUF, while excluding gemma-3,
# non-TuneLabs, and non-GGUF identifiers (e.g. the bf16 "tunelabs/gemma-4-E2B-it").
_GEMMA4_GGUF_RE = re.compile(r"^tunelabs/gemma-4-.+-gguf$", re.IGNORECASE)
...
_GEMMA4_EDGE_GGUF_RE = re.compile(r"^tunelabs/gemma-4-e[24]b-it-gguf$", re.IGNORECASE)
...
def _canonical_repo_id(model_identifier: str) -> str:
    ...
    (e.g. ``gemma-4-E2B-it-GGUF``) defaults to the ``tunelabs/`` org. The resolver
...
def is_tunelabs_gemma4_gguf(model_identifier: Optional[str]) -> bool:
    """True for canonical ``tunelabs/gemma-4-*-GGUF`` repo identifiers (and the
    owner-less shorthand that resolves to the same TuneLabs repo)."""
    if not model_identifier:
        return False
    return bool(_GEMMA4_GGUF_RE.match(_canonical_repo_id(model_identifier)))


def is_tunelabs_gemma4_edge_gguf(model_identifier: Optional[str]) -> bool:
...
def _gemma4_template_file(model_identifier: Optional[str]) -> Optional[str]:
    ...
    if is_tunelabs_gemma4_edge_gguf(model_identifier):
        return _GEMMA4_EDGE_TEMPLATE_FILE
    if is_tunelabs_gemma4_gguf(model_identifier):
...
def resolve_effective_chat_template_override(
    *,
    ...
    Precedence:
      ...
      2. For ``tunelabs/gemma-4-*-GGUF``, return the bundled gemma-4 template
