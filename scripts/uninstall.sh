...
#
# TuneLabs Studio uninstaller (macOS / Linux / WSL).
...
_pkill_studio() {
    ...
    for _data_dir in "$HOME/.local/share/tunelabs" $(_custom_studio_data_dirs); do
    ...
    # different Studio install (different TUNELABS_STUDIO_HOME) is not touched.
    _kill_roots="$HOME/.tunelabs/studio"
...
$_roots_from_conf"
    ...
    printf '%s\n' "$_kill_roots" | while IFS= read -r _root; do
        ...
        # `tunelabs studio` (default port) + `-p N` + `--port N` forms, all
        # anchored on the install root's venv path.
        for _pat in \
            "${_re}/tunelabs_studio/bin/[^ ]* studio( |\$|.*-p[ =][0-9])" \
            "${_re}/tunelabs_studio/bin/[^ ]* studio.*--port[ =][0-9]" \
...
# env-mode ownership guard at install.sh:1358-1361). A bare tunelabs_studio/
# directory is NOT enough -- require the install-time owner marker so a user
# directory that happens to contain a folder named "tunelabs_studio" is safe.
_is_studio_root() {
    _r="$1"
    [ -n "$_r" ] || return 1
    [ -f "$_r/share/studio.conf" ] && return 0
    [ -f "$_r/tunelabs_studio/.tunelabs-studio-owned" ] && return 0
    if [ -L "$_r/bin/tunelabs" ]; then
        _t=$(readlink "$_r/bin/tunelabs" 2>/dev/null || true)
        case "$_t" in *tunelabs_studio/bin/tunelabs) return 0 ;; esac
    fi
    return 1
}
...
#   1. TUNELABS_STUDIO_HOME / STUDIO_HOME env vars at uninstall time
#   2. Default-mode studio.conf at $HOME/.local/share/tunelabs/studio.conf
#   3. Env-mode studio.conf at $<root>/share/studio.conf (discovered via 1)
# install.sh writes TUNELABS_EXE='<root>/tunelabs_studio/bin/tunelabs', so
...
_custom_studio_roots() {
    ...
    _emit() {
        ...
        [ -n "$_canon" ] && _r="$_canon"
        case "$_r" in "$HOME/.tunelabs/studio"|/|"") return 0 ;; esac
    ...
    _from_conf() {
        ...
        _exe=$(sed -n "s/^TUNELABS_EXE='\(.*\)'\$/\1/p" "$1" | head -n1)
        _exe=$(printf '%s' "$_exe" | sed "s/'\\\\''/'/g")
        [ -n "$_exe" ] || return 0
        _emit "$(dirname "$(dirname "$(dirname "$_exe")")")"
    }
    # Mirror install.sh's precedence: TUNELABS_STUDIO_HOME wins, STUDIO_HOME is
    ...
    if [ -n "${TUNELABS_STUDIO_HOME:-}" ]; then
        _emit "$TUNELABS_STUDIO_HOME"
        _from_conf "$TUNELABS_STUDIO_HOME/share/studio.conf"
    elif [ -n "${STUDIO_HOME:-}" ]; then
        _emit "$STUDIO_HOME"
        _from_conf "$STUDIO_HOME/share/studio.conf"
    fi
    # Default-mode conf.
    _from_conf "$HOME/.local/share/tunelabs/studio.conf"
}

# Remove $HOME/.local/bin/tunelabs only if it's a Studio-managed symlink.
# Studio's install.sh writes this as a symlink into the studio venv
# (install.sh: `ln -sfn "$VENV_DIR/bin/tunelabs" "$_shim_path"`). A
# pip-installed `tunelabs` CLI is a regular file — leave it alone to avoid
# wiping an unrelated install.
_remove_cli_shim() {
    _shim="$HOME/.local/bin/tunelabs"
    [ -L "$_shim" ] || return 0
    _target=$(readlink "$_shim" 2>/dev/null || true)
    case "$_target" in
        */tunelabs_studio/bin/tunelabs) _remove_path "$_shim" ;;
        *) ;;
    esac
}
...
echo "Stopping any running TuneLabs Studio servers..."
_pkill_studio
...
_remove_path "$HOME/.tunelabs/studio"
...
# when absent. A user-set TUNELABS_LLAMA_CPP_PATH is intentionally kept.
_remove_path "$HOME/.tunelabs/llama.cpp"
_remove_path "$HOME/.tunelabs/.cache"
...
_remove_path "$HOME/.tunelabs/.staging"
...
_remove_path "$HOME/.tunelabs/librocdxg"
_remove_path "$HOME/.tunelabs/rocm-smoketest"
# Drop ~/.tunelabs only if now empty (rmdir refuses non-empty, so user content is kept).
rmdir "$HOME/.tunelabs" 2>/dev/null || true
_remove_path "$HOME/.local/share/tunelabs"
# CLI shim: only the symlink Studio created, never a pip-installed file.
_remove_cli_shim

echo "Removing desktop shortcut and launcher lock..."
# install.sh creates Desktop/TuneLabs Studio as a symlink. If the user has an
# unrelated regular directory by that name, leave it alone.
_desktop_link="$HOME/Desktop/TuneLabs Studio"
...
_remove_path "$HOME/Desktop/tunelabs-studio.desktop"
# Locks are namespaced per-uid; env-mode adds an extra suffix.
_lock_glob="${XDG_RUNTIME_DIR:-/tmp}/tunelabs-studio-launcher-${_uid}"
...
case "$_os" in
    Darwin)
        ...
        _remove_path "$HOME/Applications/TuneLabs Studio.app"
        ...
        if [ -x "$_lsr" ]; then
            "$_lsr" -u "$HOME/Applications/TuneLabs Studio.app" 2>/dev/null || true
        fi
        ;;
    Linux)
        if [ "$_is_wsl" = "1" ]; then
            echo "Removing WSL Windows-side shortcuts..."
            # install.sh creates per-distro 'TuneLabs Studio (WSL - <distro>).lnk'
            ...
            # native install's "TuneLabs Studio.lnk". Prefer powershell.exe; test it
               ...
               powershell.exe -NoProfile -Command "exit 0" >/dev/null 2>&1; then
                ...
                powershell.exe -NoProfile -Command '$distro = "'"$_wsl_distro"'";
                    ...
                    foreach ($d in $dirs) {
                        ...
                        Get-ChildItem -LiteralPath $d -Filter "TuneLabs Studio*.lnk" -ErrorAction SilentlyContinue | ForEach-Object {
                            try {
                                ...
                                if ($distro) {
                                    $nameMatch = ($_.Name -eq "TuneLabs Studio (WSL - $distro).lnk");
            ...
            # WSL .lnk files via drvfs. The "TuneLabs Studio (WSL..." name is
            # WSL-specific, so a native install's "TuneLabs Studio.lnk" never matches.
            if [ "$_ps_ran" = "0" ]; then
            ...
            # Remove TuneLabs's own ROCDXG config (the env it persisted). The system
            # ROCm userspace is a shared prereq (like CUDA) and is LEFT IN PLACE by
            # default; set TUNELABS_UNINSTALL_ROCM=1 to remove it too.
            echo "Removing ROCm-on-WSL config..."
            _sudo=""
            if [ "$_uid" != "0" ] && command -v sudo >/dev/null 2>&1; then _sudo="sudo"; fi
            $_sudo rm -f /etc/profile.d/tunelabs-rocm-wsl.sh 2>/dev/null || true
            if [ -f "$HOME/.bashrc" ] && grep -q "TuneLabs ROCm-on-WSL" "$HOME/.bashrc" 2>/dev/null; then
                _bk=$(mktemp 2>/dev/null || echo "$HOME/.bashrc.tunelabs.tmp")
                if sed '/# >>> TuneLabs ROCm-on-WSL/,/# <<< TuneLabs ROCm-on-WSL/d' "$HOME/.bashrc" > "$_bk" 2>/dev/null; then
                    cat "$_bk" > "$HOME/.bashrc" 2>/dev/null || true
                    echo "  cleaned ROCm-on-WSL block from ~/.bashrc"
                fi
                rm -f "$_bk" 2>/dev/null || true
            fi
            if [ "${TUNELABS_UNINSTALL_ROCM:-0}" = "1" ]; then
                echo "  removing system ROCm (TUNELABS_UNINSTALL_ROCM=1)..."
                ...
                echo "        Remove it by re-running with TUNELABS_UNINSTALL_ROCM=1, or manually:"
                echo "          sudo rm -rf /opt/rocm /opt/rocm-* && sudo ldconfig"
            fi
        fi
        echo "Removing Linux .desktop entry..."
        _remove_path "$HOME/.local/share/applications/tunelabs-studio.desktop"
...
echo ""
echo "TuneLabs Studio uninstalled."
...
if [ -z "${TUNELABS_STUDIO_HOME:-}" ] && [ -z "${STUDIO_HOME:-}" ]; then
    echo ""
